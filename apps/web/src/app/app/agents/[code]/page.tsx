...
export default function AgentWorkspacePage() {
  ...
  useEffect(() => {
    ...
    setLoadError("");
    if (code) {
      apiForLocale<Workflow>(`/api/agents/${encodeURIComponent(code)}`, locale, { signal: controller.signal })
        .then((wf) => {
        ...
        .catch((error) => {
          if (error?.name !== "AbortError") setLoadError(publicError(error, "工作流暂时无法加载"));
        });
    } else setLoadError("工作流编码无效");
    return () => {
      controller.abort();
    };
  }, [code, locale]);
  ...
  const run = async (inputs: Record<string, unknown> = form) => {
    ...
    try {
      ...
      setError(publicError(err, t("agentRun.startFailed")));
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const loadHistory = async (id: string) => {
    try {
      ...
      if (p.status === "pending" || p.status === "running" || p.status === "waiting_confirm") startPolling(p.public_id);
        ...
        setError(publicError(err, "历史记录加载失败"));
    }
  };
    ...
    return (
      <div className="flex flex-1 items-center justify-center p-6">
        <div className="text-center">
          <p className="text-sm text-gray-400">{loadError ? "工作流不存在或暂时无法加载" : t("common.loading")}</p>
          {loadError && <p className="mt-2 max-w-md text-xs text-gray-500">{loadError}</p>}
          {loadError && <button type="button" onClick={() => router.push("/app?section=workflows")} className="mt-5 rounded-xl bg-gray-900 px-4 py-2 text-sm font-medium text-white hover:bg-gray-800 dark:bg-white dark:text-gray-900 dark:hover:bg-gray-100">返回工作流</button>}
        </div>
      </div>
    );
  }
  ...
  const publicWorkflowName = publicText(workflow.name, "工作流");
  const publicWorkflowDescription = publicText(workflow.description, "");
  ...
  if (isVirtualTryOn) {
    ...
    return (
      <div className="relative flex min-h-[100dvh] flex-col overflow-hidden bg-[#fff1f3] text-gray-900 dark:bg-[#12070a] dark:text-white">
        ...
        {project ? (
          <div className="relative z-10 flex min-h-0 flex-1 flex-col"><VirtualTryOnResult workflowCode={workflow.code} workflowName={publicWorkflowName} project={project as any} onNewTask={() => { setProject(null); setPhotoInputKey((key) => key + 1); }} onLoadHistory={loadHistory} /></div>
        ) : (
          <div className="relative z-10 flex min-h-0 flex-1 flex-col overflow-y-auto"><VirtualTryOnLanding workflowCode={workflow.code} workflowName={publicWorkflowName} workflowDescription={publicWorkflowDescription} roles={roles} onNewTask={() => setPhotoInputKey((key) => key + 1)} onLoadHistory={loadHistory} /></div>
        )}
        {tryOnBar}
      </div>
    );
        ...
        <div className="relative flex h-screen flex-col overflow-hidden bg-[#fdf0f9] text-gray-900 dark:bg-[#0a0510] dark:text-white">
          ...
          <div className="relative z-10 flex min-h-0 flex-1 flex-col overflow-y-auto">
            <PhotoStudioLanding
              ...
              workflowName={publicWorkflowName}
              workflowDescription={publicWorkflowDescription}
              roles={roles}
          ...
          {project.status === "waiting_confirm" && mediaItems.length === 0 ? (
            ...
            <div className="mx-auto w-full max-w-5xl">
              ...
              <div className="mb-6 flex items-center justify-between gap-4">
                <div><h1 className="text-2xl font-bold text-gray-900 dark:text-white">{publicWorkflowName}</h1><p className="mt-1 text-sm text-gray-500 dark:text-gray-400">{project.status === "succeeded" ? "写真拍摄完成" : project.status === "failed" ? "拍摄失败" : "AI 摄影团队正在拍摄"}</p></div>
              ...
              {project.status === "failed" && (
                <div className="mt-4">
                  <p className="mb-2 text-sm text-red-500">{publicText(project.error_message, "任务暂时失败，请稍后重试")}</p>
    ...
    return (
      <div className="relative flex min-h-[100dvh] flex-col overflow-hidden bg-[#eaf7fb] text-gray-900 dark:bg-[#05080f] dark:text-white">
        ...
        <div className="relative z-10 flex min-h-0 flex-1 flex-col">
          ...
          <div className="mb-6 flex items-center justify-between gap-4">
            <div><h1 className="text-2xl font-bold">{publicWorkflowName}</h1><p className="mt-1 text-sm text-gray-500 dark:text-gray-400">{project.status === "waiting_confirm" ? "等待你的确认" : project.status === "succeeded" ? "全书创作完成" : "AI 编辑部正在协作创作"}</p></div>
    ...
    <div className="flex-1 overflow-y-auto p-8">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h1 className="text-xl font-bold">{publicWorkflowName}</h1>
            <p className="text-sm text-gray-500">{publicWorkflowDescription}</p>
          </div>
        </div>
        ...
        {project && (
          <div className="soft-card p-5">
              ...
              {(project.node_runs || []).map((n) => (
                <div key={n.node_id} className="border border-gray-100 rounded-xl p-4">
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-medium text-gray-900">{publicText(n.name, "处理步骤")}</span>
                  ...
                  {n.error && <p className="text-xs text-red-500 mt-2">{publicText(n.error, "此步骤暂时失败，请稍后重试")}</p>}
                  {n.output?.text != null && (
            ...
            {project.status === "failed" && (
              <div className="mt-4">
                <p className="text-sm text-red-500 mb-2">{publicText(project.error_message, "任务暂时失败，请稍后重试")}</p>
