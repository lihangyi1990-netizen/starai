...
export default function GalleryDetailPage() {
  ...
  useEffect(() => {
    ...
    api<GalleryItem>(`/api/gallery/${encodeURIComponent(id)}`, { signal: controller.signal })
      .then((value) => {
        if (!controller.signal.aborted) setItem(value);
      })
      .catch((cause) => {
        if (!controller.signal.aborted) setError(publicError(cause, "作品暂时无法加载"));
      });
  ...
  const cloneSame = async () => {
    ...
    try {
      ...
      setError(publicError(err, "生成同款失败"));
      setCloning(false);
    }
  };
    ...
    return (
      <div className="flex flex-1 items-center justify-center p-6">
        <div className="text-center">
          {error ? (
            <>
              <p className="text-sm text-gray-400">作品不存在或暂时无法加载</p>
              <p className="mt-2 max-w-md text-xs text-gray-500">{error}</p>
              <button type="button" onClick={() => router.push("/app?section=plaza")} className="mt-5 rounded-xl bg-gray-900 px-4 py-2 text-sm font-medium text-white hover:bg-gray-800 dark:bg-white dark:text-gray-900 dark:hover:bg-gray-100">
                返回模型广场
              </button>
            </>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <div className="rounded-2xl overflow-hidden bg-white border border-gray-100">
            {isVideo && mediaURL ? (
              ...
              <img src={mediaURL} alt={publicText(item.title, "作品")} className="w-full object-cover" />
            ) : (
              <div className="aspect-[4/3] bg-gray-100" />
            )}
          </div>
          <div>
            <h1 className="text-xl font-bold mb-2">{publicText(item.title, "未命名作品")}</h1>
            <div className="flex flex-wrap gap-1.5 mb-4">
              {item.is_paid && (
                <span className="px-2 py-0.5 rounded-full bg-primary/15 text-gray-900 text-xs font-semibold">{item.price || 0} 算力点</span>
              )}
              {item.tags?.map((t) => (
                <span key={t} className="px-2 py-0.5 rounded-full bg-gray-100 text-gray-500 text-xs">#{publicText(t, "标签")}</span>
              ))}
            </div>
            <div className="soft-card p-4 mb-4">
              <div className="text-xs text-gray-400 mb-1">提示词</div>
              <p className="text-sm text-gray-700 leading-relaxed whitespace-pre-wrap">{publicText(item.prompt, "")}</p>
            </div>
            <button
              onClick={cloneSame}
              disabled={cloning}
