...
import type { CashTransaction, ReferralSummary, Wallet, WalletTransaction, WalletUsageSummary, WithdrawalRequest } from "@starai/shared-types";
...
// Reward grants are intentionally kept out of the customer-facing wallet.
// They remain available to administrators for audit and reconciliation.
const HIDDEN_USER_TRANSACTION_TYPES = new Set(["daily_checkin", "signup_bonus"]);
...
export default function WalletPage() {
  ...
  const load = () => {
    ...
    api<{ items: WalletTransaction[] }>("/api/wallet/transactions").then((r) => setTransactions(Array.isArray(r.items) ? r.items.filter((item) => !HIDDEN_USER_TRANSACTION_TYPES.has(item.type)) : [])).catch(() => setTransactions([]));
    api<WalletUsageSummary>("/api/wallet/usage").then(setUsage).catch(() => setUsage(null));
    api<{ items: CashTransaction[] }>("/api/wallet/cash-transactions").then((r) => setCashTransactions(Array.isArray(r.items) ? r.items : [])).catch(() => setCashTransactions([]));
    api<{ items: WithdrawalRequest[] }>("/api/wallet/withdrawals").then((r) => setWithdrawals(Array.isArray(r.items) ? r.items : [])).catch(() => setWithdrawals([]));
  ...
  const submitWithdrawal = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessage(publicError(err, ts("提现申请提交失败")));
    }
  };

  return (
    <div className="pico-premium-account-page pico-premium-wallet-page page-container page-padding max-w-4xl flex-1 overflow-y-auto py-6 sm:py-8">
      ...
      <div className="soft-card mb-6 p-5">
        ...
        <div className="grid gap-3 text-sm sm:grid-cols-4">
          ...
          <Info label={ts("我的上级")} value={publicText(summary?.referrer_name, ts("无"))} />
        </div>
      </div>
                    ...
                    {summary.children.map((child) => (
                      <tr key={child.id}>
                        <td className="px-4 py-3">
                          <div className="font-medium">{publicText(child.nickname, ts("未设置昵称"))}</div>
...
function UsageSummaryPanel({ usage }: { usage: WalletUsageSummary | null }) {
  const { ts } = useI18n();
  const typeOrder = ["chat", "image", "video", "audio", "workflow"];
    ...
    <section className="soft-card mb-6 overflow-hidden">
      <div className="border-b border-gray-100 px-5 py-4 dark:border-white/10">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            <h2 className="text-base font-semibold text-gray-900 dark:text-gray-100">{ts("消费明细")}</h2>
            <p className="mt-1 text-xs text-gray-400">{ts("按已扣费的请求统计；单次费用为该模型平均每次消耗。")}</p>
          </div>
          <div className="text-right">
            <div className="text-2xl font-semibold tabular-nums text-gray-900 dark:text-gray-100">{usage?.total_operations ?? 0}</div>
            <div className="text-[11px] text-gray-400">{ts("总操作次数")}</div>
          </div>
          <div className="text-right">
            <div className="text-2xl font-semibold tabular-nums text-primary">{(usage?.total_cost ?? 0).toFixed(4)}</div>
            <div className="text-[11px] text-gray-400">{ts("总消耗算力")}</div>
          </div>
        </div>
        <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-5">
          {typeOrder.map((type) => (
            <div key={type} className="rounded-xl bg-gray-50 px-3 py-2 dark:bg-white/5">
              <div className="text-[11px] text-gray-400">{ts(USAGE_LABELS[type])}</div>
              <div className="mt-1 text-lg font-semibold tabular-nums text-gray-900 dark:text-gray-100">{byType[type] || 0}<span className="ml-1 text-xs font-normal text-gray-400">{ts("次")}</span></div>
            </div>
          ))}
        </div>
      </div>
      {!usage || usage.items.length === 0 ? (
        <div className="px-5 py-8 text-center text-sm text-gray-400">{ts("暂无已扣费的模型调用")}</div>
      ) : (
        <div className="overflow-x-auto">
          <table className="w-full min-w-[720px] text-sm">
            <thead className="bg-gray-50 text-xs text-gray-500 dark:bg-white/5 dark:text-gray-400">
              <tr>
                <th className="px-5 py-3 text-left font-medium">{ts("操作")}</th>
                <th className="px-4 py-3 text-left font-medium">{ts("模型")}</th>
                <th className="px-4 py-3 text-right font-medium">{ts("次数")}</th>
                <th className="px-4 py-3 text-right font-medium">{ts("平均单次")}</th>
                <th className="px-4 py-3 text-right font-medium">{ts("合计消耗")}</th>
                <th className="px-5 py-3 text-right font-medium">{ts("最近使用")}</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100 dark:divide-white/10">
              {usage.items.map((item) => (
                <tr key={`${item.operation_type}:${item.model_code}`} className="text-gray-700 dark:text-gray-200">
                  <td className="whitespace-nowrap px-5 py-3 font-medium">{ts(USAGE_LABELS[item.operation_type] || item.operation_type)}</td>
                  <td className="px-4 py-3">
                    <div className="font-medium">{publicText(item.model_name, "未命名模型")}</div>
                    <div className="mt-0.5 font-mono text-[11px] text-gray-400">{publicText(item.model_code, "模型编码不可用")}</div>
                  </td>
                  <td className="px-4 py-3 text-right tabular-nums">{item.count}</td>
                  <td className="px-4 py-3 text-right font-mono tabular-nums">{item.unit_cost.toFixed(4)}</td>
                  <td className="px-4 py-3 text-right font-mono font-semibold tabular-nums">{item.total_cost.toFixed(4)}</td>
                  <td className="whitespace-nowrap px-5 py-3 text-right text-xs text-gray-400">{new Date(item.last_used_at).toLocaleString("zh-CN", { hour12: false })}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        items.map((tx) => (
          <div key={tx.id} className="flex items-center justify-between gap-3 px-5 py-4">
            <div className="min-w-0 flex-1">
              <div className="truncate text-sm font-medium">{tx.remark ? publicText(tx.remark, ts("其他流水")) : ts(TX_LABELS[tx.type] || "其他流水")}</div>
        ...
        items.map((w) => (
          <div key={w.id} className="flex items-center justify-between gap-3 px-5 py-4">
            <div>
              ...
              {w.admin_note && <div className="mt-1 text-xs text-gray-500">{publicText(w.admin_note, "")}</div>}
            </div>
            <span className="rounded-full bg-gray-100 px-2 py-1 text-xs text-gray-600">{publicText(w.status, "处理中")}</span>
          </div>
        ))
      )}
    </div>
