...
export function VideoUpscaleWorkspace({ workflow }: { workflow: WorkflowLike }) {
  ...
  const configuredFeatures = (display.steps || []).map((item, index) => ({
    ...item,
    title: publicText(td(`agent.${workflow.code}.step.${index}.title`, item.title), "创作步骤"),
    subtitle: item.subtitle ? publicText(td(`agent.${workflow.code}.step.${index}.subtitle`, item.subtitle), "") : "",
    tags: item.tags?.map((tag) => publicText(td(`agent.${workflow.code}.step.${index}.tag.${tag}`, tag), "能力")),
  }));
  ...
  const heroTags = (display.hero_tags?.length ? display.hero_tags : fallbackFeatures.flatMap((item) => item.tags || []).slice(0, 4))
    .map((tag) => publicText(td(`agent.${workflow.code}.heroTag.${tag}`, tag), "AI 创作"));
  ...
  const selectFile = async (file?: File | null) => {
    ...
    try {
      ...
      setError(publicError(err, ts("视频上传失败")));
    } finally {
      setUploading(false);
    }
  };
  ...
  const selectStyleFile = async (file?: File | null) => {
    ...
    try {
      ...
      setError(publicError(err, ts("风格参考图上传失败")));
    } finally {
      setUploading(false);
    }
  };
  ...
  const openStyleAssets = async () => {
    ...
    try {
      ...
      setStyleAssets([]);
      setError(publicError(err, ts("图片资产库加载失败")));
    }
  };
  ...
  const openAssets = async () => {
    ...
    try {
      ...
      setAssets([]);
      setError(publicError(err, ts("资产库加载失败")));
    }
  };
  ...
  const run = async () => {
    ...
    try {
      ...
      setError(publicError(err, ts("任务启动失败")));
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const retry = async () => {
    ...
    try {
      ...
      setError(publicError(err, ts("重试失败")));
    }
  };
  ...
  const cancel = async () => {
    ...
    try {
      ...
      setError(publicError(err, ts("取消任务失败")));
    }
  };
      ...
      <div className="mx-auto flex min-h-0 w-full max-w-7xl flex-1 flex-col">
        {!project && (
          <AgentLanding
            ...
            workflowName={publicText(td(`agent.${workflow.code}.name`, workflow.name), "未命名工作流")}
            workflowDescription={publicText(td(`agent.${workflow.code}.description`, workflow.description || ""), "")}
            heroTags={heroTags}
            features={features}
