...
const CANVAS_IMAGE_RATIOS = ["1:1", "16:9", "9:16", "4:3", "3:4", "3:2", "2:3"];
const CANVAS_IMAGE_SIZE_TIERS = ["1K", "2K", "4K"];

// Catalog-synchronized image models may not publish an input schema. The
// regular model workbench still exposes these universal image controls, so the
// canvas must provide the same fallback instead of rendering an empty
// "no configurable parameters" panel for multi-image templates.
function canvasImageSchema(schema: Model["input_schema"]) {
  const source = (schema || {}) as Record<string, unknown>;
  const properties = { ...((source.properties as Record<string, unknown> | undefined) || {}) };

  if (!properties.count && !properties.n) {
    properties.count = {
      type: "integer",
      title: "生成数量",
      enum: [1, 2, 3, 4],
  ...
  if (!properties.aspect_ratio && !properties.ratio) {
    properties.aspect_ratio = {
      type: "string",
      title: "图片比例",
      enum: CANVAS_IMAGE_RATIOS,
      default: "1:1",
      "x-widget": "option_menu",
      "x-icon": "ratio",
      "x-order": 91,
  ...
  if (!properties.image_size && !properties.quality) {
    properties.image_size = {
      type: "string",
      title: "清晰度",
      enum: CANVAS_IMAGE_SIZE_TIERS,
      default: "1K",
      "x-widget": "option_menu",
      "x-icon": "quality",
      "x-order": 92,
  ...
  return { ...source, type: source.type || "object", properties };
}

function canvasInputSchema(kind: GeneratorKind, schema: Model["input_schema"]) {
  if (kind === "image") return canvasImageSchema(schema);
...
function NodeFrame({
  ...
  const publicTitle = publicText(title, "处理节点");
  ...
  return (
    <NodeFrame id={id} selected={selected} title={data.label || mediaTitle} icon={mediaIcon}>
      <div className="space-y-2 p-2.5">
        ...
        />
        {data.error && <p className="mt-2 text-[11px] text-red-500">{publicText(data.error, t("canvas.generationFailed"))}</p>}
      </div>
    </NodeFrame>
  );
}
    ...
    >
      <div className="space-y-2.5 p-2.5">
        ...
        >
          ...
          {models.map((model) => (
            <option key={model.code} value={model.code}>{publicText(model.display_name, "未命名模型")}</option>
          ))}
        </select>
        {selectedModel && (
              ...
              <div className="space-y-1.5">
                {storySpeakers.map(([speakerCode, speakerName]) => (
                  <label key={speakerCode} className="flex items-center gap-2 text-[9px] text-gray-500 dark:text-gray-300">
                    <span className="min-w-0 flex-1 truncate" title={`${publicText(speakerName, "角色")} (${speakerCode})`}>{publicText(speakerName, "角色")}</span>
        ...
        {data.warning && (
          <div className="rounded-lg bg-amber-50 px-2.5 py-2 text-[10px] leading-relaxed text-amber-700 dark:bg-amber-500/10 dark:text-amber-300">{publicText(data.warning, "请检查当前设置")}</div>
        )}
        {data.error && (
          <div className="flex items-center gap-2 rounded-lg bg-red-50 px-2.5 py-2 text-[11px] text-red-600 dark:bg-red-500/10 dark:text-red-300">
            <span className="min-w-0 flex-1">{publicText(data.error, t("canvas.generationFailed"))}</span>
...
function CanvasEditor({
  ...
  const upload = useCallback(async (id: string, file: File, append = false) => {
    ...
    try {
      ...
      update(id, { status: "failed", error: publicText(error instanceof Error ? error.message : "", "上传失败") });
    }
  ...
  const uploadReference = useCallback(async (id: string, kind: GeneratorKind, file: File) => {
    ...
    try {
      ...
      update(id, { status: "failed", error: publicText(error instanceof Error ? error.message : "", t("canvas.assetUploadFailed")) });
    }
  }, [authenticated, t, update]);
  ...
  const loadAssetLibrary = useCallback(async (query = "", kind = assetTargetKind) => {
    ...
    try {
      ...
      setAssetItems([]);
      setNotice(publicText(error instanceof Error ? error.message : "", t("canvas.assetLibraryLoadFailed")));
    } finally {
  ...
  const runCompositor = useCallback(async (id: string) => {
    ...
    try {
      ...
      update(id, {
        ...
        error: publicText(task.error_message, ""),
      });
      ...
      for (let attempt = 0; attempt < 240; attempt += 1) {
        ...
        if (["failed", "cancelled"].includes(task.status)) {
          update(id, { status: "failed", progress: 0, error: publicText(task.error_message, t("canvas.generationFailed")) });
          return;
        }
      }
      update(id, { status: "failed", progress: 0, error: t("canvas.generationTimeout") });
    } catch (error) {
      update(id, { status: "failed", progress: 0, error: publicText(error instanceof Error ? error.message : "", t("canvas.generationFailed")) });
    }
  }, [authenticated, t, update]);
        ...
        for (let itemIndex = 0; itemIndex < parsedPlan.items.length; itemIndex += 1) {
          ...
          if (speechTask.status === "failed") {
            update(id, {
              ...
              error: t("canvas.story.speechFailed", { index: itemIndex + 1, reason: publicText(speechTask.error_message, t("canvas.generationFailed")) }),
            });
            return;
          }
          let finished = false;
  ...
  const executeNodes = useCallback(async (scope?: Set<string>) => {
    ...
    if (missingModel) {
      ...
      setNotice(t("canvas.nodeNeedsModel", { name: publicText(missingModel.data.label, "处理节点") }));
      return;
    }
    ...
    if (missingPrompt) {
      ...
      setNotice(t("canvas.nodeInvalid", { name: publicText(missingPrompt.data.label, "处理节点"), reason: t("canvas.enterOrConnectPrompt") }));
      return;
    }
    ...
    if (invalidViralAnalysis) {
      ...
      setNotice(t("canvas.nodeInvalid", {
        name: publicText(invalidViralAnalysis.node.data.label, "处理节点"),
    ...
    if (invalidReferenceAudio) {
      ...
      setNotice(t("canvas.nodeInvalid", {
        name: publicText(invalidReferenceAudio.node.data.label, "处理节点"),
        reason,
      }));
      return;
    }
      ...
      for (let index = 0; index < ordered.length; index += 1) {
        ...
        if (unavailableDependency) {
          ...
          update(snapshot.id, {
            status: "blocked",
            dirty: true,
            error: t("canvas.upstreamFailed", { name: publicText(unavailableDependency.data.label, "前置节点") }),
          });
          continue;
        }
  ...
  const appendTemplate = useCallback((templateID: string, requestedFlowName?: string) => {
    ...
    if (templateID === "text-image" || templateID === "text-video") {
      ...
      setNotice(t("canvas.unsupportedTemplate", { name: publicText(flowName, "工作流模板") }));
      return;
    }
  ...
  const save = useCallback(async (silent = false) => {
    ...
    try {
      ...
      setTitle(publicText(item.title, t("canvas.untitled")));
      if (!silent) setNotice(t("canvas.saved"));
      refreshHistory();
    } catch (error) {
      setNotice(publicText(error instanceof Error ? error.message : "", t("canvas.saveFailed")));
    } finally {
      setSaving(false);
    }
  ...
  const loadCanvas = useCallback(async (id: string) => {
    try {
      ...
      setCanvasID(item.public_id);
      const publicCanvasTitle = publicText(item.title, t("canvas.untitled"));
      workflowNameRef.current = publicCanvasTitle;
      titleManuallyEditedRef.current = true;
      setTitle(publicCanvasTitle);
      ...
      setNotice(publicText(error instanceof Error ? error.message : "", t("canvas.loadFailed")));
    }
  ...
  const deleteCanvas = useCallback(async (event: React.MouseEvent, id: string) => {
    ...
    try {
      ...
      setNotice(publicText(error instanceof Error ? error.message : "", t("canvas.deleteFailed")));
    }
  ...
  const importCanvas = useCallback(async (file: File) => {
    try {
      ...
      setCanvasID("");
      const importedTitle = publicText(parsed.title || file.name.replace(/\.starai-canvas\.json$|\.json$/i, ""), t("canvas.importCanvas"));
      ...
      setNotice(publicText(error instanceof Error ? error.message : "", t("canvas.importFailed")));
    }
  }, [setEdges, setNodes, setViewport, t]);

  const importCanvasDocument = useCallback((template: CanvasTemplate) => {
    if (!template.document) {
      appendTemplate(template.template_id || template.id, publicText(template.name, t("canvas.untitled")));
      setImportOpen(false);
      return;
    }
    ...
    setCanvasID("");
    const templateTitle = publicText(template.name, t("canvas.untitled"));
    ...
    const templateNodes = normalizeCanvasNodes(template.id === "pasted"
      ...
      : document.nodes.map((node) => node.type === "textInput"
        ? { ...node, data: { ...node.data, label: publicText(template.name || node.data.label, "处理节点") } }
        : node));
  ...
  const importFromCode = useCallback(() => {
    try {
      ...
      importCanvasDocument({
        id: "pasted",
        name: publicText(parsed.title, t("canvas.importedCanvas")),
        document: parsed,
      });
      setImportCode("");
    } catch (error) {
      setNotice(publicText(error instanceof Error ? error.message : "", t("canvas.invalidFile")));
    }
      ...
      <div ref={editorRef} className="relative min-h-0 w-full flex-1 overflow-hidden overscroll-none bg-[#eef3f8] dark:bg-[#080d14]">
                  ...
                  {history.length ? history.map((item) => (
                    <button key={item.public_id} type="button" onClick={() => void loadCanvas(item.public_id)} className="group flex w-full items-center gap-2 rounded-xl px-3 py-2 text-left hover:bg-gray-50 dark:hover:bg-white/5">
                      <div className="min-w-0 flex-1">
                        <div title={publicText(item.title, t("canvas.untitled"))} className="truncate text-xs font-medium text-gray-800 dark:text-gray-100">{publicText(item.title, t("canvas.untitled"))}</div>
        ...
        {resultPreview && createPortal(
          <div
            role="dialog"
            aria-modal="true"
            aria-label={publicText(resultPreview.title, t("canvas.untitled"))}
          ...
          >
            ...
            >
              ...
              {resultPreview.kind === "video" ? (
                ...
                // eslint-disable-next-line @next/next/no-img-element
                <img src={resultPreview.url} alt={publicText(resultPreview.title, t("canvas.untitled"))} className="h-auto max-h-[88dvh] w-auto max-w-full object-contain" />
              )}
            </div>
          </div>,
          document.body
