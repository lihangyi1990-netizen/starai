...
function AssetGridCard({
  ...
  const publicTitle = publicText(title, "未命名素材");
  return (
    <div
      role="button"
      tabIndex={locked ? -1 : 0}
    ...
    >
      <div className="relative aspect-[4/5] bg-gray-100 overflow-hidden">
        {k === "image" && coverUrl ? (
          ...
          <img src={coverUrl} alt={publicTitle} className="w-full h-full object-cover" />
        ) : (
      ...
      <div className="px-2 py-2">
        <div className="text-[12px] font-medium text-gray-900 truncate">{publicTitle}</div>
      </div>
    </div>
  );
}
...
export function ChatTopTools({
  ...
  const deleteUserAsset = async (asset: AssetItem) => {
    ...
    const publicTitle = publicText(title, "该素材");
    if (typeof window !== "undefined" && !window.confirm(td("asset.confirmDelete", "Delete asset \"{name}\"? It can no longer be used after deletion.", { name: publicTitle }))) {
      return;
    }
    ...
    try {
      ...
      setAssetNotice({ type: "success", message: td("asset.deleted", '"{name}" deleted.', { name: publicTitle }) });
      if (assetOpen && assetTab === "mine") await loadAssets();
    } catch (err) {
      setAssetNotice({ type: "error", message: publicError(err, t("asset.uploadFailed")) });
    } finally {
    ...
    <div className="flex flex-wrap items-center gap-1.5 sm:gap-2 min-w-0 max-w-full">
        ...
        <div className="scroll-x-only flex flex-nowrap items-center gap-1.5 max-w-full sm:max-w-[420px] shrink-0">
          {value.files.slice(0, 5).map((f) => (
            <div key={f.public_id} className="flex items-center gap-1.5 px-2 py-1 rounded-xl bg-gray-50 border border-gray-200 text-xs text-gray-600 dark:bg-white/5 dark:border-white/10 dark:text-gray-300">
              <span className="truncate max-w-[120px]">{publicText(f.name, "附件")}</span>
              <button
                type="button"
      ...
      {showRole && (
        <button
          ...
          title={publicText(value.role_name, t("role.select"))}
        >
          {value.role_icon_url ? (
            // eslint-disable-next-line @next/next/no-img-element
          ...
          <span className="truncate hidden sm:inline whitespace-nowrap">{publicText(value.role_name, t("role.select"))}</span>
        </button>
      )}
                        ...
                        <div key={r.id} className="w-full bg-white border border-gray-100 rounded-2xl px-4 py-3 flex items-center justify-between gap-3 hover:border-gray-200 transition dark:bg-white/5 dark:border-white/10 dark:hover:border-white/20">
                          <div className="flex items-center gap-3 min-w-0">
                            ...
                            <div className="min-w-0">
                              <div className="font-semibold text-gray-900 truncate dark:text-gray-100">{publicText(r.name, "未命名角色")}</div>
                              <div className="text-xs text-gray-500 mt-0.5 line-clamp-1">{publicText(r.description, "")}</div>
                            </div>
                          </div>
                          <button
                            type="button"
                      ...
                      {tpls.map((t) => (
                        ...
                        >
                          <div className="flex items-center gap-3">
                            ...
                            <div className="min-w-0">
                              <div className="text-sm font-semibold text-gray-900 truncate dark:text-gray-100">{publicText(t.name, "未命名模板")}</div>
                              <div className="text-[11px] text-gray-500 mt-0.5 line-clamp-2">{publicText(t.description, "")}</div>
                            </div>
                          </div>
                        </button>
                      ))}
                          ...
                          {imageAssetItems.map((a) => {
                            ...
                            return (
                              <AssetGridCard
                                key={a.public_id}
                                coverUrl={a.url}
                                title={publicText(a.name || a.public_id, "未命名素材")}
                                tag={t("图片")}
                                kind="image"
                                selected={selected}
                                locked={deletingAssetId === a.public_id}
                                onClick={() => toggleRefPick({ url: a.url, name: publicText(a.name || a.public_id, "未命名素材"), public_id: a.public_id })}
                        ...
                        {filteredGalleryItems.map((item) => {
                          ...
                          return (
                            <ReferencePickCard
                              ...
                              title={publicText(item.title, t("未命名"))}
                              ...
                              onClick={() => {
                                ...
                                toggleRefPick({ url: item.cover_url, name: publicText(item.title, "灵感参考图") });
                              }}
                            />
                          );
                        })}
                        ...
                        {pickedRefs.map((img) => (
                          <div key={img.url} className="flex h-10 min-w-[130px] max-w-[180px] items-center gap-1.5 rounded-xl border border-gray-100 bg-white p-1 dark:border-white/10 dark:bg-white/5">
                            <div className="h-8 w-8 shrink-0 overflow-hidden rounded-lg border border-gray-200 bg-gray-50 dark:border-white/10 dark:bg-white/10">
                              {/* eslint-disable-next-line @next/next/no-img-element */}
                              <img src={img.url} alt={publicText(img.name, "参考图片")} className="w-full h-full object-cover" />
                            </div>
                            <div className="min-w-0 flex-1 truncate text-xs font-medium text-gray-900 dark:text-gray-100">{publicText(img.name, "参考图片")}</div>
                            <button
                              type="button"
                      ...
                      return (
                        <div key={a.public_id} className="min-w-[180px] bg-white border border-gray-100 rounded-2xl p-2 flex items-center gap-2 dark:bg-white/5 dark:border-white/10">
                          ...
                          <div className="min-w-0 flex-1">
                            <div className="text-sm font-semibold text-gray-900 truncate dark:text-gray-100">{publicText(a.name || a.public_id, "未命名素材")}</div>
            ...
            <div className="p-6">
              ...
              {selectedUploadFile && (
                <div className="mt-3 px-4 py-3 rounded-2xl bg-white border border-gray-100 text-sm text-gray-700 flex items-center justify-between gap-3 dark:bg-white/5 dark:border-white/10 dark:text-gray-200">
                  <div className="min-w-0">
                    <div className="font-semibold truncate">{publicText(selectedUploadFile.name, "待上传文件")}</div>
      ...
      {assetPreview && (
        <div className="fixed inset-0 z-[80] bg-black/50 flex items-center justify-center p-4" onClick={() => setAssetPreview(null)}>
          <div className="bg-white rounded-2xl w-full max-w-4xl shadow-2xl overflow-hidden dark:bg-gray-900 dark:border dark:border-white/10" onClick={(e) => e.stopPropagation()}>
            <div className="px-6 py-4 border-b flex items-center justify-between dark:border-white/10">
              <div className="font-bold text-gray-900 truncate dark:text-gray-100">{publicText(assetPreview.name || assetPreview.public_id, "未命名素材")}</div>
