...
export function PricingModal({
  ...
  useEffect(() => {
    ...
    api<Model[]>("/api/models")
      ...
      .catch((e) => setErr(publicError(e, ts("加载失败"))))
                      ...
                      <div className="flex gap-3">
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center justify-between gap-2">
                            <div className="truncate text-sm font-semibold text-gray-900 dark:text-gray-100">{publicText(m.display_name, "未命名模型")}</div>
                            <span className={clsx("shrink-0 rounded-full px-1.5 py-0.5 text-[10px]", tag.className)}>{tag.label}</span>
                          </div>
                          <div className="mt-1 line-clamp-2 text-[11px] text-gray-400">{publicText(m.description, publicText(m.code, "模型信息暂不可用"))}</div>
                        </div>
                      </div>
                    </button>
                  );
              ...
              <div className="space-y-4">
                <div className="soft-card p-5">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0">
                      <div className="truncate text-lg font-bold text-gray-900 dark:text-gray-100">{publicText(active.display_name, "未命名模型")}</div>
                      <div className="mt-0.5 truncate text-xs text-gray-400">{publicText(active.code, "模型编码不可用")}</div>
                    </div>
                  </div>
                  {active.description && <div className="mt-3 whitespace-pre-wrap text-sm leading-relaxed text-gray-600 dark:text-gray-300">{publicText(active.description, "")}</div>}
                  {active.tags?.length ? (
                    <div className="mt-3 flex flex-wrap gap-1.5">
