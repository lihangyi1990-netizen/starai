    ...
    <div className="flex min-h-0 flex-1 flex-col px-3 py-2 sm:px-5 lg:px-8">
      ...
      <div className="mx-auto flex w-full max-w-[1040px] flex-1 flex-col justify-center py-3">
        <div className="text-center">
          <div className="inline-flex items-center gap-2 rounded-full bg-blue-100 px-3 py-1 text-xs font-semibold text-blue-700 dark:bg-blue-600/15 dark:text-blue-200"><Shirt size={13} />AI 视觉试穿</div>
          <div className="mt-3 flex items-center justify-center gap-3"><div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-blue-100 text-2xl dark:bg-blue-600/15">👗</div><h1 className="text-2xl font-black tracking-tight text-gray-900 dark:text-white sm:text-4xl">{publicText(workflowName, "AI试衣间")}</h1></div>
          <p className="mx-auto mt-2 max-w-3xl text-sm leading-6 text-gray-500 dark:text-slate-300">{publicText(workflowDescription, "上传人物照和服装图，快速生成自然的视觉试穿效果。")}</p>
          <div className="mt-2 flex flex-wrap justify-center gap-2">{(heroTags?.length ? heroTags : ["双图试穿", "人物保真", "服装还原"]).map((tag) => <span key={tag} className="hero-tag">{publicText(tag, "AI 能力")}</span>)}</div>
        </div>
        {!!steps?.length && <div className="mx-auto mt-4 grid w-full grid-cols-2 gap-2 sm:grid-cols-4">{steps.map((step, index) => <div key={index} className="rounded-2xl border border-white bg-white/75 p-3 dark:border-white/10 dark:bg-white/5"><div className="text-lg">{step.icon}</div><div className="mt-1 text-xs font-semibold text-gray-800 dark:text-gray-100">{publicText(step.title, "创作步骤")}</div><div className="mt-0.5 line-clamp-2 text-[10px] leading-4 text-gray-400">{publicText(step.subtitle, "")}</div></div>)}</div>}
        <div className="mx-auto mt-4 w-full">
          <h2 className="text-center text-sm font-bold text-blue-600 dark:text-blue-300 sm:text-base">为你效力的 AI 穿搭团队</h2>
          <div className="mx-auto mt-3 grid max-w-[640px] grid-cols-4 gap-2 sm:gap-4">{team.map((role) => <div key={role.id} title={publicText(role.description, "AI 协作角色")} className="flex flex-col items-center gap-1.5 text-center"><div className="relative"><img src={(role.avatar || "").startsWith("/") ? role.avatar : TEAM_AVATARS[role.id] || DEFAULT_TEAM[0].avatar} alt={publicText(role.name, "协作角色")} width={120} height={120} className="h-12 w-12 rounded-full object-cover shadow-sm ring-1 ring-blue-200 dark:ring-blue-500/30" /><span className="absolute -bottom-0.5 -right-0.5 h-3 w-3 rounded-full border-2 border-white bg-emerald-400 dark:border-[#12070a]" /></div><span className="whitespace-nowrap text-[11px] text-gray-600 dark:text-slate-300">{publicText(role.name, "协作角色")}</span></div>)}</div>
        </div>
      </div>
    </div>
  );
...
function ImagePick({ label, value, uploading, onUpload, onClear }: { label: string; value: Pick | null; uploading: boolean; onUpload: (file?: File) => void; onClear: () => void }) {
  return value ? (
    ...
    <label className="flex h-24 w-20 shrink-0 cursor-pointer flex-col items-center justify-center gap-1 rounded-2xl border border-dashed border-gray-200 bg-white text-[10px] text-gray-400 shadow-sm hover:border-blue-300 hover:bg-blue-50/40 dark:border-white/10 dark:bg-white/5 dark:text-gray-300"><>{uploading ? <Loader2 size={19} className="animate-spin text-blue-500" /> : <Upload size={19} />}</><span className="px-1 text-center">{uploading ? "上传中" : label}</span><input type="file" accept="image/png,image/jpeg,image/webp" className="hidden" disabled={uploading} onChange={(event) => { onUpload(event.target.files?.[0]); event.currentTarget.value = ""; }} /></label>
  );
}
...
export function VirtualTryOnInputBar({ defaultModelCode, initialInputs, error, featureTags, onSubmit }: { defaultModelCode?: string; initialInputs?: Record<string, any>; error?: string; featureTags?: string[]; onSubmit: (inputs: Record<string, any>) => void | Promise<void> }) {
  ...
  catch (err) { setLocalError(publicText(err instanceof Error ? err.message : "图片上传失败", "图片上传失败")); }
  ...
  const label = (items: { value: string; label: string }[], value: string) => publicText(items.find((item) => item.value === value)?.label || value, "未选择");
  ...
  return (
    <div className="relative z-20 shrink-0 px-3 pb-3 pt-1 sm:px-6 sm:pb-5">
      <div className="mx-auto w-full max-w-[1040px]">
        {!!featureTags?.length && <div className="mb-1.5 flex flex-wrap justify-center gap-1.5">{featureTags.map((tag) => <span key={tag} className="rounded-full border border-blue-100 bg-white/70 px-2.5 py-0.5 text-[10px] text-blue-600 dark:border-blue-500/20 dark:bg-white/5 dark:text-blue-300">{tag}</span>)}</div>}
        {(error || localError) && <p className="mb-2 px-1 text-sm text-red-500">{publicText(localError || error, "操作失败，请稍后重试")}</p>}
        <div className="soft-input overflow-hidden">
          ...
          <div className="flex flex-wrap items-center justify-between gap-2 border-t border-gray-50 px-3 py-3 dark:border-white/10">
            <div className="flex flex-wrap items-center gap-2">
              <MediaOptionMenu icon={<Sparkles size={15} />} activeLabel={publicText(models.find((item) => item.code === model)?.display_name || model, "模型")} title="试衣模型" subtitle="仅显示支持多参考图的图片模型" compactOnMobile menuWidth={300}>
                {(close) => <div className="space-y-2">{models.length === 0 ? <div className="px-2 py-3 text-center text-xs text-gray-400">暂无可用的试衣模型</div> : models.map((item) => <MediaMenuOption key={item.code} selected={model === item.code} onClick={() => { setModel(item.code); close(); }}>{publicText(item.display_name, "未命名模型")}</MediaMenuOption>)}</div>}
              </MediaOptionMenu>
