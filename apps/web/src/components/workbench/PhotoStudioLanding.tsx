...
export function PhotoStudioTopBar({ workflowCode, onNewTask, onLoadHistory, historyFallbackTitle = "写真任务" }: TopBarProps) {
  ...
  const toggleHistory = async () => {
    ...
    try {
      ...
      setHistoryItems([]);
      setHistoryError(publicText(err instanceof Error ? err.message : "历史记录加载失败", "历史记录加载失败"));
    }
  };
...
export function PhotoStudioInputBar({ defaultModelCode, error, featureTags, onSubmit }: InputBarProps) {
  ...
  const upload = async (file?: File) => {
    ...
    try {
      ...
      setPhoto(null);
      setUploadError(publicText(err instanceof Error ? err.message : "照片上传失败", "照片上传失败"));
    } finally {
      setUploading(false);
    }
  };
  ...
  const modelLabel = publicText(models.find((item) => item.code === model)?.display_name || model, "出图模型");
              ...
              {(close) => (
                <div className="space-y-2">
                  {models.length === 0 ? <div className="px-2 py-3 text-center text-xs text-gray-400">暂无可用的图片模型</div> : models.map((item) => (
                    <MediaMenuOption key={item.code} selected={model === item.code} onClick={() => { setModel(item.code); close(); }}>{publicText(item.display_name, "未命名模型")}</MediaMenuOption>
                  ))}
                </div>
              )}
            </MediaOptionMenu>
