...
function InteractiveHeroCanvas() {
  ...
  useEffect(() => {
    ...
    const drawGrid = () => {
      ...
      ctx.strokeStyle = "rgba(126,103,232,0.35)";
      ctx.lineWidth = 1;
      const gap = 58;
    ...
    const draw = () => {
      ...
      bg.addColorStop(0, "#151736");
      bg.addColorStop(0.34, "#2a1c52");
      bg.addColorStop(0.68, "#173d53");
      bg.addColorStop(1, "#1e1b48");
      ctx.fillStyle = bg;
      ...
      nodes.forEach((p, i) => {
        ...
        ctx.fillStyle = `rgba(${pull > 0 ? "78,244,201" : "182,109,255"},${0.3 + pull * 0.34})`;
        ctx.fill();
        for (let j = i + 1; j < nodes.length; j++) {
          ...
          if (dist < 120) {
            ...
            ctx.moveTo(x, y);
            ctx.lineTo(bx, by);
            ctx.strokeStyle = `rgba(118,157,255,${0.2 * (1 - dist / 120)})`;
            ctx.stroke();
          }
        }
      });
      ...
      logos.forEach((logo, idx) => {
        ...
        ctx.fillStyle = "rgba(25, 24, 62, 0.88)";
        ctx.strokeStyle = "rgba(112, 241, 208, 0.48)";
        ctx.lineWidth = 1;
        ctx.beginPath();
        ctx.roundRect(logo.x - tw / 2, logo.y - 16, tw, 30, 15);
        ctx.fill();
        ctx.stroke();
        ctx.fillStyle = "rgba(255, 244, 209, 0.94)";
...
export default function LandingPageClient() {
  ...
  const enterAppOrLogin = (mode?: "chat" | "image" | "video" | "audio" | "workflow") => {
    const hasToken = token || hasUserSession();
    if (hasToken) {
      router.push(mode === "workflow" ? "/app?section=workflows" : mode ? `/app?mode=${mode}` : "/app");
      return;
    }
    setShowLogin(true);
  };
  ...
  return (
      <div className="pico-landing pico-premium-landing min-h-screen overflow-hidden bg-[#07080b] text-white">
      <style jsx global>{`
      ...
      `}</style>
      <section className="pico-clubhouse-hero relative min-h-screen overflow-hidden">
        <div className="pico-clubhouse-sky" />
        <nav className="pico-clubhouse-nav relative z-[70] mx-auto flex max-w-[1440px] items-center justify-between gap-3 px-5 py-5 sm:px-8">
          <SiteBrand href="/" className="min-w-0 flex-1 gap-2 pr-1" nameClassName="text-lg font-black text-[#68473e] sm:text-xl" subtitleClassName="max-w-[130px] text-xs text-[#a67765] sm:max-w-none sm:text-sm" badgeClassName="h-10 w-10 rounded-xl text-sm" />
          <div className="flex shrink-0 items-center gap-1.5 sm:gap-3">
            {apiDocsVisible && <Link href="/app/api-docs" className="hidden rounded-xl border border-[#e9be94] bg-[#fffaf0] px-4 py-2 text-sm font-semibold text-[#8b6256] transition hover:-translate-y-0.5 hover:border-[#db8d79] sm:inline-flex">{t("landing.apiDocs")}</Link>}
            <button onClick={() => enterAppOrLogin()} className="h-10 max-w-[62px] truncate whitespace-nowrap rounded-xl border border-[#e9be94] bg-[#fffaf0] px-3 text-xs font-bold leading-none text-[#805c51] transition hover:-translate-y-0.5 sm:max-w-none sm:px-5 sm:text-sm">{t("landing.login")}</button>
            <button onClick={() => enterAppOrLogin()} className="h-10 max-w-[82px] truncate whitespace-nowrap rounded-xl bg-[#ffb36f] px-3 text-xs font-black leading-none text-[#5b3e38] shadow-[3px_3px_0_#d98970] transition hover:-translate-y-0.5 min-[390px]:max-w-[96px] sm:max-w-none sm:px-5 sm:text-sm"><span className="block truncate">{t("landing.start")}</span></button>
            <UILanguageSelector compact />
          </div>
        </nav>

        <main className="pico-clubhouse-main relative z-10 mx-auto flex min-h-[calc(100vh-88px)] w-full max-w-[1440px] flex-col justify-center px-5 pb-12 pt-3 sm:px-8 lg:pt-0">
          <div className="grid items-center gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(520px,.94fr)] lg:gap-14">
            <div className="pico-clubhouse-copy max-w-3xl">
              <div className="pico-clubhouse-badge"><Sparkles size={16} /> {displayName} 小动物创作屋 · {displayName}</div>
              <h1>一张创作桌，<br /><span>玩遍所有 AI。</span></h1>
              <p className="pico-clubhouse-desc">对话、生图、视频和音频都在同一张桌子上。选模型、上传素材、开始生成，不需要在页面之间来回跳转。</p>
              <div className="pico-clubhouse-actions">
                <button type="button" onClick={() => enterAppOrLogin("chat")}><MessageCircle size={19} /> 对话</button>
                <button type="button" onClick={() => enterAppOrLogin("image")}><ImageIcon size={19} /> 生图</button>
                <button type="button" onClick={() => enterAppOrLogin("video")}><Play size={19} /> 视频</button>
                <button type="button" onClick={() => enterAppOrLogin("audio")}><Headphones size={19} /> 音频</button>
                <button type="button" onClick={() => enterAppOrLogin("workflow")}><Boxes size={19} /> 工作流</button>
              </div>
              <button onClick={() => enterAppOrLogin()} className="pico-clubhouse-start">进入 {displayName} 创作台 <ArrowRight size={19} /></button>
            </div>

            <div className="pico-clubhouse-stage" aria-label={`${displayName} 小动物创作桌`}>
              <div className="pico-clubhouse-window"><span /><span /><span /></div>
              <PixelPet kind="cat" size={164} className="pico-clubhouse-cat" />
              <PixelPet kind="bunny" size={132} className="pico-clubhouse-bunny" />
              <PixelPet kind="bear" size={112} className="pico-clubhouse-bear" />
              <div className="pico-clubhouse-screen"><div className="pico-clubhouse-screen-tabs"><span>对话</span><span>生图</span><span>视频</span><span>音频</span><span>工作流</span></div><div className="pico-clubhouse-screen-body"><span>今天想和 {displayName} 一起做什么？</span><i>✦</i></div></div>
              <div className="pico-clubhouse-note">模型切换就在输入框里</div>
            </div>
          </div>

          <div className="pico-clubhouse-stats mt-8 grid grid-cols-2 gap-3 sm:grid-cols-4">
            {[["01", "一个主页"], ["04", "创作模式"], ["∞", "模型可切换"], ["✦", "工作流接力"]].map(([value, label]) => (
              <div key={label} className="pico-clubhouse-stat"><div>{value}</div><span>{label}</span></div>
            ))}
          </div>
        </main>
      </section>

      <div className="pico-legacy-landing-sections">
