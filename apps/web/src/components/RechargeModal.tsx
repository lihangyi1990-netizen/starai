...
export function RechargeModal({ open, onClose, onSuccess }: Props) {
  ...
  const handleRedeem = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(publicError(err, t("recharge.failed")));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleOnline = async () => {
    ...
    try {
      ...
      setError(publicError(err, t("recharge.paymentFailed")));
    } finally {
      setLoading(false);
    }
  };
              ...
              <div className="space-y-4">
                {packages.length > 0 ? <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
                  {packages.map((item) => (
                    ...
                    >
                      {item.badge ? <span className="absolute -right-1.5 -top-2 max-w-[90%] truncate rounded-full bg-amber-500 px-1.5 py-0.5 text-[9px] font-medium text-white">{publicText(item.badge, "")}</span> : null}
                      ...
                      {item.name && item.name !== `${item.amount} ${config?.payment_currency || ""}` ? <span className="mt-1 block truncate text-[10px] font-normal opacity-65">{publicText(item.name, "充值套餐")}</span> : null}
                    </button>
                  ))}
