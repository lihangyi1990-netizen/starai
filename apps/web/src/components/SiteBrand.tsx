...
const DEFAULT_BRANDING: BrandData = {
  ...
  site_logo: "",
  site_favicon: "",
  site_description: "把 AI 变简单的创作工作台",
...
export function SiteBrand({
  ...
  const siteName = site_name || DEFAULT_BRANDING.site_name || "tuna";
  ...
  return (
    <Link href={href} className={clsx("flex min-w-0 items-center gap-2.5", className)}>
      <div
        className={clsx(
          "pico-brand-mark flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-xl bg-primary text-sm font-bold text-dark",
          badgeClassName
        )}
      >
        {site_logo ? (
