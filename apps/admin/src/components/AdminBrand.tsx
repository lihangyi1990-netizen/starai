...
async function fetchBranding() {
  ...
  if (!brandPromise) {
    brandPromise = fetch(`${API_URL}/api/system-configs/public`)
      ...
      .catch(() => ({ site_name: "tuna", site_logo: "", site_description: "AI 大模型聚合平台", admin_site_description: "管理后台" }));
  }
  return brandPromise;
}
...
export function useAdminBranding() {
  const [branding, setBranding] = useState<BrandData>(
    brandCache || { site_name: "tuna", site_logo: "", site_description: "AI 大模型聚合平台", admin_site_description: "管理后台" }
  );
