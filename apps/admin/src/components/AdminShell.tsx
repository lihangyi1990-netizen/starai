...
import { adminApi, clearAdminSession, hasAdminSession } from "@/lib/api";
import { AdminBrand } from "./AdminBrand";

interface Sub2APIConsoleConfig {
  available: boolean;
  url?: string;
}
...
const NAV_GROUPS = [
  ...
  },
  {
    title: "模型管理",
    items: [
      { href: "/admin/models", label: "模型与售价", icon: Boxes },
  ...
  },
  {
    title: "增长与内容",
    items: [
      { href: "/admin/system-config#payment-channel-settings", label: "充值渠道", icon: CreditCard },
...
// Keep infrequent configuration and content tools available without letting
// them compete with the daily operations in the primary navigation.
...
export function AdminShell({ children }: { children: React.ReactNode }) {
  ...
  const [sub2apiConsole, setSub2apiConsole] = useState<Sub2APIConsoleConfig | null>(null);

  const advancedActive = ADVANCED_NAV_ITEMS.some((item) => pathname === item.href || pathname.startsWith(`${item.href}/`));
  ...
  useEffect(() => {
    ...
    setReady(true);
    // This endpoint is super-admin-only. A 403 for an operations account is
    // expected and simply keeps the separate upstream console link hidden.
    adminApi<Sub2APIConsoleConfig>("/sub2api/config")
      .then((config) => setSub2apiConsole(config?.available && config.url ? config : null))
      .catch(() => setSub2apiConsole(null));
  }, [router]);

  useEffect(() => {
    // Never leave an active secondary page visually hidden behind a collapsed
    // group when navigating directly to its URL or using the browser buttons.
    ...
    <div className="flex min-h-screen bg-[#f3f5f9] text-gray-900">
      <aside className="sticky top-0 flex h-screen w-[272px] shrink-0 flex-col border-r border-gray-200/80 bg-white/95 backdrop-blur">
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-4">
          ...
          <div className="mb-6">
            <button
              ...
              aria-controls="admin-advanced-navigation"
              className={clsx(
            ...
            >
              ...
              <ChevronDown size={16} className={clsx("transition-transform", advancedOpen && "rotate-180")} />
            </button>
            {advancedOpen && (
              <div id="admin-advanced-navigation" className="mt-1 space-y-1 border-l border-gray-200 pl-2">
                {ADVANCED_NAV_ITEMS.filter((item) => item.href !== "/admin/admin-accounts" || adminRole === "super_admin").map((item) => {
                  const Icon = item.icon;
                  const active = pathname === item.href || pathname.startsWith(`${item.href}/`);
                  return (
                    <Link
        ...
        <div className="border-t border-gray-100 p-4">
          {sub2apiConsole?.available && sub2apiConsole.url && (
            <a
              href={sub2apiConsole.url}
              target="_blank"
              rel="noopener noreferrer"
              referrerPolicy="no-referrer"
              className="mb-3 flex h-11 w-full items-center justify-center gap-2 rounded-2xl border border-gray-200 bg-gray-50 text-sm font-medium text-gray-700 transition hover:border-gray-300 hover:bg-gray-100 hover:text-gray-950"
              title="打开独立的 Sub2API 管理后台"
            >
              <ExternalLink size={16} />
              <span>Sub2API 管理后台</span>
            </a>
          )}
