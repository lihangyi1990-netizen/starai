...
import { PicoCatalogPricingEditor, type PicoCatalogPricingModel } from "@/components/PicoCatalogPricingEditor";
...
function isSub2APICatalogModel(model: Pick<AdminModel, "new_api_extra_params">) {
  return String(model.new_api_extra_params?.catalog_source || "").trim().toLowerCase() === "sub2api";
}

function picoPriceReady(model: Pick<AdminModel, "price_rule">) {
  return String(model.price_rule?.pico_pricing_status || "").trim().toLowerCase() === "published";
}

function formatPicoPrice(model: Pick<AdminModel, "price_rule" | "is_enabled">) {
  const rule = model.price_rule || {};
  const currency = String(rule.currency || "算力");
  const format = (value: unknown) => Number(value || 0).toLocaleString("zh-CN", { maximumFractionDigits: 6 });
  switch (String(rule.billing_type || "").trim().toLowerCase()) {
    case "per_token": {
      const input = rule.input_price_per_m ?? Number(rule.input_price || 0) * 1_000_000;
      const output = rule.output_price_per_m ?? Number(rule.output_price || 0) * 1_000_000;
      return `输入 ${currency} ${format(input)} / 1M · 输出 ${currency} ${format(output)} / 1M`;
    }
    case "per_image":
      return `${currency} ${format(rule.unit_price)} / 张`;
    case "per_second":
      return `${currency} ${format(rule.unit_price)} / 秒`;
    case "dynamic":
      return `动态 · 兜底 ${currency} ${format(rule.fallback_cost)}`;
    default:
      return `${currency} ${format(rule.unit_price)} / 次`;
  }
}

function picoPriceStatusLabel(model: Pick<AdminModel, "price_rule" | "is_enabled" | "new_api_extra_params">) {
  const catalogStatus = String(model.new_api_extra_params?.catalog_status || "").trim().toLowerCase();
  if (catalogStatus && catalogStatus !== "active") return "目录已移除";
  if (!picoPriceReady(model)) return "待设置售价";
...
export default function ModelsPage() {
  ...
  const syncCatalog = async () => {
    ...
    try {
      const result = await adminApi<{ count: number; synced_at?: string }>("/models/sync-catalog", { method: "POST" });
      await load();
      setCatalogSyncMessage(`已同步 ${result.count} 个 Sub2API 模型`);
    } catch (error) {
      setCatalogSyncMessage(error instanceof Error ? error.message : "同步 Sub2API 模型目录失败");
    } finally {
  ...
  const pricingSummary = useMemo(() => {
    const catalog = models.filter(isSub2APICatalogModel);
    const active = catalog.filter((model) => String(model.new_api_extra_params?.catalog_status || "").trim().toLowerCase() === "active");
    const priced = active.filter(picoPriceReady);
    const published = priced.filter((model) => model.is_enabled);
    return { total: active.length, priced: priced.length, published: published.length, pending: active.length - priced.length, unavailable: catalog.length - active.length };
  }, [models]);

  const scrollToPricing = () => {
    document.getElementById("tuna-pricing-list")?.scrollIntoView({ behavior: "smooth", block: "start" });
  };
  ...
  return (
    <div className="flex flex-col">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="px-4 py-2 rounded-xl border border-blue-200 bg-blue-50 text-blue-700 font-semibold text-sm hover:bg-blue-100 disabled:opacity-50"
            title="从已配置的 Sub2API 网关刷新模型目录"
          >
            {catalogSyncing ? "同步中…" : "同步 Sub2API"}
          </button>
      ...
      <section className="mb-5 rounded-2xl border border-gray-200 bg-white p-5 shadow-sm shadow-gray-950/5" aria-labelledby="tuna-pricing-guide-title">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <div className="flex flex-wrap items-center gap-2">
              <h2 id="tuna-pricing-guide-title" className="text-base font-semibold text-gray-950">Tuna 用户售价与发布</h2>
              <span className="rounded-full bg-gray-100 px-2.5 py-1 text-[11px] font-medium text-gray-500">仅管理员可见</span>
            </div>
            <p className="mt-1 max-w-3xl text-sm leading-6 text-gray-500">
              这里设置客户在 Tuna 端看到并实际扣除的价格。上游网关的账号池、渠道成本和负载均衡保持独立；保存售价不会修改上游配置。
            </p>
          </div>
          <button type="button" onClick={scrollToPricing} className="shrink-0 rounded-xl border border-gray-200 bg-white px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50">
            查看模型售价
          </button>
        </div>
        <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
          <div className="rounded-xl border border-gray-100 bg-gray-50/70 px-3 py-3">
            <div className="text-xs text-gray-500">目录模型</div>
            <div className="mt-1 text-xl font-semibold text-gray-950">{pricingSummary.total}</div>
          </div>
          <div className="rounded-xl border border-gray-100 bg-gray-50/70 px-3 py-3">
            <div className="text-xs text-gray-500">已设置售价</div>
            <div className="mt-1 text-xl font-semibold text-gray-950">{pricingSummary.priced}</div>
          </div>
          <div className="rounded-xl border border-emerald-100 bg-emerald-50/60 px-3 py-3">
            <div className="text-xs text-emerald-700">已发布可调用</div>
            <div className="mt-1 text-xl font-semibold text-emerald-800">{pricingSummary.published}</div>
          </div>
          <div className="rounded-xl border border-amber-100 bg-amber-50/60 px-3 py-3">
            <div className="text-xs text-amber-700">待设置售价</div>
            <div className="mt-1 text-xl font-semibold text-amber-800">{pricingSummary.pending}</div>
          </div>
        </div>
        <div className="mt-4 grid gap-2 text-xs leading-5 text-gray-500 sm:grid-cols-3">
          <div className="rounded-xl border border-gray-100 px-3 py-2"><strong className="text-gray-700">1. 刷新目录</strong><br />点击右上角“同步 Sub2API”，读取当前可用模型。</div>
          <div className="rounded-xl border border-gray-100 px-3 py-2"><strong className="text-gray-700">2. 设置售价</strong><br />在对应行点击“Tuna 售价 · 待设置售价”，选择计费方式并填写价格。</div>
          <div className="rounded-xl border border-gray-100 px-3 py-2"><strong className="text-gray-700">3. 发布</strong><br />勾选“发布到前台和 API”并保存；未发布模型不会被客户调用。</div>
        </div>
        <p className="mt-3 text-xs text-gray-400">同步目录、设置售价和发布模型需要超级管理员权限。</p>
        {pricingSummary.unavailable > 0 && <p className="mt-1 text-xs text-gray-400">{pricingSummary.unavailable} 个模型已不在最近一次目录中，系统会自动禁止发布。</p>}
      </section>
      ...
      <div id="tuna-pricing-list" className="bg-white rounded-2xl border overflow-x-auto">
          <table className="w-full text-sm">
          <thead className="bg-gray-50 text-gray-500">
            <tr>
              ...
              <th className="min-w-[190px] text-left px-4 py-3">Tuna 用户售价</th>
            ...
            {paginated.map((m) => (
              ...
              <tr>
                ...
                <td className="px-4 py-3 text-xs">
                  {String(m.new_api_extra_params?.catalog_source || "").toLowerCase() === "sub2api" ? (
                    <span className="inline-flex flex-col gap-0.5">
                      <span className="font-medium text-blue-700">Sub2API</span>
                      <span className="text-[11px] text-gray-400">{m.provider || "自动目录"}</span>
                    </span>
                  ) : (
                ...
                <td className="min-w-[190px] px-4 py-3">
                  {isSub2APICatalogModel(m) ? (
                    <div className="space-y-1.5">
                      <span
                        className={[
                          "inline-flex rounded-full border px-2 py-0.5 text-[11px] font-medium",
                          picoPriceStatusLabel(m) === "已发布"
                            ? "border-emerald-200 bg-emerald-50 text-emerald-700"
                            : picoPriceStatusLabel(m) === "目录已移除"
                              ? "border-gray-200 bg-gray-100 text-gray-500"
                              : "border-amber-200 bg-amber-50 text-amber-700",
                        ].join(" ")}
                      >
                        {picoPriceStatusLabel(m)}
                      </span>
                      <div className="max-w-[260px] text-[11px] leading-4 text-gray-500">{formatPicoPrice(m)}</div>
                      <PicoCatalogPricingEditor
                        model={m as PicoCatalogPricingModel}
                        onSaved={(updated) => {
                          setModels((current) => current.map((item) => (item.id === updated.id ? { ...item, ...updated } : item)));
                        }}
                      />
                    </div>
                  ) : (
                    <span className="text-xs text-gray-400">手动模型请点击“编辑”设置</span>
                  )}
                </td>
              ...
              {showForm && form.id === m.id && (
                <tr>
                  <td ref={setFormMount} colSpan={10} className="p-0 align-top" />
                </tr>
              )}
              </Fragment>
            ))}
