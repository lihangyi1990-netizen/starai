...
const BASE_ITEMS: ConfigItem[] = [
  ...
  { key: "payment_provider", label: "支付渠道", type: "select", options: [
    ...
    { value: "waffo_pancake", label: "Waffo Pancake Checkout" },
  ...
  { key: "payment_product_name", label: "支付商品名称", type: "text", hint: "显示在 Stripe / PayPal 收银台，例如 tuna Credits。" },
  { key: "payment_success_url", label: "支付成功返回地址", type: "text", hint: "仅 Stripe / PayPal 使用。必须是完整 http(s) 地址，可使用 {order_no}，例如 https://example.com/app/wallet?payment=success&order={order_no}" },
  { key: "payment_cancel_url", label: "取消支付返回地址", type: "text", hint: "仅 Stripe / PayPal 使用。必须是完整 http(s) 地址，例如 https://example.com/app/wallet?payment=cancel" },
  ...
  { key: "waffo_pancake_merchant_id", label: "Waffo Pancake Merchant ID", type: "text", hint: "Waffo 控制台中的 Merchant ID，格式通常为 MER_...。" },
  { key: "waffo_pancake_private_key", label: "Waffo Pancake RSA Private Key", type: "textarea", hint: "仅保存于服务端，用于创建 checkout。支持 PEM 文本或单行内容，绝不要填入前端代码或 URL。" },
  { key: "waffo_pancake_store_id", label: "Waffo Pancake Store ID", type: "text", hint: "用于充值商品的 Store ID，格式通常为 STO_...。" },
  { key: "waffo_pancake_product_id", label: "Waffo Pancake Product ID", type: "text", hint: "用于充值的已发布 Onetime Product ID，格式通常为 PROD_...。" },
  { key: "waffo_pancake_return_url", label: "Pancake 支付完成返回地址", type: "text", hint: "可选的完整 http(s) 地址，可使用 {order_no}。到账始终以服务端 webhook 为准。" },
  { key: "waffo_pancake_environment", label: "Pancake Webhook 环境", type: "select", options: [{ value: "prod", label: "Production 正式" }, { value: "test", label: "Test 测试" }], hint: "用于文档和默认配置；建议在 Pancake 控制台分别登记 /api/payment/webhooks/waffo-pancake/test 与 /prod。" },
  { key: "waffo_pancake_webhook_public_key", label: "Pancake Webhook 公钥（共享，可选）", type: "textarea", hint: "SDK 已内置官方 test/prod 公钥；只有 Waffo 提供自定义或轮换公钥时才填写。" },
  { key: "waffo_pancake_webhook_test_public_key", label: "Pancake Test Webhook 公钥（可选）", type: "textarea" },
  { key: "waffo_pancake_webhook_prod_public_key", label: "Pancake Production Webhook 公钥（可选）", type: "textarea" },
  { key: "waffo_pancake_tax_category", label: "Pancake 税务类别", type: "text", hint: "默认 saas；按 Waffo 商户税务配置填写。" },
...
const MEMBER_ITEMS: ConfigItem[] = [
  ...
  { key: "email_otp_login_enabled", label: "启用邮箱验证码登录", type: "checkbox", hint: "关闭后前台只显示账号密码登录/注册。Resend 发信域名验证完成后再开启。" },
];
...
const DYNAMIC_I18N_ITEMS: ConfigItem[] = [
  ...
  { key: "i18n_target_locales", label: "自动生成目标语言（JSON 数组）", type: "textarea", hint: '["en-US","ja-JP","ko-KR","vi-VN","it-IT"]' },
...
const DEFAULT_UI_LANGUAGES: UILanguageRow[] = [
  ...
  { code: "it-IT", short: "IT", name: "Italiano", flag: "🇮🇹", enabled: true, sort_order: 60 },
];
...
export default function SystemConfigPage() {
  ...
  useEffect(() => {
    adminApi<Record<string, unknown>>("/system-configs").then((cfg) => {
      ...
      setConfigs({
        ...
        // Opt-in rather than opt-out: the email OTP path needs a verified sender
        // domain, so an unset value must render as unchecked.
        email_otp_login_enabled: cfg.email_otp_login_enabled === true || cfg.email_otp_login_enabled === 1 || String(cfg.email_otp_login_enabled).toLowerCase() === "true",
      });
  ...
  const visiblePaymentItems = BASE_ITEMS.filter((item) => {
    ...
    if (item.key.startsWith("waffo_pancake_")) return activePaymentProvider === "waffo_pancake";
    if (item.key === "payment_product_name" || item.key === "payment_success_url" || item.key === "payment_cancel_url") {
      return activePaymentProvider === "stripe" || activePaymentProvider === "paypal";
    }
  ...
  return (
    <div className="w-full">
          ...
          <div className="flex max-h-[88vh] w-full max-w-6xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="border-b border-gray-100 p-5">
              ...
              <div className="mt-3 rounded-xl bg-sky-50 px-3 py-2 text-xs leading-5 text-sky-700">
                `zh_label` 只是翻译参考，不参与前台显示；`value` 才是当前 locale 的覆盖值。中文界面默认使用内置中文和后台原文，通常只需要维护 en-US / ja-JP / ko-KR / vi-VN / it-IT 等非中文覆盖。不要把 cn.json 导入到 en-US。
              </div>
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-2">
        ...
        <section id="payment-channel-settings" className="scroll-mt-6 rounded-2xl border border-gray-200 bg-white p-6 shadow-sm shadow-gray-950/5">
          <div className="mb-1 text-sm font-semibold text-gray-900">充值渠道设置</div>
          <p className="mb-4 text-xs leading-relaxed text-gray-400">
            先选择一个支付渠道，再填写该渠道的商户参数并保存；确认回调验证无误后，再打开“在线支付”。充值套餐金额和到账算力仍在“充值套餐”页面维护。真实支付默认关闭。Stripe 回调地址为 /api/payment/webhooks/stripe，PayPal 回调地址为 /api/payment/webhooks/paypal，通用网关回调地址为 /api/payment/webhooks/generic；Waffo Pancake 可登记 /api/payment/webhooks/waffo-pancake/test 或 /api/payment/webhooks/waffo-pancake/prod。
          </p>
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
