...
const EMPTY = { email: "", password: "", role: "operator", status: "active", linked_user_id: "" };
...
export default function AdminAccountsPage() {
  ...
  const startEdit = (item: AdminAccount) => {
    ...
    setForm({
      ...
      linked_user_id: item.linked_user_id ? String(item.linked_user_id) : "",
    });
  };
  ...
  const save = async () => {
    ...
    try {
      const body = {
        ...
        role: form.role,
        status: form.status,
        linked_user_id: form.linked_user_id.trim() ? Number(form.linked_user_id) : 0,
      };
      if (editing) {
  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-gray-500">管理后台登录账号、角色和账号状态。需要从 Tuna 设置页快捷进入时，可填写对应的 Tuna 用户 ID；直接使用管理员账号登录无需绑定。</p>
      </div>

      <div className="mb-5 grid gap-3 rounded-2xl border bg-white p-4 md:grid-cols-7">
        ...
        <input
          ...
          onChange={(e) => setForm((p) => ({ ...p, linked_user_id: e.target.value.replace(/\D/g, "") }))}
          placeholder="绑定 Tuna 用户 ID（可选）"
          inputMode="numeric"
        ...
        <table className="w-full text-sm">
          <thead className="bg-gray-50 text-gray-500">
            <tr>
              ...
              <th className="px-4 py-3 text-left">Tuna 绑定</th>
          ...
          <tbody className="divide-y">
            {items.map((item) => (
              <tr key={item.id}>
                ...
                <td className="px-4 py-3 text-xs text-gray-500">{item.linked_user_id || "未绑定"}</td>
