...
export interface WalletUsageItem {
  operation_type: 'chat' | 'image' | 'video' | 'audio' | 'workflow' | string;
  model_code: string;
  model_name: string;
  count: number;
  unit_cost: number;
...
export interface Model {
  ...
  display_name: string;
  category: string;
  /** Provider reported by the synced gateway catalog, when available. */
  provider?: string;
  icon_url?: string;
  description?: string;
  tags: string[];
...
export interface SystemConfig {
  ...
  payment_provider?: "disabled" | "mock" | "generic" | "stripe" | "paypal" | "waffo_pancake";
...
export const UI_TRANSLATION_ZH_LABELS: Record<string, string> = {
  ...
  "asset.mentionEmpty": "No matching assets",
  "asset.mentionHint": "Pick an asset to insert a reference",
...
export interface ApiToken {
  ...
  name: string;
  prefix: string;
  protocol?: 'universal' | 'openai' | 'anthropic' | 'gemini';
  product_code?: string;
  model_scopes?: string[];
  status: string;
  last_used_at?: string;
