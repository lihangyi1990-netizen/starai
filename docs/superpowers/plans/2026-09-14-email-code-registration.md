...
- **验证码防爆破**：同一验证码连续输错 5 次即作废，需重新获取（「验证码错误次数过多，请重新获取验证码」）；冷却键同步场景化为 `email_otp_cooldown:register:`
- **验证码消费时机**：校验通过只比对不删除；建号事务提交成功后才消费验证码（连失败计数一起清），DB 瞬时失败不会吞掉用户已收到的码；并发注册抢注同一邮箱时返回 409 而不是 500
- **i18n**：新增 4 个注册文案 key（6 语言）；清理 15 个失去引用的旧 key；shared-types 翻译键表与后端 `ui_translation_catalog.json` 同步登记新键、剔除死键
- **测试**：新增 `email_otp_test.go`，覆盖验码全分支（错码不消费、5 次错码作废、成功不预消费）、消费时机与发码守卫优先级（DB 错误 > 已注册 > 冷却）；`go test ./...`、前端 `tsc` 全绿
