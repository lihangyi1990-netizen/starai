...
| 接口 | 变化 |
|---|---|
| `POST /api/auth/email/send-code` | 保留，语义收窄为「仅注册发码」。请求体不变（`email`、`captcha_id`、`captcha_code`）。已注册邮箱返回 400「该邮箱已注册，请直接登录」。后台开启图形验证码时，发码与注册/密码登录走同一道图形验证码闸门（弹窗早已采集并上送这两个字段） |
...
- `VerifyRegistrationCode` 只比对、**不删除**；错码/过期返回 `ErrInvalidEmailCode`，错码不消费（TTL 内允许重试）。
- 验证码在 `Register` 建号事务 **提交成功后** 由 `ConsumeRegistrationCode` 删除（一次性）。这样验码与建号之间的 DB 瞬时失败不会烧掉用户已收到的码——可凭原码重试注册；若提交后删码失败，残留码最迟随 TTL 失效，重放注册会被邮箱查重挡下，无害。
...
- 获取验证码前要求邮箱已填、协议已勾选；后台开启图形验证码时还要求先填图形验证码（与注册/登录按钮一致）。后端拒发（如「该邮箱已注册，请直接登录」）时错误直接显示在弹窗内。
...
3. 已注册邮箱点「获取验证码」被拒，提示「该邮箱已注册，请直接登录」，且不发信、不写验证码缓存；开启图形验证码时，不填/错填图形验证码同样被拒。
4. 验证码在建号成功后一次性失效（错码不消费）；60 秒冷却与 IP 限流生效。
