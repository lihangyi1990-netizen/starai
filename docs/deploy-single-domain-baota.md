...
# When using the local Sub2API gateway, set this to true. PICO then refreshes
# the visible model directory from the gateway's /v1/models response. New rows
# are disabled until an administrator sets and publishes the PICO customer
# price in 后台 → 模型管理 → PICO 售价.
NEW_API_MODEL_CATALOG_SYNC=false
