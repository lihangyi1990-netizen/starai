...
func main() {
	...
	r := gin.Default()
	// Keep the local image-task path self-contained: the worker can fetch this
	// valid PNG from the mock container and persist it to MinIO without relying
	// on an external placeholder-image service.
	r.GET("/mock-image.png", func(c *gin.Context) {
	...
	r.GET("/mock-video.mp4", func(c *gin.Context) {
		// A tiny MP4-shaped fixture is sufficient for the local worker pipeline.
		// Real deployments receive an actual media URL from the configured provider.
		c.Data(http.StatusOK, "video/mp4", []byte{
	...
	r.POST("/v1/images/generations", func(c *gin.Context) {
		...
		for i := 0; i < n; i++ {
			...
			url := fmt.Sprintf("http://mock-new-api:3002/mock-image.png?seed=%d", seed)
	...
	handleVideoPoll := func(c *gin.Context) {
		...
		c.JSON(200, gin.H{
			...
			"status":       "completed",
			"progress":     100,
			"video_url":    "http://mock-new-api:3002/mock-video.mp4",
			"completed_at": time.Now().Unix(),
			"data": []gin.H{{
				"url":       "http://mock-new-api:3002/mock-video.mp4",
				"thumbnail": fmt.Sprintf("http://mock-new-api:3002/mock-image.png?seed=%d", seed),
			}},
		})
	}
	r.GET("/v1/video/generations/:id", handleVideoPoll)
	r.GET("/v1/videos/:id", handleVideoPoll)
	r.GET("/v1/video/generations/:id/content", func(c *gin.Context) {
		c.Redirect(http.StatusFound, "/mock-video.mp4")
	})
	r.GET("/v1/videos/:id/content", func(c *gin.Context) {
		c.Redirect(http.StatusFound, "/mock-video.mp4")
	})
