...
type Config struct {
	...
	NewAPIModelCatalogSync bool
	// Sub2APIAdminURL is the browser-reachable administration console URL.
	// It is deliberately separate from NewAPIBaseURL, which is normally an
	// internal Docker address that a user's browser cannot resolve.
	Sub2APIAdminURL string
}
...
func Load() *Config {
	return &Config{
		...
		NewAPIModelCatalogSync: getEnv("NEW_API_MODEL_CATALOG_SYNC", "false") == "true",
		Sub2APIAdminURL:        getEnv("SUB2API_ADMIN_URL", ""),
	}
}
