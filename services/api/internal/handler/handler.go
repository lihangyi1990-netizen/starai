...
type Handler struct {
	...
	gatewayCatalogMu       sync.Mutex
	gatewayCatalogAt       time.Time
	gatewayCatalogFailedAt time.Time
	gatewayCatalogErr      error
}
...
func (h *Handler) RegisterRoutes(r *gin.Engine) {
	...
	v1.Use(middleware.RateLimit(h.cache, "openapi", 120, time.Minute, middleware.UserIdentity))
	{
		...
		// Keep both spellings available.  The legacy PICO documentation uses
		// /video/generations, while OpenAI/Sora-compatible clients and the
		// Sub2API-synced catalog use the standard plural /videos route.
		v1.POST("/videos", h.OpenAPIVideoGeneration)
		v1.POST("/audio/speech", h.OpenAPIAudioSpeech)
		v1.GET("/tasks/:task_no", h.OpenAPIGetTask)
		v1.GET("/videos/:task_no", h.OpenAPIGetTask)
	...
	api := r.Group("/api")
	{
		// 100/minute per IP, raised from 5/hour at the operator's request. That is
		// permissive enough that the image captcha is the only real abuse control
		// left on this route, so keep image_captcha_enabled on in production.
		...
		// Pancake supports separate test and production webhook registrations;
		// the optional environment path lets us enforce that boundary.
		api.POST("/payment/webhooks/waffo-pancake", middleware.RateLimit(h.cache, "waffo-pancake-webhook", 240, time.Minute, middleware.ClientIPIdentity), h.WaffoPancakeWebhook)
		api.POST("/payment/webhooks/waffo-pancake/:env", middleware.RateLimit(h.cache, "waffo-pancake-webhook", 240, time.Minute, middleware.ClientIPIdentity), h.WaffoPancakeWebhook)
		...
		auth := api.Group("")
		auth.Use(middleware.UserAuth(h.cfg.JWTSecret, h.cache, h.auth))
		auth.Use(middleware.RateLimit(h.cache, "user-api", 300, time.Minute, middleware.UserIdentity))
		{
			auth.GET("/me", h.GetMe)
			auth.GET("/admin/handoff/status", h.AdminHandoffStatus)
			auth.POST("/admin/handoff", h.CreateAdminHandoff)
			...
			auth.GET("/api-token-products", h.ListApiTokenProducts)
	...
	admin := r.Group("/admin/api")
	{
		...
		admin.POST("/handoff/exchange", middleware.RateLimit(h.cache, "admin-handoff", 12, 10*time.Minute, middleware.ClientIPIdentity), h.ExchangeAdminHandoff)
		adm := admin.Group("")
		...
		adm.Use(middleware.RateLimit(h.cache, "admin-api", 300, time.Minute, middleware.ClientIPIdentity))
		{
			...
			// The Sub2API console is a separate privileged application.  Only a
			// super administrator may receive its operator-configured URL; no
			// gateway token or provider credential is ever returned here.
			adm.GET("/sub2api/config", superAdminOnly, h.AdminSub2APIConfig)
			...
			adm.POST("/models/sync-catalog", superAdminOnly, h.AdminSyncModelCatalog)
			...
			adm.PATCH("/models/:id/pico-pricing", superAdminOnly, h.AdminUpdatePicoCatalogPricing)
...
func (h *Handler) Register(c *gin.Context) {
	...
	// Signup is a more attractive abuse target than login, so it gets the same
	// captcha gate the password login already had.
	...
	if err == service.ErrWeakPassword {
		util.BadRequest(c, "密码长度需为 6-72 个字符")
		return
	}
	if err != nil {
...
func (h *Handler) LoginPassword(c *gin.Context) {
	...
	if err == service.ErrInvalidCredentials {
		// The administrator table intentionally has a separate credential
		// boundary from normal users.  Try it only after a normal-user miss so
		// ordinary logins retain their existing path and response shape.
		identity, adminErr := h.auth.AuthenticateAdmin(c.Request.Context(), req.Email, req.Password)
		if adminErr == nil {
			code, handoffErr := h.createAdminHandoff(c, identity)
			if handoffErr != nil {
				util.Fail(c, http.StatusServiceUnavailable, http.StatusServiceUnavailable, "后台入口服务暂不可用，请稍后重试")
				return
			}
			// Do not return an admin JWT here.  The admin app exchanges this
			// one-time code and receives the HttpOnly admin session cookie.
			util.OK(c, map[string]interface{}{
				"admin":              true,
				"admin_handoff_code": code,
				"expires_in":         int(adminHandoffTTL / time.Second),
			})
			return
		}
		if adminErr != service.ErrInvalidCredentials {
			util.InternalError(c, "登录暂时失败，请稍后重试")
			return
		}
...
// configTruthy interprets a system_configs value as a boolean that defaults to
// false. Values arrive as JSON, so a flag saved by the admin UI is a real bool,
// but one edited directly in the database may be a string or a number.
func configTruthy(value interface{}) bool {
	switch v := value.(type) {
	case nil:
		return false
	case bool:
...
func (h *Handler) setSessionCookie(c *gin.Context, name, token string, ttl time.Duration) {
	...
	c.SetCookie(name, token, int(ttl.Seconds()), "/", "", h.requestUsesHTTPS(c), true)
}

func (h *Handler) clearSessionCookie(c *gin.Context, name string) {
	c.SetSameSite(http.SameSiteLaxMode)
	c.SetCookie(name, "", -1, "/", "", h.requestUsesHTTPS(c), true)
}

func (h *Handler) requestUsesHTTPS(c *gin.Context) bool {
	...
	return remoteAddressTrusted(c.Request.RemoteAddr, h.cfg.TrustedProxies)
}

func remoteAddressTrusted(remoteAddress, trustedProxyList string) bool {
	...
	for _, raw := range strings.Split(trustedProxyList, ",") {
		...
		if trustedIP := net.ParseIP(strings.Trim(candidate, "[]")); trustedIP != nil {
			if trustedIP.Equal(remoteIP) {
				return true
			}
			continue
		}
		if _, network, err := net.ParseCIDR(candidate); err == nil && network.Contains(remoteIP) {
			return true
		}
	}
	return false
...
func (h *Handler) ApiTokenAuth() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		identity, err := h.ops.AuthenticateApiToken(c.Request.Context(), token)
		if err != nil {
		...
		if !apiTokenProtocolAllowsRequest(identity.Protocol, c.Request.URL.Path) {
			openAPIError(c, http.StatusForbidden, "permission_error", "此 API Key 不适用于当前调用协议，请在 API 管理中创建对应协议的 Key。")
			c.Abort()
			return
		}
		c.Set("user_id", identity.UserID)
		c.Set("api_token_identity", identity)
		c.Next()
	}
}

func apiTokenProtocolAllowsRequest(protocol, path string) bool {
	switch protocol {
	case service.ApiTokenProtocolUniversal:
		return true
	case service.ApiTokenProtocolOpenAI:
		return strings.HasPrefix(path, "/v1/") && path != "/v1/messages"
	case service.ApiTokenProtocolAnthropic:
		return path == "/v1/models" || path == "/v1/messages"
	case service.ApiTokenProtocolGemini:
		return path == "/v1/models" || strings.HasPrefix(path, "/v1beta/")
	default:
		return false
	}
}
...
const adminHandoffTTL = 60 * time.Second

func adminHandoffCacheKey(code string) string {
	return "pico:admin-handoff:" + strings.TrimSpace(code)
}

func newAdminHandoffCode() (string, error) {
	buf := make([]byte, 32)
	if _, err := rand.Read(buf); err != nil {
		return "", err
	}
	return "ah_" + base64.RawURLEncoding.EncodeToString(buf), nil
}

// safeSub2APIAdminURL accepts only an operator-configured browser URL.  The
// gateway URL used by the API is often an internal Docker hostname and must
// never be copied into a link blindly.  Rejecting userinfo, query strings and
// fragments also prevents credentials or redirect state from being smuggled
// into the admin link.
func safeSub2APIAdminURL(raw string) string {
...
// AdminSub2APIConfig returns the browser-facing Sub2API console URL only to
// super administrators.  The response intentionally contains no credential;
// Sub2API keeps its own login/session boundary.
func (h *Handler) AdminSub2APIConfig(c *gin.Context) {
	c.Header("Cache-Control", "private, no-store")
	configuredURL := ""
	if h != nil && h.cfg != nil {
		configuredURL = h.cfg.Sub2APIAdminURL
	}
	consoleURL := safeSub2APIAdminURL(configuredURL)
	util.OK(c, map[string]interface{}{
		"available": consoleURL != "",
		"url":       consoleURL,
	})
}

// AdminHandoffStatus is intentionally non-sensitive: ordinary users receive
// {available:false}, while only an email-linked active administrator sees the
// account metadata needed to render the backend entry in Settings.
func (h *Handler) AdminHandoffStatus(c *gin.Context) {
	c.Header("Cache-Control", "private, no-store")
	identity, err := h.auth.AdminForUser(c.Request.Context(), c.GetInt64("user_id"))
	if errors.Is(err, service.ErrAdminNotLinked) {
		util.OK(c, map[string]interface{}{"available": false})
		return
	}
	if err != nil {
		util.InternalError(c, "管理员状态暂时无法读取")
		return
	}
	result := map[string]interface{}{"available": true, "email": identity.Email, "role": identity.Role}
	// Keep the upstream-console URL out of ordinary user responses.  A linked
	// operator can still enter tuna's own admin panel, while the separate
	// Sub2API console remains a super-admin-only control surface.
	if identity.Role == "super_admin" {
		if consoleURL := safeSub2APIAdminURL(h.cfg.Sub2APIAdminURL); consoleURL != "" {
			result["sub2api_url"] = consoleURL
		}
	}
	util.OK(c, result)
}

// CreateAdminHandoff creates a one-time, 60-second exchange code. The code is
// stored only in Redis and maps to an administrator id; no admin credential is
// returned to or persisted by the PICO user session.
func (h *Handler) CreateAdminHandoff(c *gin.Context) {
	identity, err := h.auth.AdminForUser(c.Request.Context(), c.GetInt64("user_id"))
	if errors.Is(err, service.ErrAdminNotLinked) {
		util.Forbidden(c, "当前账号没有管理员后台权限")
		return
	}
	if err != nil {
		util.InternalError(c, "管理员状态暂时无法读取")
	...
	code, err := h.createAdminHandoff(c, identity)
	if err != nil {
		util.Fail(c, http.StatusServiceUnavailable, 503, "后台入口服务暂不可用，请稍后重试")
		return
	}
	util.OK(c, map[string]interface{}{
		"code":        code,
...
// createAdminHandoff stores an administrator id behind a short-lived,
// single-use code.  It is shared by the settings-page entry and the public
// login redirect so both paths have the same expiry and cache checks.
func (h *Handler) createAdminHandoff(c *gin.Context, identity *service.AdminIdentity) (string, error) {
	if h == nil || h.cache == nil || identity == nil || identity.ID <= 0 {
		return "", errors.New("admin handoff cache unavailable")
	}
	code, err := newAdminHandoffCode()
	if err != nil {
		return "", err
	}
	key := adminHandoffCacheKey(code)
	adminID := strconv.FormatInt(identity.ID, 10)
	if err := h.cache.SetTemp(c.Request.Context(), key, adminID, adminHandoffTTL); err != nil {
		return "", err
	}
	// SetTemp is deliberately a no-op when a cache client is unavailable. Read
	// it back before returning so the UI never receives an unusable handoff.
	if stored, ok := h.cache.GetTemp(c.Request.Context(), key); !ok || stored != adminID {
		return "", errors.New("admin handoff cache write could not be verified")
	}
	return code, nil
}
...
func (h *Handler) ListModels(c *gin.Context) {
	...
	if err := h.syncGatewayModelCatalog(c.Request.Context()); err != nil {
		// A previously materialized catalog remains safe to display while the
		// gateway is temporarily unavailable. Keep the response usable and let
		// operators/clients observe the stale marker without exposing internals.
		c.Header("X-Pico-Model-Catalog", "stale")
	}
	// The website may show active catalog entries that are awaiting a retail
	// price. They remain disabled and cannot be selected or invoked.
	models, err := h.displayModels(c.Request.Context(), category)
	if err != nil {
		util.InternalError(c, err.Error())
		return
	}
...
func (h *Handler) OpenAPIListModels(c *gin.Context) {
	...
	models, err := h.publicModels(c.Request.Context(), c.Query("category"))
	if err != nil {
		openAPIError(c, http.StatusInternalServerError, "server_error", err.Error())
		return
	}
	identity, _ := c.Get("api_token_identity")
	tokenIdentity, _ := identity.(service.ApiTokenIdentity)
	data := make([]map[string]interface{}, 0, len(models))
	for _, model := range models {
		if !h.apiTokenIdentityAllowsModel(c, tokenIdentity, model.Code) {
			continue
		}
...
func (h *Handler) GetModel(c *gin.Context) {
	...
	// Keep this endpoint aligned with the model plaza. A synchronized model that
	// awaits a tuna retail price is displayable but still rejected by every
	// generation and OpenAI-compatible invocation path.
	m, err := h.models.GetByCodeForDisplay(c.Request.Context(), c.Param("code"))
	if err != nil {
...
func (h *Handler) publicModels(ctx context.Context, category string) ([]service.ModelDTO, error) {
	if h.cfg.NewAPIModelCatalogSync {
		return h.models.ListGatewayCatalog(ctx, category)
	}
	return h.models.ListPublic(ctx, category)
}

// displayModels is deliberately separate from publicModels. The former feeds
// PICO's visual model directory; the latter feeds /v1/models and must only
// expose models that have been enabled for customer API use.
func (h *Handler) displayModels(ctx context.Context, category string) ([]service.ModelDTO, error) {
	if h.cfg.NewAPIModelCatalogSync {
		return h.models.ListGatewayCatalogForDisplay(ctx, category)
	}
	return h.models.ListPublicForDisplay(ctx, category)
}

const requestCatalogSyncBudget = 3 * time.Second

// refreshGatewayCatalogForRequest gives direct model invocations a bounded
// chance to materialize a newly added Sub2API model. A stale/failed sync never
// blocks the request from using the last cached catalog.
func (h *Handler) refreshGatewayCatalogForRequest(c *gin.Context) {
	if h == nil || h.cfg == nil || c == nil || !h.cfg.NewAPIModelCatalogSync {
		return
	}
	syncCtx, cancel := context.WithTimeout(c.Request.Context(), requestCatalogSyncBudget)
	defer cancel()
	if err := h.syncGatewayModelCatalog(syncCtx); err != nil {
		c.Header("X-Pico-Model-Catalog", "stale")
	}
}

// syncGatewayModelCatalog makes Sub2API the source of truth for the website's
// model selector. It is deliberately opt-in so development/demo deployments
// continue to use the seeded catalog. A short cache interval prevents every
// model-picker render from performing a gateway request.
func (h *Handler) syncGatewayModelCatalog(ctx context.Context) error {
	return h.syncGatewayModelCatalogWithOptions(ctx, false)
}

func (h *Handler) syncGatewayModelCatalogWithOptions(ctx context.Context, force bool) error {
	if h == nil || h.cfg == nil || !h.cfg.NewAPIModelCatalogSync {
		return nil
	}
	// Do not queue an invocation behind a long-running directory refresh. The
	// caller can safely continue with the last materialized catalog.
	if !h.gatewayCatalogMu.TryLock() {
		return errors.New("model catalog sync already in progress")
	}
	defer h.gatewayCatalogMu.Unlock()
	if h.runtime == nil || h.models == nil {
		return errors.New("model catalog sync is not configured")
	}
	if !force && !h.gatewayCatalogAt.IsZero() && time.Since(h.gatewayCatalogAt) < 30*time.Second {
		return nil
	}
	if !force && !h.gatewayCatalogFailedAt.IsZero() && time.Since(h.gatewayCatalogFailedAt) < 10*time.Second {
		return h.gatewayCatalogErr
	}

	syncCtx, cancel := context.WithTimeout(ctx, 15*time.Second)
	defer cancel()
	upstream, err := h.runtime.ListModels(syncCtx, nil)
	if err != nil {
		h.gatewayCatalogFailedAt = time.Now()
		h.gatewayCatalogErr = err
		log.Printf("sub2api model catalog sync failed; serving cached catalog: %v", err)
		return err
	}
	if err := h.models.SyncGatewayCatalog(syncCtx, gatewayCatalogEntries(upstream)); err != nil {
		h.gatewayCatalogFailedAt = time.Now()
		h.gatewayCatalogErr = err
		log.Printf("sub2api model catalog materialization failed; serving cached catalog: %v", err)
		return err
	}
	h.gatewayCatalogAt = time.Now()
	h.gatewayCatalogFailedAt = time.Time{}
	h.gatewayCatalogErr = nil
	return nil
}

func gatewayCatalogEntries(upstream []runtime.UpstreamModel) []service.GatewayCatalogModel {
	entries := make([]service.GatewayCatalogModel, 0, len(upstream))
	seen := make(map[string]bool, len(upstream))
	for _, item := range upstream {
		code := strings.TrimSpace(item.ID)
		if code == "" || len(code) > 64 || seen[code] {
			continue
		}
		seen[code] = true
		category, endpoint, mode := gatewayModelTransport(code)
		// `owned_by` is optional in OpenAI-compatible responses. Keep model-id
		// inference for display, but mark it separately so a sync cannot replace
		// an administrator's custom provider. A real owner hint is the only value
		// allowed to update an existing provider assignment.
		hint := strings.TrimSpace(item.OwnedBy)
		provider := service.CatalogProviderWithHint(code, hint)
		providerSource := "inferred"
		if hint != "" && !service.CatalogProviderHintIsGeneric(hint) {
			providerSource = "catalog"
		}
		if strings.EqualFold(provider, "other") {
			provider = ""
			providerSource = ""
		}
		entries = append(entries, service.GatewayCatalogModel{
			Code: code, DisplayName: code, Provider: provider, ProviderSource: providerSource, Category: category, Endpoint: endpoint, RequestMode: mode,
			AudioCapable: mode == "audio" || gatewayModelAudioCapable(code), SortOrder: len(entries) + 1,
		})
	}
	return entries
}

// gatewayModelAudioCapable identifies model IDs that can return standalone
// audio while retaining a different primary catalog category. Video models
// whose names contain both terms describe an embedded soundtrack instead.
func gatewayModelAudioCapable(code string) bool {
	model := strings.ToLower(strings.TrimSpace(code))
	if model == "" || (strings.Contains(model, "video") && strings.Contains(model, "audio")) {
		return false
	}
	return containsAny(model, "audio-preview", "realtime", "audio", "tts", "speech", "voice", "music", "suno", "whisper", "transcri", "eleven", "fish-speech", "cosyvoice")
}

func gatewayModelTransport(code string) (category, endpoint, requestMode string) {
	model := strings.ToLower(strings.TrimSpace(code))
	switch {
	case model == "sd_2.0_mini_special":
		// The upstream catalog reports generic model metadata. This exact model
		// is configured with a video-generation route in Tuna, so it must not
		// fall through to the chat default during catalog refreshes.
		return "video", "/v1/videos", "video"
	case strings.Contains(model, "audio-preview") || strings.Contains(model, "realtime"):
		// OpenAI audio-preview/realtime models are multimodal chat models. They
		// accept chat-completions payloads and must not be routed to TTS.
		return "chat", "/v1/chat/completions", "chat_completions"
	default:
		// Keep explicit media hints ahead of brand shortcuts.  In particular,
		// Hailuo has both video models and `hailuo-speech-*` TTS models; treating
		// every Hailuo model as video makes the latter impossible to call.
		audioHint := containsAny(model, "tts", "speech", "voice", "audio", "music", "suno", "whisper", "transcri", "eleven", "fish-speech", "cosyvoice")
		videoHint := containsAny(model, "video", "seedance", "kling", "runway", "pika", "veo", "sora", "hunyuan-video", "wan2", "wan-", "pixverse", "luma", "vidu", "cogvideo")
		if strings.Contains(model, "hailuo") && !audioHint {
			videoHint = true
		}
		// A model explicitly named for video wins when a provider appends an
		// audio/voice capability suffix (for example, video-with-audio).
		if videoHint {
			return "video", "/v1/videos", "video"
		}
		if audioHint {
			return "audio", "/v1/audio/speech", "audio"
		}
		if containsAny(model, "image", "dall-e", "flux", "midjourney", "stable-diffusion", "sdxl", "ideogram", "recraft", "qwen-image", "banana") {
			return "image", "/v1/images/generations", "images"
		}
		return "chat", "/v1/chat/completions", "chat_completions"
	}
}
...
func (h *Handler) ListCategories(c *gin.Context) {
	...
	if h.cfg.NewAPIModelCatalogSync {
		if syncErr := h.syncGatewayModelCatalog(c.Request.Context()); syncErr != nil {
			c.Header("X-Pico-Model-Catalog", "stale")
		}
		cats, err = h.models.ListGatewayCatalogCategories(c.Request.Context())
	} else {
...
func (h *Handler) GetWalletUsage(c *gin.Context) {
	usage, err := h.wallet.GetUsageSummary(c.Request.Context(), c.GetInt64("user_id"))
	if err != nil {
		util.InternalError(c, err.Error())
		return
	}
...
func (h *Handler) CreatePaymentOrder(c *gin.Context) {
	...
	if allowMockPayment && (req.Channel == "" || req.Channel == "mock") {
		...
		if !supportedPaymentChannel(req.Channel) {
...
// supportedPaymentChannel validates the channel hint accepted by the public
// order endpoint. The selected provider is still read from server-side
// configuration by CreatePendingPackageOrder; this allow-list only prevents
// malformed client hints from reaching the payment flow. Keep Waffo Pancake
// here because the web client sends the configured provider as channel.
func supportedPaymentChannel(channel string) bool {
	switch strings.ToLower(strings.TrimSpace(channel)) {
	case "", "generic", "stripe", "paypal", "waffo_pancake":
		return true
	default:
		return false
	}
...
func (h *Handler) WaffoPancakeWebhook(c *gin.Context) {
	...
	result, handled, err := h.payment.CompleteWaffoPancakeWebhook(
		c.Request.Context(), raw, c.GetHeader("X-Waffo-Signature"), c.Param("env"),
	)
	if err != nil {
		middleware.RecordPaymentWebhookRejected()
		util.BadRequest(c, err.Error())
...
func (h *Handler) ListConversations(c *gin.Context) {
	page, pageSize, err := parseConversationPagination(c)
	if err != nil {
		util.BadRequest(c, err.Error())
		return
	}
	items, err := h.chat.ListConversations(c.Request.Context(), c.GetInt64("user_id"), c.Query("model_code"), page, pageSize)
	if err != nil {
		util.InternalError(c, err.Error())
		return
	}
...
const (
	conversationHandlerDefaultPage     = 1
	conversationHandlerDefaultPageSize = 50
	conversationHandlerMaxPage         = 100000
	conversationHandlerMaxPageSize     = 100
)

// parseConversationPagination rejects malformed or unreasonably large query
// values before they reach the database. Missing parameters retain the
// pre-pagination defaults, so existing clients continue to receive an array.
func parseConversationPagination(c *gin.Context) (int, int, error) {
	page := conversationHandlerDefaultPage
	pageSize := conversationHandlerDefaultPageSize

	if raw, ok := c.GetQuery("page"); ok {
		value, err := strconv.Atoi(raw)
		if err != nil || value < 1 || value > conversationHandlerMaxPage {
			return 0, 0, fmt.Errorf("page 必须是 1-%d 之间的整数", conversationHandlerMaxPage)
		}
		page = value
	}
	if raw, ok := c.GetQuery("page_size"); ok {
		value, err := strconv.Atoi(raw)
		if err != nil || value < 1 || value > conversationHandlerMaxPageSize {
			return 0, 0, fmt.Errorf("page_size 必须是 1-%d 之间的整数", conversationHandlerMaxPageSize)
		}
		pageSize = value
	}
...
func (h *Handler) ChatCompletion(c *gin.Context) {
	...
	model, err := h.chat.ResolveInputModel(c.Request.Context(), &input)
	if err != nil {
		openAPIError(c, http.StatusBadRequest, "model_not_found", "模型不存在或未启用，请检查 model 是否为后台模型编码或接入模型名")
		return
	}
	if !h.apiTokenAllowsModel(c, model.Code) {
		return
	}
...
func (h *Handler) AnthropicMessages(c *gin.Context) {
	...
	model, err := h.chat.ResolveInputModel(c.Request.Context(), &input)
	if err != nil {
		openAPIError(c, http.StatusBadRequest, "model_not_found", "模型不存在或未启用，请检查 model 配置")
		return
	}
	if !h.apiTokenAllowsModel(c, model.Code) {
		return
...
func (h *Handler) chatStreamSingle(c *gin.Context, userID int64, input service.CompletionInput, model *service.ModelFull) {
	...
	convID, finalizeErr := h.chat.FinalizeStream(context.Background(), userID, requestID, input, fullContent, fullReasoningContent, usage, estimated)
	if finalizeErr != nil {
		openAIStreamError(c, "费用结算失败，请联系客服核对账单")
		flusher.Flush()
		return
	}
	writeOpenAIStreamChunkWithConversation(c, requestID, input.ModelCode, map[string]interface{}{}, "stop", usage, convID)
...
func writeOpenAIStreamChunk(c *gin.Context, requestID, model string, delta map[string]interface{}, finishReason string, usage *runtime.ChatUsage) {
	writeOpenAIStreamChunkWithConversation(c, requestID, model, delta, finishReason, usage, "")
}

func writeOpenAIStreamChunkWithConversation(c *gin.Context, requestID, model string, delta map[string]interface{}, finishReason string, usage *runtime.ChatUsage, conversationID string) {
	payload := buildOpenAIStreamPayloadWithConversation(requestID, model, delta, finishReason, usage, conversationID)
	data, _ := json.Marshal(payload)
	_, _ = c.Writer.Write([]byte("data: " + string(data) + "\n\n"))
}

func buildOpenAIStreamPayloadWithConversation(requestID, model string, delta map[string]interface{}, finishReason string, usage *runtime.ChatUsage, conversationID string) map[string]interface{} {
...
func (h *Handler) chatMultiStream(c *gin.Context, userID int64, input service.CompletionInput, channelKey string) {
	...
	// A channel preset can fan one request out to several underlying models.
	// The top-level collaboration model scope alone is not sufficient for an
	// API token: otherwise a vendor-scoped key could smuggle requests through a
	// preset containing models from another provider. Check every model that
	// this stream may invoke before freezing any balance or writing output.
	if !h.apiTokenAllowsCollabModels(c, append(append([]string{}, modelCodes...), summaryModelCodes...)) {
		return
	}
	...
	if len(input.Messages) > 0 {
		convID = h.chat.SaveMultiMessages(context.Background(), convID, userID, input.ModelCode, input.Messages, results, summary)
	}
...
// apiTokenAllowsCollabModels is a no-op for normal signed-in web requests.
// For /v1 requests it validates each resolved constituent model against the
// same API-token product scope used by single-model calls. Missing models are
// skipped here because the existing preset logic may intentionally fall back
// around a stale/disabled entry; a custom model override has already been
// validated by validateCustomCollabModels.
func (h *Handler) apiTokenAllowsCollabModels(c *gin.Context, modelCodes []string) bool {
	if c == nil {
		return false
	}
	if _, exists := c.Get("api_token_identity"); !exists {
		return true
	}
	if h.models == nil {
		if c != nil {
			openAPIError(c, http.StatusInternalServerError, "authentication_error", "API Key 身份校验暂不可用，请稍后重试。")
		}
		return false
	}
	for _, code := range uniqueModelCodes(modelCodes) {
		model, err := h.models.GetFullByCode(c.Request.Context(), code)
...
// ExchangeAdminHandoff consumes the code atomically and establishes the same
// HttpOnly admin session used by the regular admin login form.
func (h *Handler) ExchangeAdminHandoff(c *gin.Context) {
	var req struct {
		Code string `json:"code"`
	}
	if err := c.ShouldBindJSON(&req); err != nil {
		util.BadRequest(c, "后台入口代码无效")
		return
	}
	code := strings.TrimSpace(req.Code)
	if len(code) < 20 || len(code) > 128 || h.cache == nil {
		util.Unauthorized(c, "后台入口代码无效或已过期")
		return
	}
	adminIDRaw, ok := h.cache.GetDelTemp(c.Request.Context(), adminHandoffCacheKey(code))
	if !ok {
		util.Unauthorized(c, "后台入口代码无效或已过期")
		return
	}
	...
	result, err := h.auth.IssueAdminTokenForID(c.Request.Context(), adminID, h.cfg.AdminJWT)
	if errors.Is(err, service.ErrAdminNotLinked) {
		util.Unauthorized(c, "管理员账号已停用，请重新登录")
		return
	}
	if err != nil {
		util.InternalError(c, "后台登录暂时失败")
		return
	}
	h.setSessionCookie(c, "starai_admin_session", result.Token, 24*time.Hour)
	util.OK(c, result)
...
// AdminSyncModelCatalog explicitly refreshes the Sub2API-backed model catalog.
// The public model endpoints also refresh opportunistically, but an explicit
// action is useful after an operator adds or removes an upstream account and
// wants to see the change immediately in the admin console.
func (h *Handler) AdminSyncModelCatalog(c *gin.Context) {
	if h == nil || h.cfg == nil || !h.cfg.NewAPIModelCatalogSync {
		util.BadRequest(c, "Sub2API 模型目录同步未开启，请配置 NEW_API_MODEL_CATALOG_SYNC=true")
		return
	}
	ctx, cancel := context.WithTimeout(c.Request.Context(), 20*time.Second)
	defer cancel()
	if err := h.syncGatewayModelCatalogWithOptions(ctx, true); err != nil {
		c.Header("X-Pico-Model-Catalog", "stale")
		util.Fail(c, http.StatusBadGateway, 502, "同步 Sub2API 模型目录失败，请检查网关地址、Token 和上游账号")
		return
	}
	items, err := h.models.ListGatewayCatalogForDisplay(ctx, "")
	if err != nil {
		util.InternalError(c, err.Error())
		return
	}
	h.admin.LogOperation(c.Request.Context(), c.GetInt64("admin_id"), "sync_sub2api_model_catalog", "model", "catalog", map[string]interface{}{
		"count": len(items),
	})
	util.OK(c, map[string]interface{}{
		"items":        items,
		...
		"source":       "sub2api",
		"pricing_note": "新模型需要设置 PICO 用户售价后才能发布",
	})
}
...
// AdminUpdatePicoCatalogPricing is intentionally narrower than the general
// model update endpoint. It lets an operator set the PICO customer price for
// a Sub2API-synchronised model without sending (or overwriting) upstream
// credentials, endpoint configuration, schemas, or runtime rules.
func (h *Handler) AdminUpdatePicoCatalogPricing(c *gin.Context) {
	id, err := strconv.ParseInt(c.Param("id"), 10, 64)
	if err != nil || id <= 0 {
		util.BadRequest(c, "模型 ID 无效")
		return
	...
	if req.PriceRule == nil {
		req.PriceRule = model.PriceRule
	}
	enabled := model.IsEnabled
	if req.IsEnabled != nil {
		enabled = *req.IsEnabled
	}
	updated, err := h.models.UpdatePicoCatalogPricing(c.Request.Context(), id, req.PriceRule, enabled)
	if err != nil {
		util.BadRequest(c, err.Error())
		return
	}
	h.admin.LogOperation(c.Request.Context(), c.GetInt64("admin_id"), "update_pico_catalog_pricing", "model", updated.Code, map[string]interface{}{
		"is_enabled": updated.IsEnabled,
		"price_rule": updated.PriceRule,
	})
	util.OK(c, updated)
}
...
func (h *Handler) ListAPIDocs(c *gin.Context) {
	...
	for i := range items {
		service.SanitizePublicAPIDoc(&items[i])
	}
	util.OK(c, map[string]interface{}{"items": items})
}
...
func (h *Handler) GetAPIDoc(c *gin.Context) {
	...
	service.SanitizePublicAPIDoc(item)
	util.OK(c, item)
}
...
func (h *Handler) autoTranslateContent(ctx context.Context, locale, modelCode, entityType, entityKey string, limit int) (int, error) {
	...
	targetName := map[string]string{"en-US": "English", "ja-JP": "Japanese", "ko-KR": "Korean", "vi-VN": "Vietnamese", "it-IT": "Italian"}[locale]
	if targetName == "" {
		targetName = locale
	}
...
func (h *Handler) GetPublicSystemConfigs(c *gin.Context) {
	...
	util.OK(c, map[string]interface{}{
		...
		// Defaults to false, unlike api_docs_enabled: the email OTP path depends on
		// a working outbound mailer, so it must be switched on deliberately.
		"email_otp_login_enabled":         configTruthy(cfg["email_otp_login_enabled"]),
...
func (h *Handler) UploadAsset(c *gin.Context) {
	...
	case strings.HasPrefix(contentType, "video/") || strings.HasSuffix(lowerName, ".mp4") || strings.HasSuffix(lowerName, ".mov") || strings.HasSuffix(lowerName, ".webm") || strings.HasSuffix(lowerName, ".mkv") || strings.HasSuffix(lowerName, ".avi"):
		inferredKind = "video"
	case strings.HasPrefix(contentType, "audio/") || strings.HasSuffix(lowerName, ".mp3") || strings.HasSuffix(lowerName, ".wav") || strings.HasSuffix(lowerName, ".m4a") || strings.HasSuffix(lowerName, ".aac") || strings.HasSuffix(lowerName, ".ogg") || strings.HasSuffix(lowerName, ".opus") || strings.HasSuffix(lowerName, ".flac"):
		inferredKind = "audio"
	}
	if kind == "" {
		kind = inferredKind
	...
	if kind == "audio" && !strings.HasPrefix(contentType, "audio/") && inferredKind != "audio" {
...
// ListApiTokenProducts is derived from the PICO catalog. A new model added
// in Sub2API will appear here only after it has a PICO retail price and has
// been explicitly published by the operator.
func (h *Handler) ListApiTokenProducts(c *gin.Context) {
	if err := h.syncGatewayModelCatalog(c.Request.Context()); err != nil {
		c.Header("X-Pico-Model-Catalog", "stale")
	}
	catalogSync := h.cfg != nil && h.cfg.NewAPIModelCatalogSync
	items, err := h.models.ListApiKeyProductsForCatalog(c.Request.Context(), catalogSync)
	if err != nil {
		util.InternalError(c, err.Error())
		return
	}
...
func (h *Handler) CreateApiToken(c *gin.Context) {
	...
	if err := h.syncGatewayModelCatalog(c.Request.Context()); err != nil {
		// Creating a key from the last known, published catalog is safe: scopes
		// are still checked against the materialized PICO catalog and never grant
		// access to an unknown upstream model.
		c.Header("X-Pico-Model-Catalog", "stale")
	}
	catalogSync := h.cfg != nil && h.cfg.NewAPIModelCatalogSync
	product, err := h.models.ApiKeyProductByCodeForCatalog(c.Request.Context(), req.ProductCode, catalogSync)
	if err != nil {
		util.BadRequest(c, err.Error())
		return
	}
	// New keys must always carry a bounded entitlement. The legacy all-model
	// product remains readable for existing keys, but it cannot be used to
	// mint another unrestricted key; callers must select concrete models from
	// the catalog instead.
	if strings.EqualFold(strings.TrimSpace(product.Code), "pico-all") {
		if len(req.ModelScopes) == 0 {
			util.BadRequest(c, "请按具体模型创建 API Key，不能创建未限制的全模型 Key")
			return
		}
		for _, scope := range req.ModelScopes {
	...
	// The product is resolved from the server-side published catalog above.
	// Any model list supplied by the client is only a narrowing request; it can
	// never expand a provider/all-model product or authorize an unpublished ID.
	modelScopes, err := service.SelectApiTokenScopes(product, req.ModelScopes)
	if err != nil {
		util.BadRequest(c, err.Error())
		return
	}
	token, dto, err := h.ops.CreateApiToken(c.Request.Context(), c.GetInt64("user_id"), req.Name, req.Protocol, product.Code, modelScopes)
	if err != nil {
		util.BadRequest(c, err.Error())
		return
	}
...
func (h *Handler) apiTokenAllowsModel(c *gin.Context, modelCode string) bool {
	raw, exists := c.Get("api_token_identity")
	if !exists {
		return true
	}
	identity, ok := raw.(service.ApiTokenIdentity)
	// ApiTokenAuth always stores a concrete identity before entering /v1
	// handlers.  Treat a present but malformed value as an authentication
	// failure instead of accidentally allowing every model.
	if !ok {
		openAPIError(c, http.StatusInternalServerError, "authentication_error", "API Key 身份无效，请重新认证。")
		return false
	}
	if h.apiTokenIdentityAllowsModel(c, identity, modelCode) {
		return true
	}
	openAPIError(c, http.StatusForbidden, "model_not_allowed", "此 API Key 未获得该模型的调用权限，请在 API 管理中创建对应的 PICO Key。")
	return false
}

const apiTokenProductScopesCacheKey = "pico_api_token_product_scopes"

// apiTokenStoredScopesNeedRefresh identifies tokens created before vendor
// products switched to exact model scopes. Those rows can still contain a
// broad prefix wildcard (for example `gpt-*`); trusting it would bypass the
// current published catalog. The all-model product intentionally keeps its
// `*` scope, because model resolution/publication checks remain the final
// gate for that product.
func apiTokenStoredScopesNeedRefresh(identity service.ApiTokenIdentity) bool {
	productCode := strings.TrimSpace(identity.ProductCode)
	if productCode == "" || strings.EqualFold(productCode, "pico-all") {
		return false
	}
	for _, scope := range identity.ModelScopes {
		if strings.ContainsAny(strings.TrimSpace(scope), "*?[") {
			return true
		}
	}
	return false
...
// apiTokenScopeDecision separates the immutable exact-scope decision from the
// legacy wildcard refresh path. The second return value is true only when a
// pre-existing vendor wildcard must be resolved against the current catalog.
// This is deliberately pure so the no-fallback rule for selected-model keys is
// easy to regression-test without a database.
func apiTokenScopeDecision(identity service.ApiTokenIdentity, modelCode string) (allow, refresh bool) {
	refresh = apiTokenStoredScopesNeedRefresh(identity)
	if !service.ApiTokenAllowsModel(identity.ModelScopes, modelCode) {
		return false, refresh
	}
	if refresh {
		// Legacy wildcard scopes are re-evaluated against the published product;
		// a matching prefix alone is not enough.
		return false, true
	}
	return true, false
}

// apiTokenIdentityAllowsModel performs the non-writing part of API Key scope
// validation. Keeping it separate lets /v1/models advertise newly published
// models to existing vendor keys while apiTokenAllowsModel can still emit a
// protocol-shaped error for an actual invocation.
func (h *Handler) apiTokenIdentityAllowsModel(c *gin.Context, identity service.ApiTokenIdentity, modelCode string) bool {
	// Exact scopes are authoritative. In particular, a key created for one
	// selected model must not fall through to its provider product and silently
	// gain access to every sibling model. Only migration-era vendor keys that
	// contain a wildcard are eligible for the catalog refresh below.
	allow, needsRefresh := apiTokenScopeDecision(identity, modelCode)
	if allow {
		return true
	}
	if !needsRefresh {
	...
	// model_scopes is stored on the token as a creation-time snapshot for
	// auditability. When a vendor product receives a newly published Sub2API
	// model, refresh that product once on the miss so existing keys pick up the
	// current catalog without being regenerated. Model resolution has already
	// checked that modelCode is enabled and PICO-priced; this lookup only narrows
	// the token entitlement and never grants access to an unpublished model.
	productCode := strings.TrimSpace(identity.ProductCode)
	if h.models == nil || c == nil || productCode == "" || strings.EqualFold(productCode, "pico-all") {
		return false
	}
	var scopesByProduct map[string][]string
	if cached, exists := c.Get(apiTokenProductScopesCacheKey); exists {
		scopesByProduct, _ = cached.(map[string][]string)
	}
	if scopesByProduct == nil {
		scopesByProduct = make(map[string][]string)
		c.Set(apiTokenProductScopesCacheKey, scopesByProduct)
	}
	if scopes, loaded := scopesByProduct[productCode]; loaded {
		return service.ApiTokenAllowsModel(scopes, modelCode)
	}
	catalogSync := h.cfg != nil && h.cfg.NewAPIModelCatalogSync
	product, err := h.models.ApiKeyProductByCodeForCatalog(c.Request.Context(), productCode, catalogSync)
	if err != nil || product == nil {
		// Cache a nil/empty result for this request as well, so a long model list
		// cannot turn one unknown product code into N identical database queries.
		scopesByProduct[productCode] = nil
		return false
	}
	scopesByProduct[productCode] = product.ModelScopes
	return service.ApiTokenAllowsModel(product.ModelScopes, modelCode)
}
