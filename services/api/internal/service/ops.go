...
const (
	ApiTokenProtocolUniversal = "universal"
	ApiTokenProtocolOpenAI    = "openai"
	ApiTokenProtocolAnthropic = "anthropic"
	ApiTokenProtocolGemini    = "gemini"
)

func normalizeApiTokenProtocol(protocol string) (string, error) {
	switch strings.ToLower(strings.TrimSpace(protocol)) {
	case "", ApiTokenProtocolUniversal:
		return ApiTokenProtocolUniversal, nil
	case ApiTokenProtocolOpenAI:
		return ApiTokenProtocolOpenAI, nil
	case ApiTokenProtocolAnthropic:
		return ApiTokenProtocolAnthropic, nil
	case ApiTokenProtocolGemini:
		return ApiTokenProtocolGemini, nil
	default:
		return "", errors.New("不支持的 API Key 协议")
	}
}

// normalizeStoredApiTokenScopes keeps the migration-era compatibility rule
// (an omitted scope on the legacy all-model product means all published
// models) without letting an empty vendor scope accidentally become a global
// grant. New vendor products are always persisted with exact model IDs.
func normalizeStoredApiTokenScopes(productCode string, scopes []string) ([]string, error) {
	productCode = strings.TrimSpace(productCode)
	allProduct := productCode == "" || strings.EqualFold(productCode, "pico-all")
	...
	if len(clean) == 0 {
		...
		return nil, errors.New("API Key 权限配置无效")
	}
	if !allProduct {
		for _, scope := range clean {
			if strings.ContainsAny(scope, "*?[") {
				return nil, errors.New("厂商 API Key 必须使用具体模型权限")
			}
		}
	}
	return clean, nil
...
func (s *OpsService) ListApiTokens(ctx context.Context, userID int64) ([]ApiTokenDTO, error) {
	rows, err := s.db.Query(ctx,
		// Never select token_cipher for a list request. The plaintext token is
		// returned only in the response that creates it; a later list call gets
		// the prefix and metadata needed for management without creating another
		// decryption oracle.
		`SELECT id, name, prefix, protocol, product_code, model_scopes, status, last_used_at, created_at FROM api_tokens WHERE user_id=$1 ORDER BY created_at DESC`, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		var created time.Time
		if err := rows.Scan(&t.ID, &t.Name, &t.Prefix, &t.Protocol, &t.ProductCode, &scopes, &t.Status, &lastUsed, &created); err != nil {
			return nil, err
		}
		_ = json.Unmarshal(scopes, &t.ModelScopes)
		// A malformed/empty vendor scope is shown as unavailable rather than
		// being rendered as an all-model key. The API authentication path below
		// fails closed for the same row.
		if normalized, normalizeErr := normalizeStoredApiTokenScopes(t.ProductCode, t.ModelScopes); normalizeErr == nil {
			t.ModelScopes = normalized
		} else {
			t.ModelScopes = nil
		}
...
func (s *OpsService) CreateApiToken(ctx context.Context, userID int64, name, protocol, productCode string, modelScopes []string) (string, *ApiTokenDTO, error) {
	protocol, err := normalizeApiTokenProtocol(protocol)
	if err != nil {
		return "", nil, err
	}
	if name == "" {
	...
	normalizedScopes, err := normalizeStoredApiTokenScopes(productCode, modelScopes)
	if err != nil {
		return "", nil, err
	}
	...
	var created time.Time
	err = s.db.QueryRow(ctx,
		`INSERT INTO api_tokens (user_id, name, token_hash, prefix, protocol, product_code, model_scopes, token_cipher, status) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,'active') RETURNING id, created_at`,
		userID, name, hash, prefix, protocol, productCode, scopesJSON, tokenCipher).Scan(&id, &created)
	if err != nil {
		return "", nil, err
	}
	return token, &ApiTokenDTO{ID: id, Name: name, Prefix: prefix, Protocol: protocol, ProductCode: productCode, ModelScopes: normalizedScopes, Token: &token, Status: "active", CreatedAt: created.Format(time.RFC3339)}, nil
}
...
func (s *OpsService) AuthenticateApiToken(ctx context.Context, token string) (ApiTokenIdentity, error) {
	if token == "" {
		return ApiTokenIdentity{}, errors.New("API Key 不能为空")
	}
	sum := sha256.Sum256([]byte(token))
	hash := hex.EncodeToString(sum[:])
	var userID int64
	var tokenID int64
	var protocol, productCode string
	var scopes []byte
	err := s.db.QueryRow(ctx, `
		SELECT t.user_id, t.id, t.protocol, t.product_code, t.model_scopes
		FROM api_tokens t
		JOIN users u ON u.id = t.user_id
		WHERE t.token_hash=$1
		  AND COALESCE(NULLIF(t.status, ''), 'active')='active'
		  AND COALESCE(NULLIF(u.status, ''), 'active')='active'
		LIMIT 1`, hash).Scan(&userID, &tokenID, &protocol, &productCode, &scopes)
	if err != nil {
		return ApiTokenIdentity{}, errors.New("API Key 无效或已停用")
	}
	protocol, err = normalizeApiTokenProtocol(protocol)
	if err != nil {
		return ApiTokenIdentity{}, errors.New("API Key 协议无效")
	}
	var modelScopes []string
	_ = json.Unmarshal(scopes, &modelScopes)
	modelScopes, err = normalizeStoredApiTokenScopes(productCode, modelScopes)
	if err != nil {
		return ApiTokenIdentity{}, err
	}
	_, _ = s.db.Exec(ctx, `UPDATE api_tokens SET last_used_at=now() WHERE id=$1`, tokenID)
	return ApiTokenIdentity{UserID: userID, TokenID: tokenID, Protocol: protocol, ProductCode: productCode, ModelScopes: modelScopes}, nil
}
