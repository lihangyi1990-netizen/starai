...
// publicModelAvailabilitySQL is shared by every customer-facing resolver.
// Sub2API rows are materialized as disabled until an operator publishes a
// PICO retail price, and they become unavailable when the latest gateway sync
// no longer advertises them. Checking both fields here prevents an old/manual
// enable flag from making a stale upstream model callable again.
const publicModelAvailabilitySQL = `(new_api_extra_params->>'catalog_source' IS DISTINCT FROM 'sub2api'
	OR (new_api_extra_params->>'catalog_status'='active'
		AND price_rule->>'pico_pricing_status'='published'))`

// publicModelDisplayAvailabilitySQL is intentionally wider than the callable
// predicate. It exposes an active, synchronized catalog row only as metadata,
// so a customer can see why it is unavailable while the runtime and /v1 API
// continue to require a published retail price.
const publicModelDisplayAvailabilitySQL = `((is_enabled=true AND ` + publicModelAvailabilitySQL + `) OR
	(new_api_extra_params->>'catalog_source' = 'sub2api' AND
	 new_api_extra_params->>'catalog_status' = 'active'))`

// publicAudioCapabilitySQL describes models that can produce an audio result
// even when audio is not their primary catalog category. A model row has one
// primary category, but providers commonly expose multimodal models (for
// example gpt-4o audio-preview/realtime) through the chat category as well.
// Keep the primary category untouched and use this predicate only when a
// caller explicitly asks for the audio catalog. The runtime `audio` object is
// ignored for video transports because video models often use that key merely
// to describe an embedded soundtrack. Name-based detection is deliberately
// symmetric so both `video-with-audio` and `audio-video` stay out of this view.
const publicAudioCapabilitySQL = `(
	(
		category = 'audio'
		OR request_mode = 'audio'
		OR lower(COALESCE(new_api_endpoint, '')) ~ '(^|/)audio(/|$)'
		OR (
			category <> 'video'
			AND request_mode <> 'video'
			AND lower(COALESCE(new_api_endpoint, '')) !~ '(^|/)videos?(/|$)'
			AND (
				runtime_rule->'audio' = 'true'::jsonb
				OR jsonb_typeof(runtime_rule->'audio') = 'object'
			)
			)
		OR COALESCE(runtime_rule, '{}'::jsonb)->'capabilities'->>'audio' = 'true'
		OR COALESCE(runtime_rule, '{}'::jsonb)->'capabilities'->>'audio_generation' = 'true'
		OR COALESCE(runtime_rule, '{}'::jsonb)->'capabilities'->>'audio_output' = 'true'
		OR COALESCE(runtime_rule, '{}'::jsonb)->'capabilities'->>'tts' = 'true'
		OR COALESCE(runtime_rule, '{}'::jsonb)->'capabilities'->>'voice_generation' = 'true'
		OR COALESCE(runtime_rule, '{}'::jsonb)->'capabilities'->>'music_generation' = 'true'
		OR COALESCE(capabilities, '{}'::jsonb)->>'audio' = 'true'
		OR COALESCE(capabilities, '{}'::jsonb)->>'audio_generation' = 'true'
		OR COALESCE(capabilities, '{}'::jsonb)->>'audio_output' = 'true'
		OR COALESCE(capabilities, '{}'::jsonb)->>'tts' = 'true'
		OR COALESCE(capabilities, '{}'::jsonb)->>'voice_generation' = 'true'
		OR COALESCE(capabilities, '{}'::jsonb)->>'music_generation' = 'true'
		OR COALESCE(capabilities, '{}'::jsonb)->'modalities' ? 'audio'
		OR COALESCE(capabilities, '{}'::jsonb)->'output_modalities' ? 'audio'
		OR COALESCE(capabilities, '{}'::jsonb)->'response_modalities' ? 'audio'
		OR COALESCE(runtime_rule, '{}'::jsonb)->'capabilities'->'modalities' ? 'audio'
		OR COALESCE(runtime_rule, '{}'::jsonb)->'capabilities'->'output_modalities' ? 'audio'
		OR COALESCE(runtime_rule, '{}'::jsonb)->'capabilities'->'response_modalities' ? 'audio'
		OR COALESCE(runtime_rule, '{}'::jsonb)->'modalities' ? 'audio'
		OR COALESCE(runtime_rule, '{}'::jsonb)->'output_modalities' ? 'audio'
		OR COALESCE(runtime_rule, '{}'::jsonb)->'response_modalities' ? 'audio'
		OR COALESCE(new_api_extra_params, '{}'::jsonb)->>'audio_capable' = 'true'
		OR lower(COALESCE(code, '')) ~ '(^|[-_. ])(audio|audio-preview|realtime|tts|speech|voice|music|suno|whisper|transcri[a-z]*|eleven|fish[-_. ]?speech|cosyvoice)([-_. ]|$)'
		OR lower(COALESCE(display_name, '')) ~ '(^|[-_. ])(audio|audio-preview|realtime|tts|speech|voice|music|suno|whisper|transcri[a-z]*|eleven|fish[-_. ]?speech|cosyvoice)([-_. ]|$)'
		OR lower(COALESCE(tags::text, '')) ~ '["](audio|audio-preview|realtime|tts|speech|voice|music|suno|whisper|transcri[a-z]*|eleven|fish[-_. ]?speech|cosyvoice)["]'
	)
	AND NOT (
		(
			lower(COALESCE(code, '')) LIKE '%video%'
			AND lower(COALESCE(code, '')) LIKE '%audio%'
		)
		OR (
			lower(COALESCE(display_name, '')) LIKE '%video%'
			AND lower(COALESCE(display_name, '')) LIKE '%audio%'
		)
	)
)`
...
type ModelDTO struct {
	...
	// Provider is sourced from the gateway catalog metadata when available.
	// It is intentionally a public, non-secret field; connection credentials
	// remain in the admin-only extra params object.
...
func (s *ModelService) ListPublic(ctx context.Context, category string) ([]ModelDTO, error) {
	return s.listPublic(ctx, category, false)
}

// ListPublicForDisplay returns the model directory used by the PICO web app.
//
// A Sub2API model is materialized as disabled until an operator sets PICO's
// retail price. It still needs to be visible in the directory so the operator
// (and the user) can understand why it cannot be selected yet. Only entries
// that are still marked active by the latest gateway sync are included; stale
// rows marked unavailable remain hidden. ListPublic intentionally keeps its
// stricter, callable-only semantics for /v1/models and runtime resolution.
func (s *ModelService) ListPublicForDisplay(ctx context.Context, category string) ([]ModelDTO, error) {
	return s.listPublic(ctx, category, true)
}

func (s *ModelService) listPublic(ctx context.Context, category string, includeActiveCatalog bool) ([]ModelDTO, error) {
	where := "is_enabled=true AND " + publicModelAvailabilitySQL
	if includeActiveCatalog {
		where = publicModelDisplayAvailabilitySQL
	}
	...
	if category != "" && category != "all" {
		...
		} else if category == "audio" && includeActiveCatalog {
			// Audio is a capability view, not a second primary category. Keep
			// multimodal/chat rows in their original category while allowing
			// explicitly audio-capable rows to appear here as well.
...
func (s *ModelService) GetByCode(ctx context.Context, code string, publicOnly bool) (*ModelDTO, error) {
	...
	if publicOnly {
		q += ` AND is_enabled=true AND ` + publicModelAvailabilitySQL
	}
	var m ModelDTO
	var tags, runtime, schema, defaults, price, extra []byte
	err := s.db.QueryRow(ctx, q, code).Scan(
		&m.ID, &m.Code, &m.DisplayName, &m.Category, &m.IconURL, &m.Description,
		&tags, &runtime, &schema, &defaults, &price, &m.IsEnabled, &m.SortOrder, &extra)
	if err != nil {
	...
	setModelProvider(&m, extra)
	if publicOnly {
		sanitizeGatewayCatalogPublicMetadata(&m, extra)
	}
	return &m, nil
}

// GetByCodeForDisplay follows the same visibility rule as the model plaza.
// An active Sub2API row awaiting pricing is safe to return here because this
// DTO deliberately excludes endpoint credentials and it remains disabled for
// every invocation path.
func (s *ModelService) GetByCodeForDisplay(ctx context.Context, code string) (*ModelDTO, error) {
	q := `SELECT id, code, display_name, category, icon_url, description, tags, runtime_rule, input_schema, default_params, price_rule, is_enabled, sort_order, new_api_extra_params
		FROM models WHERE code=$1 AND ` + publicModelDisplayAvailabilitySQL
	var m ModelDTO
	var tags, runtime, schema, defaults, price, extra []byte
	err := s.db.QueryRow(ctx, q, code).Scan(
		&m.ID, &m.Code, &m.DisplayName, &m.Category, &m.IconURL, &m.Description,
...
func (s *ModelService) GetByID(ctx context.Context, id int64) (*ModelDTO, error) {
	...
	err := s.db.QueryRow(ctx, `
		SELECT id, code, display_name, category, icon_url, description, tags, runtime_rule, input_schema, default_params, price_rule, is_enabled, sort_order, new_api_extra_params
...
func (s *ModelService) GetFullByCode(ctx context.Context, code string) (*ModelFull, error) {
	...
	err := s.db.QueryRow(ctx, `
		...
		WHERE code=$1
		  AND is_enabled=true
		  AND `+publicModelAvailabilitySQL+``, code).Scan(
	...
	json.Unmarshal(price, &m.PriceRule)
	json.Unmarshal(runtime, &m.RuntimeRule)
	setModelProvider(&m.ModelDTO, extra)
	return &m, nil
}
...
func (s *ModelService) ResolveChatModel(ctx context.Context, identifier string) (*ModelFull, error) {
	...
	err := s.db.QueryRow(ctx, `
		...
		WHERE is_enabled=true
		  AND new_api_model=$1
		  AND `+publicModelAvailabilitySQL+`
	...
	rows, err := s.db.Query(ctx, `
		SELECT category FROM (
			...
			WHERE EXISTS (
				...
				WHERE is_enabled=true AND `+publicModelAvailabilitySQL+` AND `+publicAudioCapabilitySQL+`
			)
		) AS model_categories
		ORDER BY category`)
	if err != nil {
...
func (s *ModelService) GetAPIDoc(ctx context.Context, slug string, publicOnly bool) (*APIDocDTO, error) {
	...
	item, err := scanAPIDocRow(s.db.QueryRow(ctx, q, slug))
	if err != nil {
		return nil, err
	}
	if publicOnly {
...
func (s *ModelService) Update(ctx context.Context, id int64, input CreateModelInput) (*ModelDTO, error) {
	...
	if err := s.markManualTransportOverride(ctx, id, &input); err != nil {
		return nil, err
	}
	if err := validateModelConnection(input); err != nil {
		return nil, err
	...
	if strings.EqualFold(stringValue(input.NewAPIExtraParams["catalog_source"]), "sub2api") {
		if picoCatalogPriceReady(input.PriceRule) {
			input.PriceRule["pico_pricing_status"] = "published"
		} else {
			input.PriceRule["pico_pricing_status"] = "pending"
...
func (s *ModelService) SetEnabled(ctx context.Context, id int64, enabled bool) (*ModelDTO, error) {
	if enabled {
		...
		if err := s.db.QueryRow(ctx, `SELECT price_rule, new_api_extra_params FROM models WHERE id=$1`, id).Scan(&price, &extra); err != nil {
			return nil, errors.New("模型不存在")
		}
		var priceRule, extraParams map[string]interface{}
		_ = json.Unmarshal(price, &priceRule)
		_ = json.Unmarshal(extra, &extraParams)
		if strings.EqualFold(stringValue(extraParams["catalog_source"]), "sub2api") {
			if !strings.EqualFold(strings.TrimSpace(stringValue(extraParams["catalog_status"])), "active") {
				return nil, errors.New("该模型已不在 Sub2API 当前模型目录中，暂不能发布")
			}
			if !picoCatalogPricePublished(priceRule) {
				return nil, errors.New("请先设置 PICO 用户售价，模型才能发布")
			}
		}
	}
...
// normalizePicoCatalogPricing applies the small, user-facing part of a
// Sub2API-synchronised model update. It deliberately returns a copied map so
// callers cannot mutate a model's stored pricing while validating it. A
// model may keep a valid price while disabled; that makes it possible to
// publish it later without re-entering the price. Invalid/zero prices are
// always marked pending and cannot be enabled.
func normalizePicoCatalogPricing(rule map[string]interface{}, requestedEnabled bool) (map[string]interface{}, bool, error) {
	if rule == nil {
		return nil, false, errors.New("price_rule 参数不能为空")
	}
	normalized := copyMap(rule)
	// The publication marker is owned by the server. Never trust a client to
	// mark an unvalidated price as published.
	delete(normalized, "pico_pricing_status")
	if err := validateModelPriceRule(normalized); err != nil {
		return nil, false, err
	}
	ready := picoCatalogPriceReady(normalized)
	if ready {
		normalized["pico_pricing_status"] = "published"
		return normalized, requestedEnabled, nil
	}
	normalized["pico_pricing_status"] = "pending"
	if requestedEnabled {
		return normalized, false, errors.New("请先设置大于 0 的 PICO 用户售价，模型才能发布")
	}
	return normalized, false, nil
}

// UpdatePicoCatalogPricing updates only the PICO retail price and publication
// flag for a model materialized from Sub2API. Upstream endpoint, credentials,
// schemas, runtime rules and routes are intentionally outside this method.
func (s *ModelService) UpdatePicoCatalogPricing(ctx context.Context, id int64, priceRule map[string]interface{}, enabled bool) (*ModelDTO, error) {
	...
	var existingPrice, extra []byte
	if err := tx.QueryRow(ctx, `
		SELECT price_rule, new_api_extra_params
		FROM models
		WHERE id=$1
		FOR UPDATE`, id).Scan(&existingPrice, &extra); err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return nil, errors.New("模型不存在")
		}
		return nil, err
	...
	_ = json.Unmarshal(extra, &extraParams)
	if !strings.EqualFold(strings.TrimSpace(stringValue(extraParams["catalog_source"])), "sub2api") {
		return nil, errors.New("只有 Sub2API 同步模型可以单独设置 PICO 用户售价")
	}
	if enabled && !strings.EqualFold(strings.TrimSpace(stringValue(extraParams["catalog_status"])), "active") {
		return nil, errors.New("该模型已不在 Sub2API 当前模型目录中，暂不能发布")
	}

	// Keep non-PICO fields already present in the rule (for example dynamic
	// billing metadata), then overlay the fields supplied by the operator.
	var currentPrice map[string]interface{}
	_ = json.Unmarshal(existingPrice, &currentPrice)
	if currentPrice == nil {
		currentPrice = map[string]interface{}{}
	}
	merged := copyMap(currentPrice)
	for key, value := range priceRule {
		merged[key] = value
	}
	normalized, finalEnabled, err := normalizePicoCatalogPricing(merged, enabled)
	if err != nil {
		return nil, err
	}
	...
	result, err := tx.Exec(ctx, `
		UPDATE models
		SET price_rule=$1, is_enabled=$2, updated_at=now()
		WHERE id=$3`, encoded, finalEnabled, id)
	if err != nil {
		return nil, err
	}
	if result.RowsAffected() == 0 {
...
// gatewayCatalogTransportChanged reports whether an operator edit moves a
// catalog-sourced row away from the category/endpoint/request_mode that the
// catalog heuristic assigned. Endpoints are compared after normalization so a
// formatting-only difference is not mistaken for an override.
func gatewayCatalogTransportChanged(oldCategory, oldEndpoint, oldMode, newCategory, newEndpoint, newMode string) bool {
	return strings.TrimSpace(oldCategory) != strings.TrimSpace(newCategory) ||
		normalizeModelEndpoint(oldEndpoint) != normalizeModelEndpoint(newEndpoint) ||
		strings.TrimSpace(oldMode) != strings.TrimSpace(newMode)
}

// retagGatewayCatalogCategory keeps the tag generated by SyncGatewayCatalog in
// step with a category change. The sync writes exactly one tag (the category),
// so a single matching tag is recognizably auto-generated; anything an operator
// actually curated is left untouched.
func retagGatewayCatalogCategory(tags []string, oldCategory, newCategory string) []string {
	newCategory = strings.TrimSpace(newCategory)
	if len(tags) != 1 || newCategory == "" {
		return tags
	}
	if !strings.EqualFold(strings.TrimSpace(tags[0]), strings.TrimSpace(oldCategory)) {
		return tags
	}
	return []string{newCategory}
}

// markManualTransportOverride records that an operator, not the catalog
// heuristic, owns this row's category/endpoint/request_mode. SyncGatewayCatalog
// reads the marker and stops overwriting those fields, the same way
// provider_source=manual protects a custom provider.
//
// The marker is only written when the values actually change, so saving an
// unrelated field (a retail price, for example) does not freeze a row against
// future upstream category corrections.
func (s *ModelService) markManualTransportOverride(ctx context.Context, id int64, input *CreateModelInput) error {
	if input == nil {
		return nil
	}
	if !strings.EqualFold(strings.TrimSpace(stringValue(input.NewAPIExtraParams["catalog_source"])), "sub2api") {
		return nil
	}
	var oldCategory, oldEndpoint, oldMode string
	var raw []byte
	if err := s.db.QueryRow(ctx,
		`SELECT category, new_api_endpoint, request_mode, new_api_extra_params FROM models WHERE id=$1`, id,
	).Scan(&oldCategory, &oldEndpoint, &oldMode, &raw); err != nil {
		return err
	}
	if input.NewAPIExtraParams == nil {
		input.NewAPIExtraParams = map[string]interface{}{}
	}

	// An override established by an earlier edit stays in force even if this
	// request omitted the marker.
	var existing map[string]interface{}
	_ = json.Unmarshal(raw, &existing)
	if strings.EqualFold(strings.TrimSpace(stringValue(existing["category_source"])), "manual") {
		input.NewAPIExtraParams["category_source"] = "manual"
	}

	if !gatewayCatalogTransportChanged(oldCategory, oldEndpoint, oldMode, input.Category, input.NewAPIEndpoint, input.RequestMode) {
		return nil
	}
	input.NewAPIExtraParams["category_source"] = "manual"
	input.Tags = retagGatewayCatalogCategory(input.Tags, oldCategory, input.Category)
	return nil
}
...
func scanModels(rows pgx.Rows) ([]ModelDTO, error) {
	// Keep collection endpoints JSON-array shaped even when the gateway has no
	// cached models yet. `null` forces every frontend caller to special-case a
	// temporarily empty catalog and was a common source of blank screens.
	models := make([]ModelDTO, 0)
	for rows.Next() {
		var m ModelDTO
		var tags, runtime, schema, defaults, price, extra []byte
		if err := rows.Scan(&m.ID, &m.Code, &m.DisplayName, &m.Category, &m.IconURL, &m.Description,
			&tags, &runtime, &schema, &defaults, &price, &m.IsEnabled, &m.SortOrder, &extra); err != nil {
			return nil, err
		}
		json.Unmarshal(tags, &m.Tags)
		json.Unmarshal(runtime, &m.RuntimeRule)
...
// setModelProvider extracts only the non-sensitive provider hint from the
// model's extra metadata. The same helper is used by public and admin DTOs so
// provider labels in the model plaza and API-key products stay in sync without
// exposing connection.api_key or other upstream credentials.
func setModelProvider(model *ModelDTO, raw []byte) {
	if model == nil {
		return
	}
	...
	if extra != nil {
		if provider, ok := extra["provider"].(string); ok && strings.TrimSpace(provider) != "" && !CatalogProviderHintIsGeneric(provider) {
			model.Provider = strings.TrimSpace(provider)
			return
		}
		if strings.EqualFold(stringValue(extra["catalog_source"]), "sub2api") {
			model.Provider = CatalogProvider(model.Code)
			return
		}
	}
}
