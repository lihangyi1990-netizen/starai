...
// validateLinkedUserID makes the explicit admin-to-Tuna link fail closed. A
// zero value is accepted by updates as an unlink request; positive values must
// refer to an existing Tuna user. The partial unique index on admin_users
// enforces the one-admin-per-user invariant concurrently.
func (s *AdminService) validateLinkedUserID(ctx context.Context, linkedUserID *int64) error {
	if linkedUserID == nil || *linkedUserID == 0 {
		return nil
	}
	if *linkedUserID < 0 {
		return fmt.Errorf("无效的 Tuna 用户 ID")
	}
	var exists bool
	if err := s.db.QueryRow(ctx, `SELECT EXISTS(SELECT 1 FROM users WHERE id=$1)`, *linkedUserID).Scan(&exists); err != nil {
		return err
	}
	if !exists {
		return fmt.Errorf("Tuna 用户不存在")
	}
	return nil
}
...
func (s *AdminService) ListAdminAccounts(ctx context.Context) ([]AdminAccountDTO, error) {
	rows, err := s.db.Query(ctx, `
		SELECT a.id, a.email, r.name, a.status, a.user_id, a.created_at, a.updated_at
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.ID, &item.Email, &item.Role, &item.Status, &item.LinkedUserID, &created, &updated); err != nil {
			return nil, err
		}
...
func (s *AdminService) CreateAdminAccount(ctx context.Context, in AdminAccountInput) (*AdminAccountDTO, error) {
	...
	if err := s.validateLinkedUserID(ctx, in.LinkedUserID); err != nil {
		return nil, err
	}
	...
	var id int64
	err = s.db.QueryRow(ctx, `
		INSERT INTO admin_users (email, password_hash, role_id, status, user_id)
		VALUES ($1,$2,$3,$4,$5) RETURNING id`, email, string(hash), roleID, status, nullableLinkedUserID(in.LinkedUserID)).Scan(&id)
	if err != nil {
		return nil, err
	}
...
func (s *AdminService) GetAdminAccount(ctx context.Context, id int64) (*AdminAccountDTO, error) {
	...
	err := s.db.QueryRow(ctx, `
		...
		Scan(&item.ID, &item.Email, &item.Role, &item.Status, &item.LinkedUserID, &created, &updated)
	if err != nil {
		return nil, err
	}
	item.CreatedAt = created.Format(time.RFC3339)
...
func (s *AdminService) UpdateAdminAccount(ctx context.Context, id int64, in AdminAccountInput) error {
	...
	if in.LinkedUserID != nil {
		...
		if _, err := s.db.Exec(ctx, `UPDATE admin_users SET user_id=$1, updated_at=now() WHERE id=$2`, nullableLinkedUserID(in.LinkedUserID), id); err != nil {
			return err
		}
	}
...
func nullableLinkedUserID(linkedUserID *int64) interface{} {
	if linkedUserID == nil || *linkedUserID == 0 {
		return nil
	}
	return *linkedUserID
...
func (s *AdminService) listAdminUserApiTokens(ctx context.Context, userID int64, activeOnly bool, limit int) []AdminUserApiToken {
	...
	rows, err := s.db.Query(ctx, fmt.Sprintf(`
		SELECT id, name, prefix, protocol, token_cipher, status, last_used_at, created_at
		FROM api_tokens
		WHERE %s
	...
	for rows.Next() {
		...
		var c time.Time
		if rows.Scan(&t.ID, &t.Name, &t.Prefix, &t.Protocol, &cipher, &t.Status, &lastUsed, &c) != nil {
			continue
		}
...
func (s *AdminService) UpdateSystemConfig(ctx context.Context, key string, value interface{}) error {
	if isSensitiveConfigKey(key) {
		if text, ok := value.(string); ok {
			preserve, err := s.shouldPreserveMaskedSecret(ctx, key, text)
			if err != nil {
				return err
			}
			if preserve {
...
// shouldPreserveMaskedSecret distinguishes a value returned by
// GetSystemConfigs from a newly entered secret that merely happens to contain
// asterisks. The database lookup is intentional: shape alone cannot prove
// that a submitted value is the mask of the currently stored secret.
func (s *AdminService) shouldPreserveMaskedSecret(ctx context.Context, key, submitted string) (bool, error) {
	if !isMaskedAdminSecret(submitted) || s == nil || s.db == nil {
		return false, nil
	}
	var raw []byte
	err := s.db.QueryRow(ctx, `SELECT value FROM system_configs WHERE key=$1`, key).Scan(&raw)
	...
	var current string
	if err := json.Unmarshal(raw, &current); err != nil || strings.TrimSpace(current) == "" {
		return false, nil
	}
	return maskedAdminSecretMatches(current, submitted), nil
}
...
func isSensitiveDetailKey(key string) bool {
	k := strings.ToLower(key)
	return strings.Contains(k, "api_key") || strings.Contains(k, "token") || strings.Contains(k, "secret") || strings.Contains(k, "password") || strings.Contains(k, "private_key") || strings.Contains(k, "authorization")
}
...
func isMaskedAdminSecret(v string) bool {
	...
	// maskAdminSecret preserves four runes at either end of longer values and
	// places exactly four asterisks in the middle. Recognize that shape instead
	// of treating any value containing three stars as an unchanged mask; the
	// latter could silently discard a newly entered secret.
	runes := []rune(value)
	return len(runes) > 8 && string(runes[4:8]) == "****"
}

func maskedAdminSecretMatches(original, submitted string) bool {
	original = strings.TrimSpace(original)
	submitted = strings.TrimSpace(submitted)
	return original != "" && isMaskedAdminSecret(submitted) && submitted == maskAdminSecret(original)
}
