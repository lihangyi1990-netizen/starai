...
type PaymentService struct {
	...
	// waffoPancakeAPIBase is intentionally empty in production so the SDK uses
	// its verified default (https://api.waffo.ai). Tests may override it with a
	// local server without changing persisted configuration.
	waffoPancakeAPIBase string
}
...
type PaymentProviderConfig struct {
	...
	WaffoPancakeWebhookProdKey   string
	WaffoPancakeTaxCategory      string
	ExpireMinutes                int
	MinAmount                    float64
	MaxAmount                    float64
}
...
func (s *PaymentService) ProviderConfig(ctx context.Context) (PaymentProviderConfig, error) {
	rows, err := s.db.Query(ctx, `
		...
		WHERE key LIKE 'payment_%' OR key LIKE 'stripe_%' OR key LIKE 'paypal_%' OR key LIKE 'waffo_pancake_%'`)
	if err != nil {
	...
	cfg := PaymentProviderConfig{
		...
		WaffoPancakeMerchantID:       strings.TrimSpace(configString(values["waffo_pancake_merchant_id"])),
		WaffoPancakePrivateKey:       strings.TrimSpace(configString(values["waffo_pancake_private_key"])),
		WaffoPancakeStoreID:          strings.TrimSpace(configString(values["waffo_pancake_store_id"])),
		WaffoPancakeProductID:        strings.TrimSpace(configString(values["waffo_pancake_product_id"])),
		WaffoPancakeReturnURL:        strings.TrimSpace(configString(values["waffo_pancake_return_url"])),
		WaffoPancakeEnvironment:      strings.ToLower(strings.TrimSpace(configString(values["waffo_pancake_environment"]))),
		WaffoPancakeWebhookPublicKey: strings.TrimSpace(configString(values["waffo_pancake_webhook_public_key"])),
		WaffoPancakeWebhookTestKey:   strings.TrimSpace(configString(values["waffo_pancake_webhook_test_public_key"])),
		WaffoPancakeWebhookProdKey:   strings.TrimSpace(configString(values["waffo_pancake_webhook_prod_public_key"])),
		WaffoPancakeTaxCategory:      strings.TrimSpace(configString(values["waffo_pancake_tax_category"])),
	...
	if cfg.ProductName == "" {
		cfg.ProductName = "tuna Credits"
	}
	if cfg.PayPalMode == "" {
		cfg.PayPalMode = "sandbox"
	}
	if cfg.PayPalBrandName == "" {
		cfg.PayPalBrandName = "tuna"
	}
	if cfg.WaffoPancakeEnvironment == "" {
		cfg.WaffoPancakeEnvironment = "prod"
	}
	if cfg.WaffoPancakeTaxCategory == "" {
		cfg.WaffoPancakeTaxCategory = "saas"
	}
...
// WaffoPancakeReady reports whether new Pancake checkout sessions can be
// created. Webhook readiness is deliberately separate so an operator can
// disable new checkouts without losing callbacks for an order already paid.
func (c PaymentProviderConfig) WaffoPancakeReady() bool {
	return c.Enabled && c.WaffoPancakeCheckoutReady()
}

// WaffoPancakeCheckoutReady validates the credentials required to create a
// new hosted checkout. The RSA private key and Product ID are intentionally
// kept out of WebhookReady: removing or rotating either must not strand an
// order that was already created.
func (c PaymentProviderConfig) WaffoPancakeCheckoutReady() bool {
	if !c.WaffoPancakeWebhookReady() {
		return false
	}
	return strings.TrimSpace(c.WaffoPancakePrivateKey) != "" &&
		validWaffoPancakeShortID(c.WaffoPancakeProductID, "PROD")
}

// WaffoPancakeWebhookReady validates only the merchant/store binding needed
// to verify and settle a Pancake order. Public keys are optional because the
// official SDK ships the provider's built-in test and production keys; the
// checkout private key and Product ID are not needed on this path.
func (c PaymentProviderConfig) WaffoPancakeWebhookReady() bool {
	if !c.PaymentProviderMatches("waffo_pancake") || normalizeCurrency(c.Currency) == "" {
		return false
	}
	if !validWaffoPancakeShortID(c.WaffoPancakeMerchantID, "MER") ||
		!validWaffoPancakeShortID(c.WaffoPancakeStoreID, "STO") ||
		(c.WaffoPancakeEnvironment != "test" && c.WaffoPancakeEnvironment != "prod") {
		return false
	}
	return true
}
...
func (s *PaymentService) createPendingOrder(ctx context.Context, userID int64, amount float64, packageID interface{}, creditsOverride *float64) (*OrderDTO, error) {
	...
	case "waffo_pancake":
		checkoutURL, providerOrderID, err = s.createWaffoPancakeCheckout(ctx, cfg, orderNo, userID, amount, expiresAt)
	}
	if err != nil {
...
func (s *PaymentService) CompleteGenericWebhook(ctx context.Context, raw []byte, timestamp, signature string) (*PaymentCompletion, error) {
	...
	if !cfg.PaymentProviderMatches("generic") || !cfg.GenericWebhookReady() {
...
func (s *PaymentService) completeOrder(ctx context.Context, orderNo, channel, providerTrade string, paidAmount float64, paidCurrency, digest string) (*PaymentCompletion, error) {
	...
	var storedProviderTrade string
	var expiresAt *time.Time
	err = tx.QueryRow(ctx, `
		SELECT user_id, amount, compute_credited, status, currency,
		       COALESCE(provider_trade_no,''), expires_at
		FROM orders WHERE order_no=$1 AND channel=$2 FOR UPDATE`, orderNo, channel,
	).Scan(&userID, &amount, &credited, &status, &currency, &storedProviderTrade, &expiresAt)
	if errors.Is(err, pgx.ErrNoRows) {
	...
	if status == "paid" {
		if err := validatePaidOrderCallback(storedProviderTrade, providerTrade, amount, paidAmount, currency, paidCurrency); err != nil {
			log.Printf("payment callback rejected after order was paid order_no=%s channel=%s reason=%v", orderNo, channel, err)
			return nil, err
		}
	...
	if expiresAt != nil && !expiresAt.After(time.Now()) {
		return nil, errors.New("支付订单已过期，不可入账")
	}
...
// validatePaidOrderCallback protects the idempotent path from accepting a
// second callback that refers to a different provider transaction or carries
// different payment facts. Generic callbacks may omit currency, so an empty
// callback currency is intentionally treated as "not supplied"; providers
// that do supply it must match the stored order currency.
func validatePaidOrderCallback(storedTrade, callbackTrade string, expectedAmount, callbackAmount float64, expectedCurrency, callbackCurrency string) error {
	storedTrade = strings.TrimSpace(storedTrade)
	callbackTrade = strings.TrimSpace(callbackTrade)
	if storedTrade == "" || callbackTrade == "" || storedTrade != callbackTrade {
		return errors.New("支付回调交易号与已支付订单不一致")
	}
	if math.IsNaN(expectedAmount) || math.IsInf(expectedAmount, 0) || math.IsNaN(callbackAmount) || math.IsInf(callbackAmount, 0) || math.Abs(expectedAmount-callbackAmount) > 0.000001 {
		return errors.New("支付回调金额与已支付订单不一致")
	}
	if strings.TrimSpace(callbackCurrency) != "" {
		expected := normalizeCurrency(expectedCurrency)
		actual := normalizeCurrency(callbackCurrency)
		if expected == "" || actual == "" || expected != actual {
			return errors.New("支付回调币种与已支付订单不一致")
		}
	}
	return nil
}
