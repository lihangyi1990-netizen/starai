...
func TestPaymentProviderMatchesIsCaseInsensitiveAndExplicit(t *testing.T) {
	cfg := PaymentProviderConfig{Provider: "  GENERIC  "}
	if !cfg.PaymentProviderMatches("generic") {
		t.Fatal("configured generic provider should match generic callbacks")
	}
	if cfg.PaymentProviderMatches("stripe") {
		t.Fatal("generic provider must not match a Stripe callback")
	}
	if cfg.PaymentProviderMatches("") {
		t.Fatal("empty callback provider must not match")
	}
}

func TestValidatePaidOrderCallback(t *testing.T) {
	tests := []struct {
		name          string
		storedTrade   string
		callbackTrade string
		expected      float64
		callback      float64
		expectedCCY   string
		callbackCCY   string
		...
		{name: "same transaction and facts", storedTrade: "trade_1", callbackTrade: "trade_1", expected: 12.34, callback: 12.34, expectedCCY: "USD", callbackCCY: "usd"},
		{name: "generic callback may omit currency", storedTrade: "trade_1", callbackTrade: "trade_1", expected: 12.34, callback: 12.34, expectedCCY: "USD"},
		{name: "different transaction", storedTrade: "trade_1", callbackTrade: "trade_2", expected: 12.34, callback: 12.34, expectedCCY: "USD", callbackCCY: "USD", wantErr: true},
		{name: "different amount", storedTrade: "trade_1", callbackTrade: "trade_1", expected: 12.34, callback: 12.35, expectedCCY: "USD", callbackCCY: "USD", wantErr: true},
		{name: "different currency", storedTrade: "trade_1", callbackTrade: "trade_1", expected: 12.34, callback: 12.34, expectedCCY: "USD", callbackCCY: "EUR", wantErr: true},
		{name: "missing transaction", storedTrade: "", callbackTrade: "trade_1", expected: 12.34, callback: 12.34, expectedCCY: "USD", callbackCCY: "USD", wantErr: true},
		{name: "non finite amount", storedTrade: "trade_1", callbackTrade: "trade_1", expected: 12.34, callback: math.NaN(), expectedCCY: "USD", callbackCCY: "USD", wantErr: true},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			err := validatePaidOrderCallback(tt.storedTrade, tt.callbackTrade, tt.expected, tt.callback, tt.expectedCCY, tt.callbackCCY)
			if (err != nil) != tt.wantErr {
				t.Fatalf("validatePaidOrderCallback() error=%v, wantErr=%v", err, tt.wantErr)
			}
		})
	}
}
