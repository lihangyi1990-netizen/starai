...
func TestNormalizePicoCatalogPricingPublishesValidPrice(t *testing.T) {
	rule, enabled, err := normalizePicoCatalogPricing(map[string]interface{}{
		"billing_type": "per_request", "unit_price": float64(0.2), "currency": "算力",
	}, true)
	if err != nil {
		t.Fatal(err)
	}
	if !enabled || rule["pico_pricing_status"] != "published" {
		t.Fatalf("rule=%#v enabled=%v", rule, enabled)
	}
}

func TestNormalizePicoCatalogPricingKeepsValidPriceWhenDisabled(t *testing.T) {
	rule, enabled, err := normalizePicoCatalogPricing(map[string]interface{}{
		"billing_type": "per_request", "unit_price": float64(0.2),
	}, false)
	if err != nil {
		t.Fatal(err)
	}
...
func TestNormalizePicoCatalogPricingRejectsPublishingZeroPrice(t *testing.T) {
	rule, enabled, err := normalizePicoCatalogPricing(map[string]interface{}{
		"billing_type": "per_request", "unit_price": float64(0),
	}, true)
	if err == nil {
		t.Fatal("zero price should not be publishable")
	}
	if enabled || rule["pico_pricing_status"] != "pending" {
		t.Fatalf("rule=%#v enabled=%v", rule, enabled)
	}
}

func TestNormalizePicoCatalogPricingRejectsInvalidBillingType(t *testing.T) {
	if _, _, err := normalizePicoCatalogPricing(map[string]interface{}{
		"billing_type": "unsupported", "unit_price": float64(1),
	}, false); err == nil {
		t.Fatal("invalid billing type should be rejected")
	}
}
