...
// UsageSummaryItem is an aggregated view of billable operations. It is built
// from settled wallet transactions, so a frozen or failed request is not
// counted until it has actually charged the user's balance.
...
// GetUsageSummary returns usage grouped by operation and model for one user.
// The source of truth is wallet_transactions, joined to the originating call
// or task for model and token metadata. This keeps the displayed totals equal
// to the amount actually deducted from the wallet.
func (s *WalletService) GetUsageSummary(ctx context.Context, userID int64) (*UsageSummary, error) {
	rows, err := s.db.Query(ctx, `
		WITH settled_usage AS (
			SELECT
				'chat'::text AS operation_type,
				COALESCE(m.code, 'unknown') AS model_code,
				COALESCE(m.display_name, m.code, '未识别模型') AS model_name,
				wt.amount::double precision AS amount,
				COALESCE(l.prompt_tokens, 0)::bigint AS input_tokens,
				COALESCE(l.completion_tokens, 0)::bigint AS output_tokens,
				COALESCE(l.total_tokens, 0)::bigint AS total_tokens,
				wt.created_at
			FROM wallet_transactions wt
			LEFT JOIN ai_call_logs l
				ON l.user_id = wt.user_id AND l.request_id = wt.ref_id
			LEFT JOIN models m ON m.id = l.model_id
			WHERE wt.user_id = $1 AND wt.direction = 'out'
			  AND wt.type = 'chat_usage' AND wt.ref_type = 'chat'
			...
			FROM wallet_transactions wt
			LEFT JOIN tasks t
				ON t.user_id = wt.user_id AND t.task_no = wt.ref_id
			LEFT JOIN models m ON m.id = t.model_id
			WHERE wt.user_id = $1 AND wt.direction = 'out'
			  AND wt.type IN ('image_usage', 'video_usage', 'audio_usage')
			  AND wt.ref_type = 'task'

			UNION ALL

			SELECT
				'workflow'::text AS operation_type,
				COALESCE(wd.code, 'workflow') AS model_code,
				COALESCE(wd.name, '工作流') AS model_name,
				wt.amount::double precision AS amount,
				0::bigint AS input_tokens,
				0::bigint AS output_tokens,
				0::bigint AS total_tokens,
				wt.created_at
			FROM wallet_transactions wt
			LEFT JOIN workflow_projects wp
				ON wp.user_id = wt.user_id AND wp.public_id = wt.ref_id
			LEFT JOIN workflow_definitions wd ON wd.id = wp.workflow_id
			WHERE wt.user_id = $1 AND wt.direction = 'out'
			  AND wt.type = 'workflow_usage' AND wt.ref_type = 'workflow'
		)
		SELECT operation_type, model_code, model_name,
		       COUNT(*)::int,
		       COALESCE(AVG(amount), 0)::double precision,
		       ...
		       MAX(created_at)
		FROM settled_usage
		GROUP BY operation_type, model_code, model_name
		ORDER BY MAX(created_at) DESC, operation_type, model_name`, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()

	out := &UsageSummary{ByType: map[string]int{}, Items: make([]UsageSummaryItem, 0)}
	for rows.Next() {
		var item UsageSummaryItem
		var latest time.Time
		if err := rows.Scan(&item.OperationType, &item.ModelCode, &item.ModelName, &item.Count,
			&item.UnitCost, &item.TotalCost, &item.InputTokens, &item.OutputTokens,
			&item.TotalTokens, &latest); err != nil {
			return nil, err
		}
		item.LastUsedAt = latest.Format(time.RFC3339)
		out.TotalOperations += item.Count
		out.TotalCost += item.TotalCost
		out.ByType[item.OperationType] += item.Count
...
func (s *WalletService) ListTransactions(ctx context.Context, userID int64, page, pageSize int) ([]TransactionItem, int, error) {
	...
	offset := (page - 1) * pageSize
	var total int
	err := s.db.QueryRow(ctx, `SELECT COUNT(*) FROM wallet_transactions WHERE user_id=$1 AND type NOT IN ('daily_checkin','signup_bonus')`, userID).Scan(&total)
	if err != nil {
		return nil, 0, err
	}
	rows, err := s.db.Query(ctx, `
		SELECT id, type, direction, amount, balance_after, ref_type, ref_id, remark, created_at
		FROM wallet_transactions WHERE user_id=$1 AND type NOT IN ('daily_checkin','signup_bonus') ORDER BY created_at DESC LIMIT $2 OFFSET $3`,
		userID, pageSize, offset)
	if err != nil {
		return nil, 0, err
	}
...
func (s *WalletService) ListRechargeRecords(ctx context.Context, userID int64, page, pageSize int) ([]RechargeRecord, int, error) {
	...
	var total int
	s.db.QueryRow(ctx,
		`SELECT COUNT(*) FROM wallet_transactions WHERE user_id=$1 AND direction='in' AND type IN ('card_recharge','online_recharge','admin_adjust')`,
		userID).Scan(&total)
	rows, err := s.db.Query(ctx, `
		SELECT id, type, amount, remark, created_at FROM wallet_transactions
		WHERE user_id=$1 AND direction='in' AND type IN ('card_recharge','online_recharge','admin_adjust')
