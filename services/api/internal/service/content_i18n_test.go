...
func TestDefaultContentTargetLocalesIncludesItalian(t *testing.T) {
	for _, locale := range defaultContentTargetLocales {
		if locale == "it-IT" {
			return
		}
	}
	t.Fatal("default content target locales must include it-IT")
}
