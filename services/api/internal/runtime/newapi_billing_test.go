...
func TestChatCompletionWithConfigKeepsGlobalTokenWhenConnectionKeyIsEmpty(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if got := r.Header.Get("Authorization"); got != "Bearer global-gateway-token" {
			t.Errorf("authorization = %q, want global gateway token", got)
		}
		w.Header().Set("Content-Type", "application/json")
	...
	client := NewClient(server.URL, "global-gateway-token", 5, 5)
	_, err := client.ChatCompletionWithConfig(context.Background(), "/v1/chat/completions", ChatRequest{
		Model: "synced-model", Messages: []ChatMessage{{Role: "user", Content: "hello"}},
