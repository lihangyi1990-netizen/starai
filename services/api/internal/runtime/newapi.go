...
func (c *Client) resolveConfig(extra map[string]interface{}) RequestConfig {
	...
	// Synchronized Sub2API catalog entries intentionally omit a per-model
	// credential. Keep the client-level gateway token in that case. An empty
	// connection.api_key must never erase the configured gateway credential.
...
func (c *Client) ListModels(ctx context.Context, extra map[string]interface{}) ([]UpstreamModel, error) {
	...
	for _, item := range payload.Models {
		...
		if id != "" {
			items = append(items, UpstreamModel{ID: id, Object: "model", OwnedBy: item.OwnedBy})
		}
	}
	return items, nil
}
