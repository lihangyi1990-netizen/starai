...
func TestPersistGeneratedAudioURLMirrorsExternalMedia(t *testing.T) {
	audio := append([]byte("ID3\x04\x00\x00\x00\x00\x00\x21"), []byte("audio-audio-audio-audio")...)
	...
	root := t.TempDir()
	store, err := storage.NewLocal(root, "http://127.0.0.1:1/uploads-local")
	if err != nil {
		t.Fatal(err)
	}
	...
	got, err := persistGeneratedMedia(context.Background(), connectionConfig{}, upstream.URL+"/audio.mp3", "task_audio", "audio_1", "audio", 100<<20, "")
	if err != nil {
		t.Fatalf("persistGeneratedMedia: %v", err)
	}
	if got == "" || got == upstream.URL+"/audio.mp3" {
		t.Fatalf("persisted URL = %q, want a managed object URL", got)
	}
	key := store.ObjectKeyFromURL(got)
	if key == "" {
		t.Fatalf("could not derive object key from %q", got)
	}
	stored, err := store.ReadAll(context.Background(), key, 100<<20)
	if err != nil {
		t.Fatalf("read persisted audio: %v", err)
	}
	if string(stored) != string(audio) {
		t.Fatalf("persisted audio differs: got %d bytes, want %d", len(stored), len(audio))
	}
}
...
func TestParseConnectionPreservesGatewayCredentialForEmptyConnectionKey(t *testing.T) {
	cfg := parseConnection(map[string]interface{}{
		"connection": map[string]interface{}{
			"base_url":  " ",
			"api_key":   "",
			"auth_type": " ",
		},
	}, "http://sub2api:8080/", "gateway-token")
	if cfg.BaseURL != "http://sub2api:8080" {
		t.Fatalf("base URL = %q", cfg.BaseURL)
	}
	if cfg.APIKey != "gateway-token" {
		t.Fatalf("API key = %q, want global gateway token", cfg.APIKey)
	}
	if cfg.AuthType != "bearer" {
		t.Fatalf("auth type = %q, want bearer", cfg.AuthType)
	}
}
...
func TestJoinBaseEndpointNormalizesMissingSlash(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"http://sub2api:8080/v1", "/v1/chat/completions", "http://sub2api:8080/v1/chat/completions"},
		{"http://sub2api:8080/v1beta", "/v1beta/models/gemini:generateContent", "http://sub2api:8080/v1beta/models/gemini:generateContent"},
	} {
		if got := joinBaseEndpoint(tc.base, tc.endpoint); got != tc.want {
			t.Errorf("joinBaseEndpoint(%q,%q)=%q, want %q", tc.base, tc.endpoint, got, tc.want)
		}
	}
}
