...
routeLoop:
	...
	if len(resultData) == 0 && !(isImage && isVideoImageAPI(endpoint, newAPIModel)) {
	...
	if isVideo {
		...
		for i, item := range resultData {
			...
			// mirrorUpstreamMedia only handles credentialed same-origin /content
			// URLs, so public CDN links reach this point untouched -- and they are
			// not playable as-is: Aliyun OSS and Volces TOS serve them with
			// Content-Disposition: attachment, which makes <video> download the file
			// instead of playing it, and TOS objects expire within a day. Persist
			// them the same way the image branch below already persists its URL
			// results. Already-mirrored URLs are skipped by isManagedMediaURL.
			if !isManagedMediaURL(videoURL) {
				stored, err := persistGeneratedMedia(ctx, conn, videoURL, p.TaskNo, fmt.Sprintf("video_%d", i+1), "video", 250<<20, "")
				if err != nil {
			...
			if itemThumb == "" || itemThumb == upstreamVideoURL {
				// These providers return the mp4 itself as the "thumbnail"; once the
				// video is local the upstream copy is no better as a poster.
				itemThumb = videoURL
			}
		...
		// Audio providers commonly return short-lived CDN URLs. Mirror external
		// results into the configured object store so completed works remain
		// playable after the provider URL expires. Managed/data URLs are ignored
		// by persistGeneratedMedia and therefore remain unchanged.
		if stored, err := persistGeneratedMedia(ctx, conn, audioURL, p.TaskNo, "audio_1", "audio", 100<<20, ""); err != nil {
		...
		targetImageSize, _ := p.Input["resolved_size"].(string)
		if targetImageSize == "" {
			targetImageSize, _ = p.Input["size"].(string)
		}
		...
		for idx, item := range resultData {
			...
			if item.B64JSON != "" {
				if stored, err := storeBase64MediaResult(ctx, p.TaskNo, idx+1, item.B64JSON, item.MimeType, "image", targetImageSize); err != nil {
			...
			if url != "" && item.B64JSON == "" {
				stored, err := persistGeneratedMedia(ctx, conn, url, p.TaskNo, fmt.Sprintf("image_%d", idx+1), "image", 50<<20, targetImageSize)
				if err != nil {
...
func loadWorkerModelRoutes(ctx context.Context, pool *pgxpool.Pool, modelID int64, fallbackBaseURL, fallbackToken, legacyModel, legacyEndpoint string, legacyExtra, legacyRuntime map[string]interface{}) ([]workerModelRoute, error) {
	...
	for rows.Next() {
		...
		// A model route may intentionally omit credentials when it is meant to
		// use the shared Sub2API gateway token.  Preserve the worker-level
		// fallback in that case; otherwise the presence of an otherwise useful
		// route would silently replace the token with an empty Authorization
		// header.  Explicit auth_type=none still remains unauthenticated.
		...
		if strings.TrimSpace(apiKey) == "" && !strings.EqualFold(strings.TrimSpace(authType), "none") {
...
func executeWorkerGenerationAttempt(ctx context.Context, pool *pgxpool.Pool, p ImageTaskPayload, route workerModelRoute, isVideo, isAudio, isImage bool, prompt string) (workerGenerationAttemptResult, error) {
	...
	if isVideo {
		...
		body, _ = json.Marshal(videoparams.BuildUpstreamAudioPayload(p.ModelCode, upstreamModel, route.RuntimeRule, extraParams, p.Input))
	} else {
		if endpoint == "" {
		...
		if isGeminiNativeImageAPI(endpoint, upstreamModel) {
		...
		} else if isChatCompletionsImageAPI(endpoint) {
			body, _ = json.Marshal(buildChatImagePayload(ctx, upstreamModel, p.ModelCode, prompt, p.Input))
			...
			imageBody := buildOpenAIImagePayload(ctx, upstreamModel, p.ModelCode, prompt, size, p.Input["reference_images"])
			// Keep the resolved endpoint on the result when the fan-out path
			// returns early (the normal path assigns it after payload build).
			result.Endpoint = endpoint
			// OpenAI-compatible image gateways commonly support one image per
			// request even when the UI asks for multiple results. Fan out here so
			// the provider never sees the unsupported n/count fields.
			if result.GenerationCount > 1 {
				batchResults := make([]mediaItem, 0, result.GenerationCount)
				var lastResponse []byte
				for index := 0; index < result.GenerationCount; index++ {
					batchBody := make(map[string]interface{}, len(imageBody))
					for key, value := range imageBody {
						batchBody[key] = value
					}
					normalizePayloadMedia(ctx, batchBody, endpoint)
					requestBody, _ := json.Marshal(batchBody)
					batchConnection := route.Connection
					batchConnection.Headers = make(map[string]string, len(route.Connection.Headers)+1)
					for key, value := range route.Connection.Headers {
						batchConnection.Headers[key] = value
					}
					batchConnection.Headers["Idempotency-Key"] = fmt.Sprintf("%s-%d", p.TaskNo, index+1)
					...
					response, status, requestErr := doJSONRequestWithLimit(ctx, batchConnection, "POST", joinBaseEndpoint(batchConnection.BaseURL, resolveModelEndpoint(endpoint, upstreamModel)), requestBody, timeout, 96<<20)
					if requestErr != nil {
					...
					items, upstreamTaskID := parseUpstreamMedia(response)
					if upstreamTaskID != "" || len(items) == 0 {
						return result, fmt.Errorf("upstream batch image request did not return an image")
					}
					batchResults = append(batchResults, items...)
					lastResponse = response
				}
				result.ResponseBody = lastResponse
				result.ResultData = batchResults
				return result, nil
			}
...
func parseConnection(extra map[string]interface{}, fallbackBaseURL, fallbackToken string) connectionConfig {
	...
	if s, ok := conn["base_url"].(string); ok && strings.TrimSpace(s) != "" {
		cfg.BaseURL = trimRightSlash(strings.TrimSpace(s))
	}
	if s, ok := conn["api_key"].(string); ok {
		// Gateway-synchronised catalog entries may carry an empty connection
		// object for metadata only. Preserve the worker's global Sub2API token
		// unless an explicit per-route credential was configured.
		if value := strings.TrimSpace(s); value != "" {
			cfg.APIKey = value
		}
	}
	if s, ok := conn["auth_type"].(string); ok && strings.TrimSpace(s) != "" {
		cfg.AuthType = strings.TrimSpace(s)
	}
	if s, ok := conn["api_key_header"].(string); ok && strings.TrimSpace(s) != "" {
		cfg.APIKeyHeader = strings.TrimSpace(s)
	}
...
func joinBaseEndpoint(baseURL, endpoint string) string {
	...
	// Accept either a host-only gateway URL or a URL that already contains the
	// common protocol prefix. This mirrors the API runtime client and avoids
	// accidentally calling `/v1/v1/...` when an operator stores
	// `http://sub2api:8080/v1` in the shared route configuration.
...
func storeBase64MediaResult(ctx context.Context, taskNo string, idx int, raw, contentType, kind, targetSize string) (string, error) {
	if objectStore == nil {
		return "", nil
	}
	...
	if kind == "image" && strings.TrimSpace(targetSize) != "" {
		normalized, normalizeErr := enforceImageOutputSize(ctx, data, targetSize)
		if normalizeErr != nil {
			log.Printf("Task %s normalize base64 image #%d failed: %v", taskNo, idx, normalizeErr)
		} else {
			data = normalized
			contentType = "image/png"
		}
...
// isChatCompletionsImageAPI marks image models whose gateway exposes no Images
// API at all. std.synapse.ac.cn answers /v1/images/generations with
// "Images API is not supported for this platform", but serves nano-banana-pro
// over /v1/chat/completions and returns the picture inside the assistant
// message. Selection is by endpoint only: the same model name is served through
// a real Images API elsewhere, so the wire format must stay explicit.
func isChatCompletionsImageAPI(endpoint string) bool {
	return strings.Contains(strings.ToLower(strings.TrimSpace(endpoint)), "/chat/completions")
}

// buildChatImagePayload renders an image request as a single chat turn.
// Reference images ride along as image_url parts. normalizePayloadMedia only
// walks a top-level content array, never messages[], so the references are
// normalized here instead.
func buildChatImagePayload(ctx context.Context, newAPIModel, fallbackModel, prompt string, input map[string]interface{}) map[string]interface{} {
	model := strings.TrimSpace(newAPIModel)
	if model == "" {
		model = strings.TrimSpace(fallbackModel)
	}
	refs := collectBananaReferenceImages(ctx, input["reference_images"])
	for _, key := range []string{"images", "image", "image_url", "reference_image"} {
		if len(refs) > 0 {
			break
		}
		refs = collectBananaReferenceImages(ctx, input[key])
	}
	if len(refs) > 5 {
		refs = refs[:5]
	}
	...
	if len(refs) > 0 {
		...
		for _, ref := range refs {
			parts = append(parts, map[string]interface{}{
				"type":      "image_url",
				"image_url": map[string]interface{}{"url": ref},
			})
		}
		content = parts
	}
...
func isGeminiNativeImageAPI(endpoint, model string) bool {
	// A model name alone is not enough to select the Gemini native wire format:
	// Sub2API exposes Gemini image models through the OpenAI-compatible
	// /v1/images/generations endpoint. Only an explicitly native endpoint may
	// receive the generateContent request body.
	endpoint = strings.ToLower(strings.TrimSpace(endpoint))
	return strings.Contains(endpoint, ":generatecontent") || strings.Contains(endpoint, "/v1beta/models/")
}

// buildOpenAIImagePayload keeps the OpenAI-compatible image contract narrow.
// UI-only controls are resolved to a pixel size before this function runs and
// must not leak into providers that reject unknown fields.
func buildOpenAIImagePayload(ctx context.Context, upstreamModel, fallbackModel, prompt, size string, refs interface{}) map[string]interface{} {
	...
	if refs != nil {
		...
		if hasReferenceImageValue(normalized) {
...
func hasReferenceImageValue(value interface{}) bool {
	switch v := value.(type) {
	case string:
...
func buildGeminiNativeImagePayload(ctx context.Context, newAPIModel, fallbackModel, prompt string, input map[string]interface{}) map[string]interface{} {
	...
	if strings.Contains(lowerModel, "gemini-3.1-flash-image") || strings.Contains(lowerModel, "nano-banana-pro") {
...
const (
	customImageDimensionMin       = 64
	customImageDimensionMax       = 4096
	customImageDimensionMaxPixels = int64(16_000_000)
)
...
func workerImageValuePresent(value interface{}) bool {
	if value == nil {
		return false
	}
	if text, ok := value.(string); ok {
...
func workerImageDimension(value interface{}) (int, bool) {
	switch v := value.(type) {
	case int:
		if v >= customImageDimensionMin && v <= customImageDimensionMax {
			return v, true
		}
	case int64:
		if v >= customImageDimensionMin && v <= int64(customImageDimensionMax) {
			return int(v), true
		}
	case float64:
		if math.Trunc(v) == v && v >= customImageDimensionMin && v <= customImageDimensionMax {
			return int(v), true
		}
	case float32:
		if math.Trunc(float64(v)) == float64(v) && v >= customImageDimensionMin && v <= float32(customImageDimensionMax) {
			return int(v), true
		}
	case json.Number:
		n, err := strconv.ParseInt(strings.TrimSpace(string(v)), 10, 64)
		if err == nil && n >= customImageDimensionMin && n <= int64(customImageDimensionMax) {
			return int(n), true
		}
	case string:
		n, err := strconv.Atoi(strings.TrimSpace(v))
		if err == nil && n >= customImageDimensionMin && n <= customImageDimensionMax {
			return n, true
		}
	}
	return 0, false
...
func workerImageSizeDimensions(value interface{}) (int, int, bool) {
	raw := strings.TrimSpace(fmt.Sprint(value))
	parts := strings.Split(strings.ToLower(raw), "x")
	if len(parts) != 2 {
		return 0, 0, false
	}
	width, widthOK := workerImageDimension(parts[0])
	height, heightOK := workerImageDimension(parts[1])
	if !widthOK || !heightOK || int64(width)*int64(height) > customImageDimensionMaxPixels {
		return 0, 0, false
	}
	return width, height, true
}

func isStandardImageSize(value string) bool {
	value = strings.TrimSpace(value)
	for _, byTier := range standardImageSizes {
		for _, size := range byTier {
			if value == size {
				return true
			}
		}
...
func workerImageDimensionRatio(width, height int) string {
	if width <= 0 || height <= 0 {
		return "1:1"
	}
	...
	for ratio := range standardImageSizes {
		...
		distance := math.Abs(aspect - numerator/denominator)
...
func customImageDimensions(input map[string]interface{}) (int, int, bool) {
	width, widthSet := input["width"]
	if !widthSet || !workerImageValuePresent(width) {
		width, widthSet = input["custom_width"]
	}
	height, heightSet := input["height"]
	if !heightSet || !workerImageValuePresent(height) {
		height, heightSet = input["custom_height"]
	}
	if widthSet && heightSet && workerImageValuePresent(width) && workerImageValuePresent(height) {
		parsedWidth, widthOK := workerImageDimension(width)
		parsedHeight, heightOK := workerImageDimension(height)
		if widthOK && heightOK && int64(parsedWidth)*int64(parsedHeight) <= customImageDimensionMaxPixels {
			return parsedWidth, parsedHeight, true
		}
		return 0, 0, false
	}
	// API validation normally canonicalizes explicit dimensions. This fallback
	// also supports older/API-key tasks that sent a non-standard size string.
	if !widthSet && !heightSet {
		if raw := strings.TrimSpace(fmt.Sprint(input["size"])); raw != "" && !isStandardImageSize(raw) {
			return workerImageSizeDimensions(raw)
		}
	}
	return 0, 0, false
}
...
func resolveImageGenerationInput(input map[string]interface{}, runtimeRule map[string]interface{}, endpoint, model string) {
	ratioSource := strings.TrimSpace(fmt.Sprint(input["aspect_ratio"]))
	if ratioSource == "" || ratioSource == "<nil>" {
		ratioSource = fmt.Sprint(input["ratio"])
	}
	ratio := normalizeImageRatio(ratioSource)
	customWidth, customHeight, hasCustomSize := customImageDimensions(input)
	if hasCustomSize {
		// Providers generally accept a finite ratio enum. Select the closest
		// canonical ratio for their request, then crop/scale the result to the
		// exact custom canvas below.
		ratio = workerImageDimensionRatio(customWidth, customHeight)
	}
	...
	tierSource := strings.TrimSpace(fmt.Sprint(input["image_size"]))
	if tierSource == "" || tierSource == "<nil>" {
		tierSource = fmt.Sprint(input["quality"])
	}
	tier := normalizeImageSizeTier(tierSource)
	...
	// Keep the user's requested canvas separate from the size sent upstream.
	// OpenAI-compatible gateways generally accept only the three canonical
	// image sizes below; arbitrary Gemini-style dimensions (for example
	// 3840x2160) are rejected as an unknown/unsupported parameter. The worker
	// can still normalize the returned image to resolved_size after download.
	requestedSize := imagePixelSize(ratio, tier)
	if hasCustomSize {
		requestedSize = fmt.Sprintf("%dx%d", customWidth, customHeight)
	}
	upstreamSize := requestedSize
	if isOpenAICompatibleImageAPI(endpoint, model) {
		if hasCustomSize {
			upstreamSize = safeOpenAIImageUpstreamSizeForDimensions(customWidth, customHeight)
		} else {
			upstreamSize = safeOpenAIImageUpstreamSize(ratio)
		}
	}
	input["aspect_ratio"] = ratio
	input["image_size"] = tier
	input["size"] = upstreamSize
	input["resolved_aspect_ratio"] = ratio
	input["resolved_image_size"] = tier
	input["resolved_size"] = requestedSize
	input["upstream_size"] = upstreamSize
	if hasCustomSize {
		input["width"], input["height"] = customWidth, customHeight
		input["custom_width"], input["custom_height"] = customWidth, customHeight
	}
}

// isOpenAICompatibleImageAPI identifies the JSON Images API path where the
// provider contract is the OpenAI size enum. Gemini-native and asynchronous
// Banana/GPT video routes have their own payload contracts and must not use
// this mapping.
func isOpenAICompatibleImageAPI(endpoint, model string) bool {
	if isGeminiNativeImageAPI(endpoint, model) || isVideoImageAPI(endpoint, model) {
		return false
	}
	normalized := strings.TrimRight(strings.ToLower(strings.TrimSpace(endpoint)), "/")
	return strings.HasSuffix(normalized, "/images/generations") ||
		strings.HasSuffix(normalized, "/images/edits")
}

// safeOpenAIImageUpstreamSize maps any supported orientation to a size accepted
// by the OpenAI-compatible Images API. The requested ratio/tier remains in
// resolved_size so the worker can crop/scale the delivered image afterwards.
func safeOpenAIImageUpstreamSize(ratio string) string {
...
func safeOpenAIImageUpstreamSizeForDimensions(width, height int) string {
...
func parseUpstreamMedia(body []byte) ([]mediaItem, string) {
	...
	if err := json.Unmarshal(body, &raw); err != nil {
		if items, upstreamID := parseSSEUpstreamMedia(body); len(items) > 0 || upstreamID != "" {
	...
	if strings.EqualFold(strings.TrimSpace(scalarString(original, "object")), "video") {
		if id := scalarString(original, "id"); id != "" {
			upstreamID = id
		}
	}
	if len(items) > 0 {
...
// parseSSEUpstreamMedia handles Gemini streamGenerateContent responses. Image
// payloads can take longer than a CDN's non-streaming request limit, so the
// native SSE endpoint is used to keep the upstream connection active. Each
// data line is still a normal Gemini generateContent response object.
func parseSSEUpstreamMedia(body []byte) ([]mediaItem, string) {
	items := make([]mediaItem, 0, 1)
	...
	for _, rawLine := range bytes.Split(body, []byte("\n")) {
		...
		if upstreamID == "" {
			upstreamID = scalarString(raw, "task_id", "taskId", "generation_id", "request_id", "id", "responseId")
		}
		for _, item := range extractMediaItems(raw) {
			digest := sha256.Sum256([]byte(item.URL + "\x00" + item.B64JSON + "\x00" + item.MimeType))
...
func extractMediaItems(raw map[string]interface{}) []mediaItem {
	raw = unwrapUpstreamBody(raw)
	if items := extractGeminiImageItems(raw); len(items) > 0 {
		return items
	}
	if items := extractChatCompletionMediaItems(raw); len(items) > 0 {
		return items
	}
...
// markdownMediaPattern matches a markdown image embed and captures its target.
var markdownMediaPattern = regexp.MustCompile(`!\[[^\]]*\]\(\s*<?([^)\s>]+)`)

// dataURIPattern matches a base64 data URI anywhere inside a text blob, for
// gateways that wrap the image in prose instead of a markdown embed.
var dataURIPattern = regexp.MustCompile(`data:[a-zA-Z0-9!#$&^_.+-]+/[a-zA-Z0-9!#$&^_.+-]+;base64,[A-Za-z0-9+/=]{32,}`)

// markdownMediaRefs pulls media targets out of assistant text, in document
// order: markdown embeds first, then a bare data URI, then a bare URL.
func markdownMediaRefs(text string) []string {
	text = strings.TrimSpace(text)
	if text == "" {
		return nil
	}
	matches := markdownMediaPattern.FindAllStringSubmatch(text, -1)
	if len(matches) > 0 {
	...
	if found := dataURIPattern.FindAllString(text, -1); len(found) > 0 {
		return found
	}
	if isHTTPURL(text) && !strings.ContainsAny(text, " \t\n") {
		return []string{text}
	}
	return nil
}

// extractChatCompletionMediaItems pulls generated media out of an OpenAI chat
// response. Gateways that expose an image model over the chat protocol return
// the picture inside the assistant message rather than in a data[] array, most
// commonly as a markdown embed: ![image](data:image/jpeg;base64,...).
func extractChatCompletionMediaItems(raw map[string]interface{}) []mediaItem {
	choices, ok := raw["choices"].([]interface{})
	if !ok || len(choices) == 0 {
		return nil
	}
	items := make([]mediaItem, 0, len(choices))
	seen := make(map[string]struct{})
	add := func(ref, mime string) {
		...
		if isHTTPURL(ref) {
			seen[ref] = struct{}{}
			items = append(items, mediaItem{URL: ref})
			return
		}
		if looksLikeEncodedMedia(ref) {
			seen[ref] = struct{}{}
			items = append(items, mediaItem{B64JSON: ref, MimeType: mime})
		}
	}
	for _, rawChoice := range choices {
		...
		// Some gateways mirror the media into a sibling array instead of, or as
		// well as, embedding it in the text.
		if it, ok := mediaItemFromValue(msg["images"], msg); ok {
			add(firstNonEmptyMediaRef(it), it.MimeType)
		}
		switch content := msg["content"].(type) {
		case string:
			for _, ref := range markdownMediaRefs(content) {
				add(ref, "")
			}
		case []interface{}:
			for _, rawPart := range content {
				...
				if it, ok := mediaItemFromMap(part); ok {
					add(firstNonEmptyMediaRef(it), it.MimeType)
					continue
				}
				if text, ok := part["text"].(string); ok {
					for _, ref := range markdownMediaRefs(text) {
						add(ref, "")
					}
				}
			}
...
func firstNonEmptyMediaRef(it mediaItem) string {
...
func mediaItemFromMap(m map[string]interface{}) (mediaItem, bool) {
	if inline, ok := m["inlineData"].(map[string]interface{}); ok {
		if b64 := firstString(inline, "data"); b64 != "" && looksLikeEncodedMedia(b64) {
			return mediaItem{B64JSON: b64, MimeType: firstString(inline, "mimeType", "mime_type", "contentType", "content_type")}, true
		}
	}
	if inline, ok := m["inline_data"].(map[string]interface{}); ok {
		if b64 := firstString(inline, "data"); b64 != "" && looksLikeEncodedMedia(b64) {
			return mediaItem{B64JSON: b64, MimeType: firstString(inline, "mimeType", "mime_type", "contentType", "content_type")}, true
		}
	}
	if file, ok := m["fileData"].(map[string]interface{}); ok {
		if mediaURL := firstMediaURL(file, "fileUri", "file_uri", "uri"); mediaURL != "" {
			return mediaItem{URL: mediaURL}, true
		}
	}
	if file, ok := m["file_data"].(map[string]interface{}); ok {
		if mediaURL := firstMediaURL(file, "fileUri", "file_uri", "uri"); mediaURL != "" {
			return mediaItem{URL: mediaURL}, true
		}
	}
...
// extractGeminiImageItems handles the native generateContent response shape:
// candidates[].content.parts[].inlineData (or fileData). Restrict traversal
// to Gemini's known container keys so metadata strings are not misclassified.
func extractGeminiImageItems(raw map[string]interface{}) []mediaItem {
	items := make([]mediaItem, 0, 1)
	...
	walk = func(value interface{}) {
		...
		case map[string]interface{}:
			for _, key := range []string{"inlineData", "inline_data"} {
				if inline, ok := v[key].(map[string]interface{}); ok {
					if b64 := firstString(inline, "data"); b64 != "" && looksLikeEncodedMedia(b64) {
						items = append(items, mediaItem{
							B64JSON:  b64,
							MimeType: firstString(inline, "mimeType", "mime_type", "contentType", "content_type"),
						})
					}
				}
			}
			for _, key := range []string{"fileData", "file_data"} {
				if file, ok := v[key].(map[string]interface{}); ok {
					if mediaURL := firstMediaURL(file, "fileUri", "file_uri", "uri"); mediaURL != "" {
						items = append(items, mediaItem{URL: mediaURL})
					}
				}
			}
...
func firstSuccessMediaURL(raw map[string]interface{}, upstreamID string, conn connectionConfig) string {
	...
	if metadata, ok := raw["metadata"].(map[string]interface{}); ok {
		if u := firstDirectMediaURL(metadata); u != "" {
			return u
		}
	}
...
// enforceImageOutputSize makes the delivered image match the selected canvas when an upstream
// gateway ignores its size controls. The filter is constructed solely from validated dimensions.
func enforceImageOutputSize(ctx context.Context, data []byte, targetSize string) ([]byte, error) {
	parts := strings.Split(strings.ToLower(strings.TrimSpace(targetSize)), "x")
	if len(parts) != 2 {
		return data, nil
	}
	width, widthErr := strconv.Atoi(strings.TrimSpace(parts[0]))
	height, heightErr := strconv.Atoi(strings.TrimSpace(parts[1]))
	if widthErr != nil || heightErr != nil || width < 64 || height < 64 || width > 4096 || height > 4096 || int64(width)*int64(height) > 16_000_000 {
		return data, nil
	}
	input, err := os.CreateTemp("", "starai-image-*")
	if err != nil {
		return nil, err
	}
	...
	resizeCtx, cancel := context.WithTimeout(ctx, 45*time.Second)
	defer cancel()
	filter := fmt.Sprintf("scale=%d:%d:force_original_aspect_ratio=increase,crop=%d:%d", width, height, width, height)
	command := exec.CommandContext(resizeCtx, "ffmpeg", "-hide_banner", "-loglevel", "error", "-y", "-i", inputPath, "-vf", filter, "-frames:v", "1", outputPath)
	if output, err := command.CombinedOutput(); err != nil {
		return nil, fmt.Errorf("ffmpeg image resize: %w: %s", err, truncateText(string(output), 240))
	}
	result, err := os.ReadFile(outputPath)
	if err != nil {
		return nil, err
	}
	if len(result) == 0 || len(result) > 50<<20 {
		return nil, fmt.Errorf("normalized image size is invalid: %d bytes", len(result))
	}
	return result, nil
}
...
func persistGeneratedMedia(ctx context.Context, conn connectionConfig, mediaURL, taskNo, itemName, kind string, maxBytes int64, targetSize string) (string, error) {
	mediaURL = strings.TrimSpace(mediaURL)
	...
	if kind == "image" && strings.TrimSpace(targetSize) != "" {
		normalized, normalizeErr := enforceImageOutputSize(ctx, data, targetSize)
		if normalizeErr != nil {
			log.Printf("Task %s normalize external image failed: %v", taskNo, normalizeErr)
		} else {
			data = normalized
			contentType = "image/png"
		}
