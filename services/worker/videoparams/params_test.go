...
func TestBuildUpstreamAudioPayloadUsesInputWithoutLeakingPrompt(t *testing.T) {
	got := BuildUpstreamAudioPayload(
		"grok-voice-latest",
		"grok-voice-latest",
		nil,
		nil,
		map[string]interface{}{
			"prompt": "internal prompt value",
			"input":  "spoken text",
			"voice":  "alloy",
			"format": "mp3",
		},
	)
	if got["model"] != "grok-voice-latest" || got["input"] != "spoken text" {
		t.Fatalf("unexpected audio payload: %#v", got)
	}
	if _, ok := got["prompt"]; ok {
		t.Fatalf("internal prompt must not leak into audio payload: %#v", got)
	}
	if got["voice"] != "alloy" || got["format"] != "mp3" {
		t.Fatalf("audio options missing: %#v", got)
	}
}

func TestBuildUpstreamAudioPayloadHonorsMappedTextField(t *testing.T) {
	got := BuildUpstreamAudioPayload(
		"custom-audio", "custom-audio",
		map[string]interface{}{"upstream": map[string]interface{}{
			"map": map[string]interface{}{"input": "text", "voice": "voice_id"},
			"include": []interface{}{"voice"},
		}},
		nil,
		map[string]interface{}{"input": "hello", "prompt": "fallback", "voice": "narrator"},
	)
	if got["text"] != "hello" || got["voice_id"] != "narrator" {
		t.Fatalf("mapped audio payload = %#v", got)
	}
	if _, ok := got["input"]; ok {
		t.Fatalf("mapped input key should replace canonical input: %#v", got)
	}
}
...
func TestBuildAILXPSeedancePayloadUsesPublicMetadataContract(t *testing.T) {
	payload := BuildUpstreamVideoPayload(
		"seedance-test",
		"seedance2.0-720",
		map[string]interface{}{
			...
			"upstream": map[string]interface{}{
				"adapter": "ailxp_seedance_2",
				"include": []interface{}{"duration", "orientation", "reference_images"},
			},
		},
		nil,
		map[string]interface{}{
			...
			"generation_mode":  "image",
			"reference_images": []interface{}{"https://example.com/reference.jpg"},
			"reference_asset_ids": []interface{}{"ast_private"},
		},
	)
	if payload["seconds"] != "4" {
		t.Fatalf("seconds = %#v, want 4", payload["seconds"])
	}
	if payload["prompt"] != "keep the subject consistent" {
	...
	if metadata["ratio"] != "9:16" || metadata["resolution"] != "720p" {
		t.Fatalf("metadata = %#v", metadata)
	}
	content, ok := metadata["content"].([]interface{})
	if !ok || len(content) != 1 {
		t.Fatalf("content = %#v", metadata["content"])
	}
	item, ok := content[0].(map[string]interface{})
	if !ok || item["type"] != "image_url" {
		t.Fatalf("content item = %#v", content[0])
	}
	image, ok := item["image_url"].(map[string]interface{})
	if !ok || image["url"] != "https://example.com/reference.jpg" {
		t.Fatalf("image item = %#v", item)
	}
	for _, key := range []string{
		"duration", "orientation", "reference_images", "reference_asset_ids",
		"portrait_asset_id",
	} {
		if _, exists := payload[key]; exists {
			t.Fatalf("%s leaked into payload: %#v", key, payload)
		}
	}
}
