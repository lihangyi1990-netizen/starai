...
func BuildUpstreamVideoPayload(
	...
	if strings.EqualFold(upCfg.Adapter, "ailxp_seedance_2") {
		out = buildAILXPSeedancePayload(out, params)
	}
...
// BuildUpstreamAudioPayload builds a provider-neutral TTS request. Audio
// gateways commonly use `input` (OpenAI/xAI) or a provider-specific text
// field, while the PICO task model stores the user text as `prompt`. Keeping
// this path separate from the video mapper prevents the internal `prompt`
// field from leaking into strict audio APIs that reject unknown fields.
func BuildUpstreamAudioPayload(
	modelCode, newAPIModel string,
	runtimeRule, extraParams map[string]interface{},
	params map[string]interface{},
) map[string]interface{} {
	upCfg := parseUpstreamConfig(runtimeRule)
	modelName := strings.TrimSpace(newAPIModel)
	if modelName == "" {
		modelName = strings.TrimSpace(modelCode)
	}
	out := map[string]interface{}{}
	setPayloadValue(out, mappedUpstreamKey(upCfg, "model", "model"), modelName)

	// `input` is the canonical public audio field. A legacy task may only have
	// `prompt`, so use it as a fallback. An explicit mapping can target `text`,
	// `lyrics`, or a nested provider field.
	text := strings.TrimSpace(fmt.Sprint(params["input"]))
	if text == "" || text == "<nil>" {
		text = strings.TrimSpace(fmt.Sprint(params["prompt"]))
	}
	if text != "" && text != "<nil>" {
		textKey := mappedUpstreamKey(upCfg, "input", "input")
		if _, explicitlyMapped := upCfg.Map["input"]; !explicitlyMapped {
			if mapped, ok := upCfg.Map["prompt"]; ok && strings.TrimSpace(mapped) != "" {
				textKey = mapped
			}
		}
		setPayloadValue(out, textKey, text)
	}

	if upCfg.Static != nil {
		for key, value := range upCfg.Static {
			out[key] = value
		}
	}
	include := append([]string(nil), upCfg.Include...)
	if len(include) == 0 {
		for key := range params {
			if key == "prompt" || key == "input" || key == "user_prompt" || key == "model" {
				continue
			}
	...
	for _, key := range include {
		val, ok := params[key]
		if !ok || val == nil || omitAutoValue(val) {
			continue
		}
		upKey := mappedUpstreamKey(upCfg, key, key)
		setPayloadValue(out, upKey, normalizeUpstreamValue(val))
	}
	// Do not copy arbitrary task bookkeeping fields or the internal connection
	// object into an upstream audio request. Explicit static/include mappings
	// above are the only fields that cross the provider boundary.
	return out
}
...
func SanitizeUpstreamPayload(out map[string]interface{}, endpoint string) map[string]interface{} {
	...
	platformOnly := []string{
		...
		"catalog_source", "catalog_status", "provider", "provider_source",
...
// buildAILXPSeedancePayload targets AILXP's public /v1/videos contract.
// AILXP accepts public reference URLs inside metadata.content.
func buildAILXPSeedancePayload(out, params map[string]interface{}) map[string]interface{} {
	stringValue := func(raw interface{}) string {
	...
	seconds := stringValue(normalizeVideoDuration(duration))
	if seconds == "" {
		seconds = "4"
	}
	out["seconds"] = seconds
	...
	if ratio == "" {
		ratio = stringValue(params["aspect_ratio"])
	}
	if ratio == "" {
		ratio = stringValue(params["orientation"])
	}
	if ratio == "" {
		ratio = stringValue(out["ratio"])
	}
	if ratio == "" {
		ratio = stringValue(out["aspect_ratio"])
	}
	...
	resolution := stringValue(params["resolution"])
	if resolution == "" {
		resolution = stringValue(out["resolution"])
	}
	if resolution == "" {
		model := strings.ToLower(stringValue(out["model"]))
		if strings.Contains(model, "1080") {
			resolution = "1080p"
		} else {
			resolution = "720p"
		}
	...
	mode := strings.ToLower(stringValue(params["generation_mode"]))
	content := make([]interface{}, 0, 16)
	seenMedia := map[string]bool{}
	addMedia := func(raw interface{}, mediaType string) {
		for _, mediaURL := range mediaURLList(raw) {
			mediaURL = strings.TrimSpace(mediaURL)
			if !isPublicSeedanceMediaReference(mediaURL) || seenMedia[mediaURL] {
				continue
			}
			seenMedia[mediaURL] = true
			content = append(content, map[string]interface{}{
	...
	switch mode {
	case "first_frame":
		addMedia(params["first_frame"], "image")
	case "first_last":
		addMedia(params["first_frame"], "image")
		addMedia(params["last_frame"], "image")
	case "image":
		addMedia(params["reference_images"], "image")
		addMedia(params["image_url"], "image")
		addMedia(params["image"], "image")
	case "video":
		addMedia(params["reference_videos"], "video")
	case "image_audio":
		addMedia(params["reference_images"], "image")
		addMedia(params["image_url"], "image")
		addMedia(params["reference_audios"], "audio")
	case "image_video":
		addMedia(params["reference_images"], "image")
		addMedia(params["image_url"], "image")
		addMedia(params["reference_videos"], "video")
	...
	case "image_video_audio":
		...
		addMedia(params["reference_audios"], "audio")
	case "text":
		// Text-only requests intentionally omit stale references.
	default:
		addMedia(params["reference_images"], "image")
		addMedia(params["reference_videos"], "video")
		addMedia(params["reference_audios"], "audio")
	...
	for _, key := range []string{
		"duration", "generation_mode", "first_frame", "last_frame",
		"portrait_asset_id", "portrait_asset_type", "reference_images",
		"reference_videos", "reference_audios", "reference_audio",
		"image", "image_url", "images", "aspect_ratio", "orientation",
		"ratio", "size", "content", "_preserve_video_params",
		"_video_upload_profile",
	} {
		delete(out, key)
	}
	return out
...
func isPublicSeedanceMediaReference(value string) bool {
