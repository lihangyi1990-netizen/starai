...
if (-not $KeepBuildCache) {
  # Each Next app may use a port-specific development directory (for example
  # .next-dev-3001). Remove only those generated directories under the two
  # known app roots so a later dev session starts with a coherent graph.
  foreach ($app in @("web", "admin")) {
    $appDir = Join-Path $Root "apps/$app"
    if (-not (Test-Path -LiteralPath $appDir -PathType Container)) { continue }
    Get-ChildItem -LiteralPath $appDir -Force -Directory -Filter ".next-dev*" -ErrorAction SilentlyContinue |
      ForEach-Object { Remove-RepoPath (Join-Path "apps/$app" $_.Name) }
  }
}
