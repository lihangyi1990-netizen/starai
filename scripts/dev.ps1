...
if (-not (Test-PortListening 3000)) {
  Start-StarAIServiceWindow "Web (:3000)" (Join-Path $Root "apps/web") "if (Test-Path '.next') { Remove-Item -Recurse -Force '.next' -ErrorAction SilentlyContinue }; Get-ChildItem -Force -Directory -Filter '.next-dev*' -ErrorAction SilentlyContinue | Remove-Item -Recurse -Force -ErrorAction SilentlyContinue; pnpm dev"
}
if (-not (Test-PortListening 3001)) {
  Start-StarAIServiceWindow "Admin (:3001)" (Join-Path $Root "apps/admin") "if (Test-Path '.next') { Remove-Item -Recurse -Force '.next' -ErrorAction SilentlyContinue }; Get-ChildItem -Force -Directory -Filter '.next-dev*' -ErrorAction SilentlyContinue | Remove-Item -Recurse -Force -ErrorAction SilentlyContinue; pnpm dev"
}
